...
| 文档状态 | 待评审（Draft） |
| 最后更新日期 | 2026-09-04（R2 修复：自动化四触发器×五动作对齐 WF-003、§8 排期 WF-002 后端入第 9 周 D1-2、§5 权限行补全、§1 注图例补句、§3 注二软依赖口径；R1：§9/§10 补齐、M11-WF、dg 错位登记、依赖图补边、画布归属、mermaid 孤点） |
...
4. **自动化规则引擎**：触发器（状态变更/任务创建/截止临近/字段变更——四触发器）× 动作（赋值/指派/改状态/通知/加标签——五动作）的受限 DSL（WF-003 §2.2 冻结枚举）——刻意不做图灵完备。
