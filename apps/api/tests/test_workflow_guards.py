...
@pytest.fixture()
def env(db):
    owner = User.objects.create_user(email="g-owner@rabbit.dev", password="Rabbit123!", display_name="管理员")
    member = User.objects.create_user(email="g-member@rabbit.dev", password="Rabbit123!", display_name="成员")
    ws = Workspace.objects.create(name="W", slug=f"w-g-{owner.id.hex[:8]}", owner=owner, created_by=owner)
...
def _graph(env, guards=None, locks=None, name="流程"):
    ...
    WorkflowTransition.objects.create(
        workflow=wf, from_state=s["待办"], to_state=s["进行中"], name="开始", guards=guards or []
    )
    WorkflowTransition.objects.create(workflow=wf, from_state=s["进行中"], to_state=s["已完成"], name="完成")
...
class TestGuardSemantics:
    def test_required_fields_missing(self, env):
        wf = _graph(env, guards=[{"type": "required_fields", "config": {"fields": ["assignees", "target_date"]}}])
        issue = _mk_issue(env)
        ...
        assert all(d["guard"] == "required_fields" and d["code"] == "REQUIRED" for d in ei.value.details)
    ...
    def test_blocker_completed_on_edge(self, env):
        ...
        with pytest.raises(TransitionError) as ei:
            WorkflowService().transition(
                ...
                actor=env["owner"],
                transition_id=str(done_edge.id),
            )
...
class TestGuardPayload:
    def test_payload_completes_in_one_request(self, env):
        wf = _graph(env, guards=[{"type": "required_fields", "config": {"fields": ["target_date"]}}])
        issue = _mk_issue(env)
        WorkflowService().transition(
            issue_id=issue.id,
            to_state_id=env["states"]["进行中"].id,
    ...
    def test_payload_assignees_membership(self, env):
        outsider = User.objects.create_user(email="g-out@rabbit.dev", password="Rabbit123!", display_name="外人")
        wf = _graph(env, guards=[{"type": "required_fields", "config": {"fields": ["assignees"]}}])
        issue = _mk_issue(env)
        with pytest.raises(TransitionError) as ei:
            WorkflowService().transition(
                issue_id=issue.id,
...
class TestFieldLocks:
    ...
    def test_detail_locked_fields(self, env):
        ...
        c = APIClient()
        c.force_authenticate(env["member"])
        resp = c.get(f"/api/v1/workspaces/{env['ws'].slug}/projects/{env['proj'].id}/issues/{issue.id}/")
        assert resp.status_code == 200
...
class TestPatchStateBypass:
    def test_controlled_project_state_patch_rejected(self, env):
        ...
        resp = c.patch(
            f"/api/v1/workspaces/{env['ws'].slug}/projects/{env['proj'].id}/issues/{issue.id}/",
            {"state_id": str(env["states"]["已完成"].id)},
            format="json",
        )
    ...
    def test_free_project_state_patch_still_works(self, env):
        ...
        resp = c.patch(
            f"/api/v1/workspaces/{env['ws'].slug}/projects/{env['proj'].id}/issues/{issue.id}/",
            {"state_id": str(env["states"]["进行中"].id)},
            format="json",
        )
...
class TestAvailablePreview:
    def test_blocked_by_and_deny_reason(self, env):
        ...
        resp = c.get(
            f"/api/v1/workspaces/{env['ws'].slug}/projects/{env['proj'].id}/issues/{issue.id}/transitions/available/"
        )
        assert resp.status_code == 200
        data = resp.json()["data"]
...
class TestGuardConfigValidation:
    def test_engine_field_rejected(self, env):
        ...
        details = guard_config_issues([{"type": "required_fields", "config": {"fields": ["parent"]}}], env["proj"])
        assert any(d["code"] == "INVALID" and "parent" in d["message"] for d in details)

    def test_unknown_cf_field_rejected(self, env):
        from plane.workflow.guards import guard_config_issues

        details = guard_config_issues([{"type": "required_fields", "config": {"fields": ["cf_nope"]}}], env["proj"])
