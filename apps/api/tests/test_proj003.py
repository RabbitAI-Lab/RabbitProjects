...
@pytest.fixture()
def env(db):
    owner = User.objects.create_user(email="p3-owner@rabbit.dev", password="Rabbit123!", display_name="张三")
    member = User.objects.create_user(email="p3-member@rabbit.dev", password="Rabbit123!", display_name="李成员")
    ws = Workspace.objects.create(name="W", slug=f"w-p3-{owner.id.hex[:8]}", owner=owner, created_by=owner)
...
def test_transition_edges_and_idempotent(env):
    ...
    assert (
        _client(env["member"])
        .post(_t_url(env, "transitions/"), format="json", data={"to_status": "archived"})
        .status_code
        == 403
    )
...
def test_close_guard_and_force(env):
    ...
    r2 = c.post(
        _t_url(env, "transitions/"), format="json", data={"to_status": "closed", "force": True, "reason": "收尾"}
    )
...
def test_closed_write_protection(env):
    ...
    c = _client(env["owner"])
    c.post(_t_url(env, "transitions/"), format="json", data={"to_status": "closed", "force": True})
    r = c.post(
        f"/api/v1/workspaces/{env['ws'].slug}/projects/{env['proj'].id}/issues/{issue.id}/comments/",
        format="json",
        data={"comment_html": "<p>hi</p>", "comment_json": {}},
    )
...
def test_lifecycle_event_emitted(env, _capture_lifecycle):
    ...
    c.post(_t_url(env, "transitions/"), format="json", data={"to_status": "archived"})
    assert any(
        k.get("field") == "status" and k.get("new_value") == "archived" and k.get("comment") == "milestone"
        for k in sent
    )
...
def test_draft_activate_guards(env):
    draft = Project.objects.create(
        name="D", identifier="D03", status="draft", workspace=env["ws"], created_by=env["owner"]
    )
    ProjectMember.objects.create(project=draft, member=env["owner"], role=ProjectRole.ADMIN, created_by=env["owner"])
...
def test_templates_crud(env):
    ...
    assert c.post(base, format="json", data={"name": "坏", "states_snapshot": [{"no_group": 1}]}).status_code == 400
    # 内置保护（BR-10）
    assert c.patch(f"{base}{builtin['id']}/", format="json", data={"name": "X"}).status_code == 403
    assert c.delete(f"{base}{builtin['id']}/").status_code == 403
    assert c.patch(f"{base}{tid}/", format="json", data={"description": "n"}).status_code == 200
    assert c.delete(f"{base}{tid}/").status_code == 204
    # 跨空间 404：另一空间的模板 id
    other = User.objects.create_user(email="p3-o2@rabbit.dev", password="Rabbit123!", display_name="外")
    ws2 = Workspace.objects.create(name="W2", slug=f"w2-p3-{other.id.hex[:8]}", owner=other, created_by=other)
...
def test_create_with_initial_status_and_template(env):
    tpl = __import__("plane.db.models", fromlist=["ProjectTemplate"]).ProjectTemplate.objects.create(
        ...
        name="建站模板",
        states_snapshot=[
            {"name": "待办", "group": "unstarted", "is_default": True},
    ...
    c = _client(env["owner"])
    r = c.post(
        f"/api/v1/workspaces/{env['ws'].slug}/projects/",
        format="json",
        data={"name": "新项目", "identifier": "NP1", "initial_status": "draft", "template_id": str(tpl.id)},
    )
    assert r.status_code == 201
    pid = r.json()["data"]["id"]
    ...
    r2 = c.post(
        f"/api/v1/workspaces/{env['ws'].slug}/projects/",
        format="json",
        data={"name": "X", "identifier": "NP2", "initial_status": "closed"},
    )
...
def test_duplicate_closed(env):
    ...
    assert ProjectMember.objects.filter(project=copy, member=env["owner"], role=ProjectRole.ADMIN).exists()
