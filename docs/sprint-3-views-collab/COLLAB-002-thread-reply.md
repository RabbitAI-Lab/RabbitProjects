...
4. **`reply_to_actor` 来源**（勘误 2026-09-06（ADR-0020）：ADR-0019 决策 1 的规格侧落地）：归并挂载后回复目标无法从 `parent_id` 反推——POST 发表回复时在 `accessory.reply_to={comment_id, actor_id}` 存档，GET 列表读档回显（`accessory` 为规格化 `images` 键之外的扩展键）；存量无存档行回退为顶层作者。
5. **父删子留占位条件收窄**（勘误 2026-09-06（ADR-0020）：ADR-0019 决策 2，收窄 BR-06 字面）：软删顶层**仅当仍有存活回复**时以 `is_deleted: true` 占位出现（引导线延续、replies 保留）；无存活回复的软删评论维持 `COLLAB-001` 消失语义。占位行的正文 / 图片 / mention 不再回传（软删内容不可再见的既有语义延续）。
