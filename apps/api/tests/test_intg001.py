...
@pytest.fixture()
def env(db):
    owner = User.objects.create_user(email="i1-owner@rabbit.dev", password="Rabbit123!",
                                     display_name="张三")
    admin = User.objects.create_user(email="i1-admin@rabbit.dev", password="Rabbit123!",
                                     display_name="李管理")
    ws = Workspace.objects.create(name="W", slug=f"w-i1-{owner.id.hex[:8]}",
    ...
    binding = IntegrationInstallation.objects.create(
        installation_id=9001, project=proj,
        repository_full_name="acme/rabbit-web", repository_node_id="R_1",
        webhook_secret=encrypt_secret(secret), created_by=owner)
    return {"owner": owner, "admin": admin, "ws": ws, "proj": proj,
...
def _signed_post(env, payload: dict, *, event="issues", delivery="d-1", secret=None):
    ...
    sig = "sha256=" + hmac.new((secret or env["secret"]).encode(), body,
                               hashlib.sha256).hexdigest()
    return APIClient().post(
        "/api/v1/integrations/github/webhook/", data=body, content_type="application/json",
        headers={"X-GitHub-Event": event, "X-GitHub-Delivery": delivery,
                 "X-Hub-Signature-256": sig})


def _issue_payload(env, *, action="opened", node="N_1", number=7, title="从 GitHub 来的",
                   updated=None):
    ...
    return {
        ...
        "issue": {"node_id": node, "number": number, "title": title,
                  "body": "正文", "state": "open",
...
def test_binding_crud(env):
    url = (f"/api/v1/workspaces/{env['ws'].slug}/projects/{env['proj'].id}/"
           f"integrations/github/bindings/")
    r = _client(env["admin"]).post(url, format="json", data={
        "repository_full_name": "acme/rabbit-api", "installation_id": 9002})
    ...
    # 重复绑定 409
    assert _client(env["admin"]).post(url, format="json", data={
        "repository_full_name": "acme/rabbit-web"}).status_code == 409
    # 上限 5（已 2，再造 4 → 第 4 个 409）
    for i in range(3):
        assert _client(env["admin"]).post(url, format="json", data={
            "repository_full_name": f"acme/r{i}"}).status_code == 201
    assert _client(env["admin"]).post(url, format="json", data={
        "repository_full_name": "acme/r9"}).status_code == 409
    # 暂停 / 恢复
    bid = row.id
    r2 = _client(env["admin"]).patch(f"{url}{bid}/", format="json",
                                     data={"sync_status": "paused"})
    assert r2.status_code == 200 and r2.json()["data"]["sync_status"] == "paused"
    # 权限：非项目成员 403
    outsider = User.objects.create_user(email="i1-out@rabbit.dev", password="Rabbit123!",
                                        display_name="外")
    WorkspaceMember.objects.create(workspace=env["ws"], member=outsider,
                                   role=WorkspaceRole.MEMBER, created_by=env["owner"])
    assert _client(outsider).post(url, format="json", data={
        "repository_full_name": "acme/x"}).status_code == 403
    # 解绑：external 保留（BR-13）
    issue = Issue.objects.create(
        name="t", project=env["proj"],
        state=State.objects.get(project=env["proj"], group="unstarted"),
        priority="none", sequence_id=1, sort_order=100, created_by=env["owner"])
...
def test_inbound_gates(env, monkeypatch):
    ...
    monkeypatch.setattr(gs.dispatch_github_event, "delay",
                        lambda ev: sent.append(ev))
    # 未绑定仓库 → 202 丢弃
    ...
    bad = _signed_post(env, _issue_payload(env), delivery="d-bad",
                       secret="wrong-secret")
...
def test_worker_issue_opened(env, monkeypatch):
    ...
    monkeypatch.setattr(
        gh.GitHubClient, "edit_issue_title",
        lambda self, repo, num, title, token_cache=None: writeback.append((repo, num, title)))
    result = gs._on_issue_opened({
        "kind": "issue.opened", "installation_id": 9001,
        "repository_full_name": "acme/rabbit-web", "delivery_id": "d-1",
...
def test_worker_edited_lww_and_conflict(env):
    ...
    gs._on_issue_opened({"kind": "issue.opened", "installation_id": 9001,
                         "repository_full_name": "acme/rabbit-web",
                         "issue": _issue_payload(env)["issue"]})
    issue = Issue.objects.get(external_id="N_1")
    # 入站较新（updated 2026-09-08）→ 覆盖
    gs._on_issue_edited({"kind": "issue.edited", "installation_id": 9001,
                         "repository_full_name": "acme/rabbit-web",
                         "issue": _issue_payload(env, action="edited",
                                                 title="改后标题",
                                                 updated="2030-01-01T00:00:00Z")["issue"]})
    ...
    gs._on_issue_edited({"kind": "issue.edited", "installation_id": 9001,
                         ...
                         "issue": _issue_payload(env, action="edited",
                                                 title="更晚的 GitHub 改动",
                                                 updated="2026-09-20T00:00:00Z")["issue"]})
    issue.refresh_from_db()
...
def test_worker_close_via_guard(env, monkeypatch):
    ...
    r = gs._on_issue_closed({"kind": "issue.closed", "installation_id": 9001,
                             "repository_full_name": "acme/rabbit-web",
                             "issue": _issue_payload(env)["issue"]})
    ...
    gs._on_issue_opened({"kind": "issue.opened", "installation_id": 9001,
                         "repository_full_name": "acme/rabbit-web",
                         "issue": _issue_payload(env, node="N_2", number=8,
                                                 title="第二个")["issue"]})
    issue2 = Issue.objects.get(external_id="N_2")
    IssueLink.objects.create(
        issue=issue2, related_issue=issue, relation_type="is_blocked_by",
        created_by=env["owner"])  # issue2 被未完成的 issue 阻塞
    issue.state = State.objects.get(project=env["proj"], group="unstarted")
    issue.save(update_fields=["state"])   # 前置回未完成 → 构成阻塞
    r2 = gs._on_issue_closed({"kind": "issue.closed", "installation_id": 9001,
                              "repository_full_name": "acme/rabbit-web",
...
def test_worker_pr_merge_and_push(env):
    ...
    issue = Issue.objects.get(external_id="N_1")
    pr_event = {
        "kind": "pull_request.merged", "installation_id": 9001,
        "repository_full_name": "acme/rabbit-web",
        "pull_request": {"number": 42, "merged": True, "title": "fix: RBT-1 修复",
                         "body": "Fixes [RBT-1]", "html_url": "https://github.com/p/1",
    ...
    # push 挂载 + 去重
    push = {
        "kind": "push", "installation_id": 9001,
        "repository_full_name": "acme/rabbit-web",
        "commits": [{"id": "a" * 40, "message": "chore: RBT-1 联调",
                      "url": "u", "author": {"name": "z"}}],
    }
