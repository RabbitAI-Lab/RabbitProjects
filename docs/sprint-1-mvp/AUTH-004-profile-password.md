...
sequenceDiagram
    ...
    alt 超限
        ...
        W->>W: 提示成功 → 跳转成功页「密码已重置」＋「去登录」手动登录（不自动登录；该用户已无任何会话）
    end
```
...
提交成功后**不自动登录**：跳转成功页展示「密码已重置」+ 主按钮「去登录」，由用户手动登录（与 §3.6 交互表口径一致——重置已吊销该用户全部会话，无自动登录的会话基础）。
...
| < 768px | 单栏：导航折叠为顶部横向 Tab（个人资料 / 安全 / 通知偏好，与 §3.1 / §3.2 三项导航一致）；头像居中；表单全宽；发送 / 保存按钮吸底 |
