...
flowchart TB
    ...
    T005["TASK-005<br/>流转拦截+依赖"] --> W001
    T010["TASK-010<br/>操作日志/Activity"] --> W001
    T005 --> W004["WF-004<br/>流转守卫"]
    W001 --> W004
    ...
    W001 --> W005
    W001 --> W006["WF-006<br/>审批留痕"]
    W002 --> W006
    T010 --> W006
    T008["TASK-008<br/>自定义字段基座"] --> T012["TASK-012<br/>高级字段"]
    T011["TASK-011<br/>组合筛选器/视图"] --> T012
    T006 --> T013["TASK-013<br/>团队工时"]
    R002["RPT-002<br/>报表聚合口径"] --> T013
```

> 注一（模块码与编号映射）：模块码以 [`docs/architecture/dependency-graph.md`](../architecture/dependency-graph.md) §1.2 为唯一事实——本迭代覆盖 **M11-WF（企业工作流与审批）** 与 **M4-TASK（任务核心）**。编号-标题映射以 [`docs/README.md`](../README.md) §4 为准（同 §1 注）；dg 曾存在的 `WF-002~006` 编号-标题整体错位一档（及 `AUTH-009/010` 互换、`TASK-009/010/011` 错位）**已由 2026-09-06「架构文档专项回改批次」（两期）统一修复**——§1.3 台账、§4 依赖行与 §4.15 mermaid 现与 README §4 一致，本文依赖边直接引 dg 现行口径，不再需要标题换算（本注为历史登记消解注，2026-09-08 R0 回改收口）。

> 注二（依赖边换算口径）：上图依赖边 = `dependency-graph.md` §4.5（`TASK-012`/`TASK-013` 行）与 §4.6（M11-WF 六行）的现行记载边（2026-09-06 架构文档专项回改批次修复后的口径，2026-09-08 R0 同步收口）。原两处例外已消解：① `WF-006` 行 dg 已补（上游 `WF-001`＋`WF-002`＋`TASK-010`——留痕读取审批执行模型与状态机执行模型、留痕事件经 Activity 管道），本图与 dg 同口径画三条直边；`GANTT-002` 为 `WF-006` §0 meta 的导出范式弱引用（`BR-11` 文件名模板 + §2.6 文件名模板范式），不构成结构依赖，不入依赖图；② dg 原 `TASK-012 ← AUTH-008` 跨迭代倒挂边已在回改批次中**撤销**——按 `TASK-012` 口径处理为「字段权限先落固定角色码、自定义角色矩阵在 Sprint 8 `AUTH-008` 落地后自动生效，零改表零回改」，本图不画该边。**直接 / 传递 / 弱引用三层分级**——图上主路径为「直接结构依赖」（dg 实际记载边）；弱引用 ⊝ 仅在 §3 文本表格中提及（如 `GANTT-002` 的文件名模板范式借鉴），不构成依赖边、不入图。图上未画主路径即视为非结构依赖。**软依赖不入图口径**：各子文档 §0 meta 的上游依赖系统性多于本图（如 WF-005←WF-002/WF-004、TASK-012←FILE-001、TASK-013←TASK-010/COLLAB-001）——此类为管道/范式复用的**软依赖**，不重复入图；结构依赖以 dg 现行边（上图）为准，两者不冲突。
...
- `TASK-005` 流转路径守卫（BLOCKER 钩子）与 `TASK-010` Activity/操作日志管道稳定——工作流在冻结状态模型基线上扩表（引擎替换固定状态流转），完成守卫复用依赖钩子、流转留痕经 Activity 管道（dg §4.6 `WF-001` 行；类型挂载底座为下条 `State.issue_type` 预留列）。
...
| 6 | **Sprint 8 依赖回改**：`AUTH-008` 自定义角色依赖 WF 节点权限模型（dg §2.2 S7→S8 硬阻塞、§4.2 `AUTH-008 ← WF-003`——该编号按本文口径即流转守卫）；引擎协议字段（`guards`/`side_effects`/`approval`）「协议先冻结」若失守，Sprint 8 与 dg 依赖边需连锁回改 | 中：Sprint 8 排期受阻，架构文档漂移面扩大 | 中 | ① 三协议字段在 `WF-001` 冻结评审后本迭代内只填充功能不扩协议；② 本文 §3 注一/注二登记的 dg 错位（WF-002~006 编号-标题、`WF-006` 缺行、`TASK-012 ← AUTH-008` 倒挂）按纪律先登记「架构文档待回改」再继续，未登记视为未收口（§10 第 3 项）——**该三项目前已由 2026-09-06「架构文档专项回改批次」完成回改，Sprint-7 R0（2026-09-08）核验消解**；③ 收口时向 Sprint 8 交接「节点权限模型 + 协议字段冻结清单」 | `WF-001` §1 / §4、本文档 §3 / §10、`docs/adr/` |
...
1. **功能验收**：§6 验收清单八条逐项绿——8 份规格均含七章结构与全部详尽要素、标准版升级后零行为变化（回归套件全绿）、画布「研发需求流程」**自建演示图**编辑并生效于需求类型（WF-001 独立演示图，非 WF-005 预设实例化——与 §6 第三条同口径）、五审批场景全链路可演示且留痕完整可审计、自动化规则可配置可 Dry Run 可回溯日志、字段级权限四处不可见、工时审批全链路、全部新端点通过 `api-conventions.md` §14 检查清单。
...
3. **文档同步（含架构偏离 ADR 登记回改）**：本迭代 8 份功能文档状态全部标为「已实现」；实现过程中的架构偏离——`dependency-graph.md` §1.3/§4.6 的 WF-002~006 编号-标题错位与 `WF-006` 缺行、`TASK-012 ← AUTH-008` 倒挂边（**此三项已由 2026-09-06 架构文档专项回改批次完成，R0 核验消解**）、权限 Key / 错误码 / 三协议字段（guards/side_effects/approval）的任何增删——必须先登记 `docs/adr/` 或在架构文档内联标注「架构文档待回改」并完成回改，未登记视为未收口；产出「协议字段冻结清单」与「已知技术债」交接 Sprint 8（`AUTH-008` 消费节点权限模型）。
