...
| 范围 | 本迭代交付 | 明确不做 |
| --- | --- | --- |
| 多看板 / 视图 | `IssueView` 表、视图 CRUD、视图切换器、显示配置（卡片字段 / 空组显隐） | 视图团队共享 / 管理员锁定（P3 `BOARD-005`）；跨项目全局视图（P4，09-06 批次口径对齐 BOARD-003 §4.1.1） |
...
| 实时同步 | live WebSocket：拖卡 / 状态 / 评论 / 指派 / 新建删除推送；在线成员感知 | 富文本协同编辑（Yjs CRDT 协同与协作光标归 **P3**——届时 Hocuspocus 挂入 `apps/live` 同进程、复用 `COLLAB-004` 预留的票据与房间模型；Sprint 4 仅交付 `FILE-002/003/004` 与甘特，不含 Yjs 运行时；`FILE-005` Wiki（Sprint 9 企业版）仅预留 `collab_doc_id`，其 Yjs 实时协同为 P4 评估；本迭代 live 仅做业务事件广播，不实现 Yjs）；消息已读回执（P3） |
...
- Sprint 0-2 验收全部通过；`KanbanGroupView` 分组契约（State 裸 UUID 键（`BOARD-002` BR-02 / `BOARD-003` §2 冻结契约）/ 空列恒在 / 组内 25 条）稳定。
- `field-schema/` 端点（ETag 协商）与 `filterable/sortable/groupable` 推导已交付（`TASK-008`）。
- `issue_activity` 管道（幂等 / epoch）已全事件覆盖（`TASK-010`）。
- live 服务容器在 Compose 内可起，Nginx upgrade 路由可通（`INFRA-002` §4.4）；协同票据的 RS256 密钥对配置项就绪（`INFRA-004`）——票据签发 / 验签链路本身是 `COLLAB-004`（Day 4）交付物，不作为进入条件。
...
| 性能 | 分组看板首屏 SQL ≤ 10 条；动态流首页与过滤查询 P95 < 150ms（10 万任务 / 100 万 Activity 数据集，`COLLAB-003` BR-14 基准）；WS 推送端到端 < 1s（`COLLAB-004` IT-01 同口径） |

## 6. 验收标准

- [x] 6 份功能规格均含完整的 7 章结构与逐端点 JSON 契约。
- [x] 视图：保存「筛选 + 分组 + 显示列」组合后刷新还原；四布局切换条件不丢；视图可分享（P2 个人视图，URL 直达）。
- [x] 看板按优先级 / 负责人 / 自定义字段 select 分组正确，空组列恒在，分组列与 `options` 配置一致。
- [x] 批量：100 条上限与超限 `400 VALIDATION_BULK_LIMIT_EXCEEDED`；单事务回滚验证（中途失败零残留）；动态流聚合为「批量更新了 N 个任务」。
- [x] 评论：楼中楼两级、表情切换、图片评论上传展示全链路可用；通知去重正确。
- [x] 活动流：项目动态流按时间聚合渲染；任务时间线与 `TASK-010` 契约一致（epoch 组 / 过滤 / 分页）。
- [x] 实时：双浏览器拖卡 / 评论 / 改状态 < 1s 可见；离线重连补偿拉取；票据过期自动降级提示。
- [x] 筛选器：嵌套 AND/OR 树保存与还源；占位符 `@me` / `today` 跨用户跨日期「活」；10 万任务 5 条混合条件 P95 < 200ms。
- [x] 任何越权访问（他人项目视图 / 未共享动态 / 房间）均 404 / 拒入。
...
| Day 5 | 联调、压测（WS 并发 50 连接；概览级集成压测口径——验收期 k6 脚本执行，用例在验收仓库非功能文档，不在本迭代 6 份规格内） | 回归、无障碍、文档评审 | Sprint 验收清单全部通过 |
...
| 7 | **范围蔓延**：Yjs 协同编辑 / 已读回执 / 视图团队共享等「顺手加一下」 | 高：5 天排期与 9 条验收项互相挤压 | 高 | §2「明确不做」列为硬性范围基线：Yjs 归 P3（Hocuspocus 挂入 `apps/live` 同进程，`COLLAB-004` 预留票据与房间；Sprint 4 不含 Yjs 运行时，`FILE-005` Wiki 的实时协同为 P4 评估）、共享视图归 P3 `BOARD-005`、回执归 P3+；新增能力一律转入后续迭代排期，不占本迭代 20% 缓冲 | 本文档 |
...
2. **工程质量**：本迭代无未修复的 P0 / P1 级缺陷；WS 并发 50 连接压测（概览级集成压测口径：验收期 k6 脚本执行，用例在验收仓库非功能文档）、票据续签与降级、批量行锁冲突（nowait `409`）、DSL 深度 / 节点边界拒绝等异常路径测试全部通过；20% 缓冲未被功能蔓延占用。
...
1. **功能验收**：§6 九条验收标准逐条核验通过——六段业务流 `tests/jmeter/sprint-3-flow.py` 全绿（214 断言，覆盖视图保存还原 / 多维分组 / 批量事务与超限 400 / 楼中楼表情图片 / 动态流聚合 / 双端实时同步与重连补偿 / 嵌套筛选与占位符）；e2e 123 通过 / 2 失败（均为存量用例，非本迭代引入）/ 1 跳过；越权路径由 pytest 越权矩阵覆盖。
2. **工程质量**：pytest 350 通过；性能四门禁 `tests/jmeter/sprint-3-bench.py` 全绿（含 G1 分组看板 SQL 计数、动态流 / 筛选器 P95、WS 推送口径）；无未修复 P0/P1；实现期偏差已登记 ADR-0017（上游服务 epoch 风暴）、ADR-0018（用户设置存储）、ADR-0019（COLLAB-002 偏差）、ADR-0020（Sprint-3 偏差汇总，含门禁口径与规格勘误）。
3. **文档同步**：六份功能规格 + 本概览已标记「已实现（Implemented，2026-09-06）」；规格勘误已按 ADR-0020 回写（BOARD-003 §3.5/§4.4、TASK-011 §3.2、COLLAB-002 §4.2.2、COLLAB-004 §3.1/BR-11/§2.5）；`CLAUDE.md` 测试命令 / 已知坑 / 文档体系已同步。
