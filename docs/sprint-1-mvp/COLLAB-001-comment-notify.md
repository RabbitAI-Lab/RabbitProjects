...
### 3.1 评论 Tab（任务详情抽屉『评论』Tab；Tab 条结构见 TASK-002 §3.6：描述｜评论｜动态｜附件）
...
> 倒计时起点为「剩余 15:00」：编辑窗口为发表后 **15 分钟**（§2.2 / BR-05），进入编辑态即显示当前剩余时间（如发表后 10 分 28 秒进入则显示「剩余 04:32」），最后 30s 变橙（§3.4）。
...
| 元素 | 规格 |
| --- | --- |
| 铃铛 | 全局顶栏常驻（工作台与项目内顶栏一致）；徽标 `bg-red-500` 圆形，数字 99+ 封顶；`aria-live="polite"` 播报变化 |
| 抽屉 | 右侧滑入 420px；`role="dialog"`；打开时自动拉取列表（不全量预取） |
| 分组 | 今天 / 昨天 / 更早（`date-fns` `isToday/isYesterday`） |
| 「更早 (N)」分组 | 分组底部「加载更多」按钮；cursor 分页追加（与任务列表口径一致） |
| 事件图标 | `issue.assigned` 👤 / `mentioned` @ / `commented` 💬 / `updated` ✏️（蓝点同时存在，图标非唯一信号） |
| 「仅看未读」 | 抽屉内本地开关状态（抽屉无路由，不进浏览器 URL）；开启后列表请求带 `?unread=true` 参数（与 `RPT-001` 通知摘要端点参数一致） |
