...
@pytest.fixture()
def env(db):
    owner = User.objects.create_user(email="a6-owner@rabbit.dev", password="Rabbit123!", display_name="张三")
    admin = User.objects.create_user(email="a6-admin@rabbit.dev", password="Rabbit123!", display_name="李管理")
    member = User.objects.create_user(email="a6-member@rabbit.dev", password="Rabbit123!", display_name="王成员")
    wsonly = User.objects.create_user(email="a6-wsonly@rabbit.dev", password="Rabbit123!", display_name="赵仅空间")
    outsider = User.objects.create_user(email="a6-out@rabbit.dev", password="Rabbit123!", display_name="钱外人")
    ws = Workspace.objects.create(name="W", slug=f"w-a6-{owner.id.hex[:8]}", owner=owner, created_by=owner)
    for u, r in (
        (owner, WorkspaceRole.OWNER),
        (admin, WorkspaceRole.ADMIN),
        (member, WorkspaceRole.MEMBER),
        (wsonly, WorkspaceRole.MEMBER),
    ):
    ...
    issue = Issue.objects.create(
        name="任务甲", project=proj, state=todo, priority="none", sequence_id=1, sort_order=100, created_by=owner
    )
    return {
        "owner": owner,
        "admin": admin,
...
def test_matrix_visibility_branches(env):
    ...
    draft = Project.objects.create(
        name="draft-p", identifier="D06", workspace=env["ws"], status="draft", created_by=env["owner"]
    )
    # draft 项目即便是显式成员也不可见（BR-11）——突变咬合点
    ProjectMember.objects.create(
        project=draft, member=env["member"], role=ProjectRole.CONTRIBUTOR, created_by=env["owner"]
    )
    pub = Project.objects.create(
        name="pub-p", identifier="U06", workspace=env["ws"], created_by=env["member"], visibility="public"
    )
...
def test_bulk_role_partial_semantics(env):
    ...
    admin2 = User.objects.create_user(email="a6-admin2@rabbit.dev", password="Rabbit123!", display_name="孙管理")
    WorkspaceMember.objects.create(
        workspace=env["ws"], member=admin2, role=WorkspaceRole.ADMIN, created_by=env["owner"]
    )
    WorkspaceMember.objects.filter(workspace=env["ws"], member=env["wsonly"]).update(role=WorkspaceRole.GUEST)
    ...
    res = _client(env["admin"]).post(
        ...
        data={
            "user_ids": [
                str(env["member"].id),  # MEMBER→GUEST：updated
...
def test_bulk_role_owner_target_skipped_not_counted(env):
    res = _client(env["admin"]).post(
        _bulk_url(env), format="json", data={"user_ids": [str(env["owner"].id)], "role": WorkspaceRole.MEMBER}
    )
    data = res.json()["data"]
...
def test_bulk_role_structural_400(env):
    c = _client(env["owner"])
    assert c.post(_bulk_url(env), format="json", data={"user_ids": [], "role": 10}).status_code == 400
    assert (
        c.post(_bulk_url(env), format="json", data={"user_ids": [str(env["member"].id)], "role": 20}).status_code == 400
    )
...
def test_bulk_role_permission_403(env):
    res = _client(env["member"]).post(
        _bulk_url(env), format="json", data={"user_ids": [str(env["wsonly"].id)], "role": WorkspaceRole.GUEST}
    )
...
def test_bulk_role_cascade_br15(env):
    ...
    assert ProjectMember.objects.get(project=env["proj"], member=env["member"]).role == ProjectRole.CONTRIBUTOR
    res = _client(env["owner"]).post(
        _bulk_url(env), format="json", data={"user_ids": [str(env["member"].id)], "role": WorkspaceRole.GUEST}
    )
    assert res.status_code == 200 and res.json()["data"]["updated"] == 1
    assert ProjectMember.objects.get(project=env["proj"], member=env["member"]).role == ProjectRole.COMMENTER


def test_bulk_role_cascade_last_admin_protected(env):
    """末位 PROJ_ADMIN 保护优先于联动（rbac §7.2）。"""
    solo = User.objects.create_user(email="a6-solo@rabbit.dev", password="Rabbit123!", display_name="周唯一")
    WorkspaceMember.objects.create(workspace=env["ws"], member=solo, role=WorkspaceRole.MEMBER, created_by=env["owner"])
    p2 = Project.objects.create(name="P2", identifier="B06", workspace=env["ws"], created_by=env["owner"])
    ProjectMember.objects.create(project=p2, member=solo, role=ProjectRole.ADMIN, created_by=env["owner"])
    _client(env["owner"]).post(
        _bulk_url(env), format="json", data={"user_ids": [str(solo.id)], "role": WorkspaceRole.GUEST}
    )
...
def test_project_bulk_role(env):
    ...
    res = _client(env["owner"]).post(
        url, format="json", data={"member_ids": [str(env["member"].id)], "role": ProjectRole.VIEWER}
    )
    assert res.status_code == 200 and res.json()["data"]["updated"] == 1
    assert ProjectMember.objects.get(project=env["proj"], member=env["member"]).role == ProjectRole.VIEWER
    # 末位 ADMIN 保护
    res = _client(env["owner"]).post(
        url, format="json", data={"member_ids": [str(env["admin"].id)], "role": ProjectRole.VIEWER}
    )
...
def test_disable_self_400(env):
    ...
    res = _client(env["admin"]).post(f"/api/v1/workspaces/{env['ws'].slug}/members/{row.id}/disable/", format="json")
    assert res.status_code == 400
...
def test_disable_revokes_session_and_401(env):
    ...
    victim = User.objects.create_user(email="a6-victim@rabbit.dev", password="Rabbit123!", display_name="吴被禁")
    WorkspaceMember.objects.create(
        workspace=env["ws"], member=victim, role=WorkspaceRole.MEMBER, created_by=env["owner"]
    )
    ...
    res = _client(env["owner"]).post(f"/api/v1/workspaces/{env['ws'].slug}/members/{row.id}/disable/", format="json")
    assert res.status_code == 200
    data = res.json()["data"]
    ...
    # 幂等重放：revoked 全 0
    res2 = _client(env["owner"]).post(f"/api/v1/workspaces/{env['ws'].slug}/members/{row.id}/disable/", format="json")
    assert res2.status_code == 200 and res2.json()["data"]["revoked"]["sessions"] == 0


def test_disable_owner_forbidden_even_multi(env):
    second = User.objects.create_user(email="a6-own2@rabbit.dev", password="Rabbit123!", display_name="郑二主")
    WorkspaceMember.objects.create(
        workspace=env["ws"], member=second, role=WorkspaceRole.OWNER, created_by=env["owner"]
    )
...
def test_enable_restores(env):
    victim = User.objects.create_user(email="a6-v2@rabbit.dev", password="Rabbit123!", display_name="冯复用")
    WorkspaceMember.objects.create(
        workspace=env["ws"], member=victim, role=WorkspaceRole.MEMBER, created_by=env["owner"]
    )
    row = WorkspaceMember.objects.get(workspace=env["ws"], member=victim)
    _client(env["owner"]).post(f"/api/v1/workspaces/{env['ws'].slug}/members/{row.id}/disable/", format="json")
    res = _client(env["owner"]).post(f"/api/v1/workspaces/{env['ws'].slug}/members/{row.id}/enable/", format="json")
...
def test_lint_access_scanner():
    ...
    st = subprocess.run(
        [sys.executable, f"{repo}/scripts/lint_access.py", "--selftest"], capture_output=True, text=True
    )
    assert st.returncode == 0, st.stdout + st.stderr
    st = subprocess.run([sys.executable, f"{repo}/scripts/lint_access.py"], capture_output=True, text=True)
    assert st.returncode == 0, st.stdout + st.stderr
...
def matrix_env(env, db):
    ...
    sysadmin = User.objects.create_user(email="a6-sys@rabbit.dev", password="Rabbit123!", display_name="系统管")
    SystemAdmin.objects.create(user=sysadmin)
    WorkspaceMember.objects.create(
        workspace=env["ws"], member=sysadmin, role=WorkspaceRole.MEMBER, created_by=env["owner"]
    )
    ...
    if issue_type is None:  # a6 env 不走 signup 种子——按需补一条任务类型
        issue_type = IssueType.objects.create(workspace=env["ws"], name="任务", created_by=env["owner"])
    return {
        **env,
        "sysadmin": sysadmin,
        "guest": env["outsider"],
...
def _hit(client, resource, action, e):
    ...
    if resource == "workspace":
        if action == "create":
            return client.post(
                "/api/v1/workspaces/",
                {"name": f"W{u.uuid4().hex[:6]}", "identifier": f"W{u.uuid4().hex[:4].upper()}"},
                format="json",
            )
        if action == "read":
            return client.get(f"/api/v1/workspaces/{ws}/")
        if action == "update":
            return client.patch(f"/api/v1/workspaces/{ws}/", {"name": e["ws"].name}, format="json")
        return client.delete(f"/api/v1/workspaces/{ws}/")
    if resource == "project":
        base = f"/api/v1/workspaces/{ws}/projects/"
        if action == "create":
            return client.post(
                base, {"name": f"P{u.uuid4().hex[:6]}", "identifier": f"X{u.uuid4().hex[:4].upper()}"}, format="json"
            )
        if action == "read":
    ...
    if resource == "issue":
        ...
        if action == "create":
            return client.post(
                base,
                {"name": f"I{u.uuid4().hex[:6]}", "priority": "none", "type_id": str(e["issue_type"].id)},
                format="json",
            )
        if action == "read":
        ...
        if action == "update":
            return client.patch(
                f"{base}{iid}/", {"name": e["issue"].name, "type_id": str(e["issue_type"].id)}, format="json"
            )
    ...
    if action == "create":
        return client.post(
            base, {"url": f"https://h.example.com/{u.uuid4().hex[:6]}", "events": ["issue.updated"]}, format="json"
        )
    if action == "read":
...
def test_it_sec_matrix_64(subject, resource, action, matrix_env):
    ...
    assert ok, (
        f"IT-SEC[{subject}/{resource}/{action}] want={want} got={r.status_code} body={getattr(r, 'data', '')!s:.500}"
    )
