...
FILTER_KEYS = {"actor", "category", "action", "object_type", "ip", "search", "created_at"}
ORDERING_WHITELIST = {"created_at", "-created_at", "ip", "-ip", "category", "-category"}
...
def _filtered_queryset(request, ws_id):
    ...
    if unknown:
        raise AppException(
            ...
            details=[{"field": f, "code": "INVALID", "message": "不在筛选白名单"} for f in sorted(unknown)],
        )
    ...
    for field in ordering.split(","):
        if field and field.strip() not in ORDERING_WHITELIST:
            raise AppException(
                "VALIDATION_ERROR",
                message="非法排序字段",
                details=[{"field": "ordering", "code": "INVALID_PARAM", "message": f"{field} 不在排序白名单"}],
            )
    return qs.order_by(*[f.strip() for f in ordering.split(",") if f.strip()], "-id")
...
class AuditCatalogView(APIView):
    ...
    def get(self, request, slug):
        ...
        data = [
            {"category": c, "actions": [{"action": a, "label": label} for a, label in actions.items()]}
...
class AuditExportView(APIView):
    ...
    def post(self, request, slug):
        ...
        ws, _ = get_workspace_or_404(slug, request.user)
        # AUTH-012 §4.5 第 5 强制点：R-03 当日导出行数越界硬拒（409；
        # BR-11 门控与未治理租户跳过内置——R1 挂创建路径，预签名换发复核
        # 随 FILE-002/RPT-005 出口同批收口）
        from plane.governance.enforcement import check_export_deny

        check_export_deny(ws)
        if cache.get("audit_chain_broken"):  # §2.4 断链冻结导出
            raise AppException("RESOURCE_STATE_INVALID", message="审计链完整性异常，导出已冻结")
        # BR-04 双授权：二次确认密码
        from django.contrib.auth import authenticate

        password = str(request.data.get("password") or "")
        if authenticate(request, email=request.user.email, password=password) is None:
            raise AppException("AUTH_INVALID_CREDENTIALS", message="二次确认密码错误")
        qs = _filtered_queryset(request, ws.id)
        total = qs.count()
        if total > 500_000:  # §2.2 上限
            raise AppException("RESOURCE_LIMIT_EXCEEDED", message="导出上限 50 万行，请收窄条件")
        ...
        if total > AUDIT_SYNC_EXPORT_ROW_LIMIT:
            ...
            task = ExportTask.objects.create(
                ...
                params={
                    ...
                    "filters": {k: v for k, v in request.query_params.items() if k in FILTER_KEYS},
                    "total_hint": total,
                },
        ...
        conditions = {k: v for k, v in request.query_params.items() if k in FILTER_KEYS}
        ...
        record(
            event_key=_hashlib.sha256(
                f"audit.export:{ws.id}:{request.user.id}:{total}:{_uuid.uuid4()}".encode()
            ).hexdigest()[:80],
            category="audit",
            action="exported",
            workspace_id=ws.id,
        ...
        resp["Content-Disposition"] = f'attachment; filename="audit-log-{ws.slug}.csv"'
...
class InstanceAuditLogListView(APIView):
    ...
    def get(self, request):
        ...
        if not SystemAdmin.objects.filter(user=request.user, is_active=True).exists():
        ...
        record(
            event_key=_hashlib.sha256(f"audit.instance_query:{request.user.id}:{_uuid.uuid4()}".encode()).hexdigest()[
                :80
            ],
            category="audit",
            action="instance_query",
            workspace_id=None,
            actor=request.user,
            obj={"type": "instance", "id": "system"},
            detail={"filters": dict(request.query_params), "rows": meta["count"]},
        )
        from plane.base.response import success_response
