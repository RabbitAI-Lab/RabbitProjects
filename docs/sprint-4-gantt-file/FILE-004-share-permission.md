...
| 文档状态 | 待评审（Draft） |
| 最后更新日期 | 2026-09-01 |
| 上游依赖 | **`FILE-002`（可见性单入口 / 预签名通道）**、`FILE-003`（预览器与版本——分享跟随当前版本）、`INFRA-004`（Argon2id 与随机数工具）、`AUTH-005`（权限门控） |
| 下游消费 | `FILE-006`（P4 水印 / 禁转 / 合规留存——分享是其管控落点）、`INTG-004`（P4 外链生态） |
| 上游依据 | `docs/需求文档.md` §3.7（生成文件分享链接、链接密码、有效期设置）、§8.2 文件管理 P2 列 |
| 关联架构文档 | [`api-conventions.md`](../architecture/api-conventions.md)（§2.1 **公开 API 分组 `/api/v1/public/`——匿名访问的架构基座**、§8 错误码、§13.4 安全头）、[`rbac-permission-model.md`](../architecture/rbac-permission-model.md)（`file.share` 权限码） |
...
- **不可枚举链接**：`/s/{slug}`（22 位 base62，≈128bit 熵，不可猜测不可遍历）；
...
2. **资源不可枚举**：slug 22 位 base62；无效 slug、过期、密码错误的响应**不泄露文件元信息**（错误体不含文件名/项目名）。
3. **预签名短时效 + 每次签发**：匿名访问每次换取 5 分钟预签名（下载/预览分权限签发）；不缓存长链接、不暴露桶直址。
...
| 依赖 | 内容 | 阻塞原因 |
| --- | --- | --- |
| `FILE-002` | `can_view_file`、预签名通道 | 创建前置权限与底层取数 |
| `FILE-003` | 预览器（衍生物重生成复用） | 匿名预览渲染 |
| `apps/space` | 公开应用骨架 | 匿名访问宿主 |
| `INFRA-004` | Argon2id / secrets / 限流 | 密码与防爆破 |
...
| 网盘类 | 提取码 + 有效期 + 权限分离 | 范式采纳；密码任意长度对齐安全 |
...
sequenceDiagram
    ...
    V->>SPACE: GET …/content/?download=1（携 cookie）
    SPACE->>SPACE: 权限分派：view→预览调度；download→预签名
    SPACE->>S3: 按 token 声明签发 5 分钟预签名
    SPACE-->>V: 文件流（预览/下载）
    SPACE->>SPACE: FileShareAccess 落痕（IP/UA/动作）
```
...
| BR-02 | slug = `secrets.token_urlsafe(16)`（22 位 base62）；唯一冲突重生成（约束兜底） | Service | — |
...
| BR-09 | 访问留痕：unlock/预览/下载/密码错误均落 `FileShareAccess`（IP/UA/时间/成败）；**不留身份** | Service | — |
| BR-10 | 匿名响应脱敏：未解锁的 meta 仅 `{requires_password}`；错误与过期页不含文件名/项目名 | Serializer | — |
| BR-11 | 上限：单文件活动分享 10 条；单人全项目 100 条 | Service | `409 RESOURCE_LIMIT_EXCEEDED` |
...
| --- | --- | --- | --- |
| slug 无效 | 404 | `RESOURCE_NOT_FOUND` | 通用「链接不存在或已失效」 |
...
| 创建超上限 | 409 | `RESOURCE_LIMIT_EXCEEDED` | 「该文件已有 10 个有效分享」 |
...
def generate_share_slug() -> str:
    return secrets.token_urlsafe(16)[:22]          # base62 ≈128bit 熵（BR-02）
...
| 3 | `PATCH` | `…/share-links/{link_id}/extend/` | 延期（≤365 天） | `file.share`（创建者/ADMIN） | `200` |
...
| 6 | `POST` | `/api/v1/public/shares/{slug}/unlock/` | 密码校验 → 2h cookie | 匿名 | `200` |
| 7 | `GET` | `/api/v1/public/shares/{slug}/content/?download=1` | 预览调度 / 下载预签名 | 匿名 + cookie | `200/202` |
...
**成功 `200`**（Set-Cookie: `share_token`；HttpOnly；SameSite=Lax；2h）：
...
def _resolve_active_share(slug: str) -> FileShareLink:
    ...
    if link.expires_at and link.expires_at < timezone.now():
        ...
        raise Gone()
    asset = link.asset
    if (asset.deleted_at or asset.status != "active"
            or asset.project.status != "active"):
        _mark(link, "invalidated")
        raise Gone()
    return link
```
...
def unlock(slug: str, password: str, request) -> Response:
    ...
    resp.set_cookie("share_token", token, max_age=7200, httponly=True, samesite="Lax")
    return resp
```
...
def content(slug: str, request) -> Response:
    ...
    if request.GET.get("download") == "1":
        ...
        url = presign_get(link.asset.asset, expires=300)    # 5 分钟（§1.2 底线）
        FileShareAccess.objects.create(share=link, action="download", ip=…)
        return ok({"download_url": url, "expires_in": 300})
    FileShareAccess.objects.create(share=link, action="view", ip=…)
    return preview_dispatch(link.asset, anonymous=True)     # 复用 FILE-003 调度
```
...
| --- | --- | --- | --- | --- |
| UT-01 | slug 熵与唯一 | 批量生成 | 22 位 base62 无碰撞 | 正常 |
...
| UT-06 | 有效期硬校验 | 到期访问 | 410 + expired | 边界 |
...
| UT-12 | 上限 | 第 11 条 | 409 | 边界 |
| UT-13 | 延期上限 | 累计 >365 天 | 400 | 边界 |
...
| IT-04 | 三因一页 | 过期/吊销/源失效 | 匿名页 | 同一 410 文案 |
...
| 后端 | 内部四端点、space 三端点、读时四查、Argon2id + 限速、2h HMAC token、`sweep_expired_shares` beat |
