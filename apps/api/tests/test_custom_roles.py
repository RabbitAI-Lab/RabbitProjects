...
@pytest.fixture()
def env(db):
    owner = User.objects.create_user(email="cr-owner@rabbit.dev", password="Rabbit123!", display_name="空间主")
    viewer = User.objects.create_user(email="cr-viewer@rabbit.dev", password="Rabbit123!", display_name="查看者")
    guest = User.objects.create_user(email="cr-guest@rabbit.dev", password="Rabbit123!", display_name="访客")
    ws = Workspace.objects.create(name="C", slug=f"w-cr-{owner.id.hex[:8]}", owner=owner, created_by=owner)
    for u, r in ((owner, WorkspaceRole.OWNER), (viewer, WorkspaceRole.MEMBER), (guest, WorkspaceRole.GUEST)):
        WorkspaceMember.objects.create(workspace=ws, member=u, role=r, created_by=owner)
    proj = Project.objects.create(name="P", identifier="CRP", workspace=ws, created_by=owner)
    seed_project_states(proj)
    pm_owner = ProjectMember.objects.create(project=proj, member=owner, role=PR.ADMIN, created_by=owner)
    pm_viewer = ProjectMember.objects.create(project=proj, member=viewer, role=PR.VIEWER, created_by=owner)
    return {
        "owner": owner,
        "viewer": viewer,
        "guest": guest,
...
class TestCatalog:
    ...
    def test_catalog_excludes_manage_codes(self):
        assert not any(c.endswith(".manage") or c == "integration.config" for c in CUSTOMIZABLE_CATALOG)
    ...
    def test_guest_ceiling_violations(self):
        ...
        assert "issue.update" in guest_ceiling_violations(["issue.read", "issue.update"])
...
class TestRoleCrud:
    ...
    def test_br03_duplicate_name_case_insensitive(self, env):
        RoleService().create_role(actor=env["owner"], project=env["proj"], name="QA", permissions=["issue.read"])
        with pytest.raises(AppException) as ei:
            RoleService().create_role(actor=env["owner"], project=env["proj"], name="qa", permissions=["issue.read"])
    ...
    def test_br04_twenty_role_cap(self, env):
        for i in range(20):
            RoleService().create_role(actor=env["owner"], project=env["proj"], name=f"R{i}", permissions=["issue.read"])
        with pytest.raises(AppException) as ei:
            RoleService().create_role(actor=env["owner"], project=env["proj"], name="R20", permissions=["issue.read"])
        assert ei.value.error_code == "RESOURCE_LIMIT_EXCEEDED"

    def test_br14_template_adoption(self, env):
        role = RoleService().create_role(actor=env["owner"], project=env["proj"], name="", template_key="qa_engineer")
    ...
    def test_br05_delete_with_assignment_rejected(self, env):
        role = RoleService().create_role(
            actor=env["owner"], project=env["proj"], name="在挂", permissions=["issue.read"]
        )
        RoleService().assign(actor=env["owner"], role=role, target_user=env["viewer"])
        with pytest.raises(AppException) as ei:
            RoleService().delete_role(actor=env["owner"], role=role)
        assert ei.value.error_code == "RESOURCE_IN_USE"
        RoleService().revoke(actor=env["owner"], role=role, target_user=env["viewer"])
        RoleService().delete_role(actor=env["owner"], role=role)  # 卸除后可删
        assert not CustomRole.objects.filter(pk=role.pk, deleted_at__isnull=True).exists()
    ...
    def test_br10_delete_with_guard_reference_rejected(self, env):
        ...
        role = RoleService().create_role(
            actor=env["owner"], project=env["proj"], name="被引用", permissions=["issue.read"]
        )
        wf = Workflow.objects.create(project=env["proj"], name="W", created_by=env["owner"])
        states = {
            n: WorkflowState.objects.create(workflow=wf, state=s, is_initial=(n == "待办"))
            for n, s in [("待办", env["proj"].states.all()[0]), ("进行中", env["proj"].states.all()[1])]
        }
        WorkflowTransition.objects.create(
            workflow=wf,
            from_state=states["待办"],
            to_state=states["进行中"],
            name="开始",
    ...
    def test_update_role_codes_tightened(self, env):
        role = RoleService().create_role(
            actor=env["owner"], project=env["proj"], name="收紧", permissions=["issue.read", "issue.update"]
        )
        RoleService().update_role(actor=env["owner"], role=role, permissions=["issue.read"])
...
class TestAssignment:
    def test_br06_assign_idempotent(self, env):
        role = RoleService().create_role(
            actor=env["owner"], project=env["proj"], name="幂等", permissions=["issue.read"]
        )
        assert RoleService().assign(actor=env["owner"], role=role, target_user=env["viewer"]) is True
        assert RoleService().assign(actor=env["owner"], role=role, target_user=env["viewer"]) is False
        assert (
            ProjectRoleAssignment.objects.filter(
                project=env["proj"], user=env["viewer"], deleted_at__isnull=True
            ).count()
            == 1
        )
    ...
    def test_br16_guest_ceiling_attach(self, env):
        ok_role = RoleService().create_role(
            actor=env["owner"], project=env["proj"], name="只读干系人", permissions=["issue.read", "comment.read"]
        )
        over_role = RoleService().create_role(
            actor=env["owner"], project=env["proj"], name="测试工程师", permissions=["issue.read", "issue.update"]
        )
        # GUEST 项目内可见（VIEWER）——先加项目成员
        pm_guest = ProjectMember.objects.create(
            project=env["proj"], member=env["guest"], role=PR.VIEWER, created_by=env["owner"]
        )
        RoleService().assign(actor=env["owner"], role=ok_role, target_user=env["guest"])  # 天花板内成功
        with pytest.raises(AppException) as ei:
            RoleService().assign(actor=env["owner"], role=over_role, target_user=env["guest"])
    ...
    def test_br16_ws_downgrade_cascades(self, env):
        role = RoleService().create_role(
            actor=env["owner"], project=env["proj"], name="越界角色", permissions=["issue.read", "issue.update"]
        )
        RoleService().assign(actor=env["owner"], role=role, target_user=env["viewer"])
        ...
        member_admin.bulk_role_workspace(
            workspace=env["ws"], actor=env["owner"], user_ids=[str(env["viewer"].id)], role=WorkspaceRole.GUEST
        )
        assert not ProjectRoleAssignment.objects.filter(user=env["viewer"], deleted_at__isnull=True).exists()
...
class TestUnionPermission:
    ...
    def _invoke(self, env, user, code):
        ...
        request.user = user
        view = type("V", (), {"kwargs": {"slug": env["ws"].slug, "project_id": str(env["proj"].id)}})()
    ...
    def test_union_grant_promotes(self, env):
        ...
        role = RoleService().create_role(
            actor=env["owner"], project=env["proj"], name="提升", permissions=["issue.update"]
        )
        RoleService().assign(actor=env["owner"], role=role, target_user=env["viewer"])
    ...
    def test_union_only_adds_not_cross_project(self, env):
        ...
        role = RoleService().create_role(
            actor=env["owner"], project=env["proj"], name="隔离", permissions=["issue.update"]
        )
        RoleService().assign(actor=env["owner"], role=role, target_user=env["viewer"])
        proj2 = Project.objects.create(name="P2", identifier="CRQ", workspace=env["ws"], created_by=env["owner"])
        seed_project_states(proj2)
        ProjectMember.objects.create(project=proj2, member=env["viewer"], role=PR.VIEWER, created_by=env["owner"])
    ...
    def test_br12_update_takes_effect_immediately(self, env):
        role = RoleService().create_role(
            actor=env["owner"], project=env["proj"], name="即时", permissions=["issue.update"]
        )
        RoleService().assign(actor=env["owner"], role=role, target_user=env["viewer"])
...
class TestEndpoints:
    ...
    def test_effective_permissions_endpoint(self, env):
        role = RoleService().create_role(
            actor=env["owner"], project=env["proj"], name="面板", permissions=["issue.update"]
        )
        RoleService().assign(actor=env["owner"], role=role, target_user=env["viewer"])
        r = _client(env["viewer"]).get(_roles_url(env, "members-effective-permissions", [str(env["pm_viewer"].id)]))
    ...
    def test_role_crud_endpoints(self, env):
        c = _client(env["owner"])
        r = c.post(_roles_url(env), {"name": "端点角色", "permissions": ["issue.read"]}, format="json")
        assert r.status_code == 201, r.json()
        role_id = r.json()["data"]["role"]["id"]
        assert "Location" in r
        r2 = c.patch(
            _roles_url(env, "roles-detail", [role_id]), {"permissions": ["issue.read", "issue.update"]}, format="json"
        )
    ...
    def test_assign_via_endpoint(self, env):
        role = RoleService().create_role(actor=env["owner"], project=env["proj"], name="挂", permissions=["issue.read"])
        c = _client(env["owner"])
        r = c.post(
            _roles_url(env, "role-assignments-list-create", [str(env["pm_viewer"].id)]),
            {"role_id": str(role.id)},
            format="json",
        )
        assert r.status_code == 201
        r2 = c.get(_roles_url(env, "role-assignments-list-create", [str(env["pm_viewer"].id)]))
        assert r2.status_code == 200
        assert r2.json()["data"][0]["role_name"] == "挂"
        r3 = c.delete(_roles_url(env, "role-assignments-delete", [str(env["pm_viewer"].id), str(role.id)]))
        assert r3.status_code == 204

    def test_br17_name_resolution(self, env):
        RoleService().create_role(actor=env["owner"], project=env["proj"], name="唯一名", permissions=["issue.read"])
        c = _client(env["owner"])
    ...
    def test_bulk_assign_by_department(self, env):
        ...
        dept = dept_svc.create_department(actor=env["owner"], workspace=env["ws"], name="研发部")
        WorkspaceMember.objects.filter(workspace=env["ws"], member=env["viewer"]).update(department=dept)
        role = RoleService().create_role(
            actor=env["owner"], project=env["proj"], name="批量", permissions=["issue.read"]
        )
        r = _client(env["owner"]).post(
            _roles_url(env, "roles-bulk-assign", [str(role.id)]), {"department_id": str(dept.id)}, format="json"
        )
        assert r.status_code == 201, r.json()
        data = r.json()["data"]
        assert str(env["viewer"].id) in data["added"]
        assert ProjectRoleAssignment.objects.filter(user=env["viewer"], role=role, deleted_at__isnull=True).exists()

    def test_pm_matrix_role_manage(self, env):
        """PM：WS_MEMBER 建角色 → 403 PERM_WORKSPACE_ADMIN_REQUIRED。"""
        r = _client(env["viewer"]).post(_roles_url(env), {"name": "x", "permissions": ["issue.read"]}, format="json")
...
class TestPerformance:
    def test_cached_lookup_p99_under_1ms(self, env):
        ...
        role = RoleService().create_role(
            actor=env["owner"], project=env["proj"], name="性能", permissions=["issue.update"]
        )
        RoleService().assign(actor=env["owner"], role=role, target_user=env["viewer"])
        # 预热缓存
