...
def env(db):
    ...
    issue = Issue.objects.create(
        name="T", project=proj, sequence_id=1, sort_order=100, created_by=owner, estimate_minutes=480
    )
...
def test_window_and_minutes_validation(env):
    ...
    with pytest.raises(WorklogValidationError):  # 未来日期（UT-15）
        log_work(issue=issue, actor_id=owner.id, minutes=30, worked_on=_tz.localdate() + _td(days=1))
...
def test_edit_revalidates_window_on_new_value(env):
    ...
    with pytest.raises(WorklogValidationError):  # UT-16：对新值重校验
        update_worklog(log_id=log.id, issue_id=issue.id, actor_id=owner.id, is_admin=False, worked_on=_d(31))
    _, spent = update_worklog(log_id=log.id, issue_id=issue.id, actor_id=owner.id, is_admin=False, minutes=60)
    assert spent == 60
...
def test_owner_only_edit(env):
    ...
    with pytest.raises(WorklogPermissionError):
        update_worklog(log_id=log.id, issue_id=issue.id, actor_id=other.id, is_admin=False, minutes=60)
    update_worklog(log_id=log.id, issue_id=issue.id, actor_id=other.id, is_admin=True, minutes=90)  # ADMIN 可改他人
...
def test_subtree_summary_no_join_amplification(env):
    ...
    Issue.objects.create(
        name="S", project=proj, parent=issue, sequence_id=2, sort_order=200, created_by=owner, estimate_minutes=120
    )
    for i in range(3):
