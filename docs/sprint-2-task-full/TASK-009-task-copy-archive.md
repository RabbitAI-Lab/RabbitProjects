...
| 优先级 | P2（标准版完整级） |
| 所属模块 | M4-TASK｜任务核心 |
| 文档状态 | 已实现（Implemented，2026-09-05；实现偏差见 ADR-0013/0014/0015 及各回改注记） |
| 最后更新日期 | 2026-09-06（P4 批次跨文档回改：§2.3 补回收站物理清理待建登记） |
...
> **回收站与物理删除（P4 批次跨文档登记注）**：Issue 行物理删除路径当前不存在（回收站仅软删 + 管理端恢复，UI P4 评估——§1.4「回收站 UI」行）；P4 `TASK-015` 基线 `purged` 兜底依赖的回收站物理清理为待建项（TASK-015 BR-04 登记：其前置表述为「上游未来启用回收站物理清理（待建，P4 评估）」，`ON DELETE SET_NULL` 快照行保留系防御性兜底）。来源：TASK-015/p4 BR-04（P4 批次 2026-09-06 登记）
