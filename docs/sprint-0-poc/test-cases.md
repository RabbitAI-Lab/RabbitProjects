...
| 底部动作 | 【默认态】[重置]（恢复默认）+ [保存到视图]（写回当前视图 display_props；内置视图允许——BR-03）；未保存关闭 = 仅本会话生效 + 已修改条 | 交互断言 | BOARD-003 §3.3 / BR-03 |
...
| 甘特 ⋯ 菜单 | 【交互】甘特控制条右端 ⋯（与 BOARD-003 视图 ⋯ 并存，O2）→ 导出 PNG / 全屏 / 重置缩放 | 交互断言 | GANTT-002 §3.3 入口行 / 原型 O2 |

### C.99 甘特左栏行树与栏宽（归属：GANTT-001 §3.1）
...
| 组件/字段/交互 | 规格 | 断言方式 | 来源 |
| --- | --- | --- | --- |
| 列头 | 【默认态】日：日期数字（+节假日名）；周：起–止区间；月：年月；role=application + aria-roledescription=甘特图 | 断言 | GANTT-001 §3.1 / §3.6 / 原型 O3 |
| 周末底纹 | 【默认态】6% 灰；中国法定节假日 10% 灰 + 节假名列头标注 | 断言 | GANTT-001 §3.1 周末/节假日底纹行 |
...
| VIEWER 只读 | 【条件态】访客拖拽禁用（PATCH 403 提示）；<1024px 拖拽仅桌面 | 条件断言 | GANTT-002 §3.4 / 权限矩阵 |
...
| 工具条 | 【默认态】名称过滤（防抖 300ms）+ 类型/上传人/时间筛选 + 视图切换 + 上传按钮（VIEWER 禁用） | 断言 | FILE-002 §3.1 工具条行 |
...
| 可见性角标 | 【条件态】🔒 仅管理员 / 👥 指定成员 / 🔗N 分享数（CONTRIBUTOR 列表不可见 admin 态文件） | 条件断言 | FILE-002 §3.3 / BR 三态 |
...
| 直传口径 | 【交互】上传到 MinIO（Django 零字节流）；>50MB 自动走分片（FILE-003） | 交互断言 | FILE-002 §4 / 概览 §5 |
...
| 配额预检 | 【条件态】将满（≥95%）：上传前弹层用量/配额 + 「仍要上传」/「取消」 | 条件断言 | FILE-002 §3.2 / BR-14 |
| 配额条 | 【默认态】工作区存储用量条（warn/full 分色） | 断言 | FILE-002 §3.2 / 原型 |
...
| 组件/字段/交互 | 规格 | 断言方式 | 来源 |
| --- | --- | --- | --- |
| 已删列表 | 【默认态】名称/原位置/删除人/删除时间/剩余天数；按 file.delete R1 口径过滤（CONTRIBUTOR 仅本人、ADMIN 全量） | 断言 | FILE-002 §3.1 回收站行 / BR-13 |
...
| 组件/字段/交互 | 规格 | 断言方式 | 来源 |
| --- | --- | --- | --- |
| 抽屉框架 | 【默认态】头部（名 + vN · 大小 · 人 · 时间 + [版本▾][下载] ✕）；960px；role=dialog + 焦点陷阱；Esc 关闭 | 断言 | FILE-003 §3.2 / §3.5 |
...
### C.121 版本面板与对比/回滚（归属：FILE-003 §3.2）
...
| 入口权限 | 【条件态】VIEWER 无分享项（⋯ 菜单禁用） | 条件断言 | FILE-004 权限矩阵 |
...
| 吊销确认 | 【条件态】「吊销后链接立即失效，访问者将看到『链接不存在或已失效』」 | 条件断言 | FILE-004 §3.2 / §7.2-3 |
