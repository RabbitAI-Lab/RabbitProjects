...
_BUILTIN_DEFAULT_STATES = {
    ...
    "version": 0,
    "groups": [
        {"group": "unstarted", "color": "#9CA3AF", "states": [{"name": "待办", "sequence": 1}]},
        {"group": "started", "color": "#3B82F6", "states": [{"name": "进行中", "sequence": 2}]},
        {"group": "completed", "color": "#10B981", "states": [{"name": "已完成", "sequence": 3}]},
        {"group": "cancelled", "color": "#6B7280", "states": [{"name": "已取消", "sequence": 4}]},
    ],
}
_LABEL_LIMIT = 100
...
def record_login_hits(user) -> None:
    ...
    rows = WorkspaceMember.objects.filter(member=user, is_active=True, deleted_at__isnull=True).values_list(
        "workspace_id", "member_id"
    )
    for ws_id, member_id in rows:
        WorkspaceLoginDailyAggregate.objects.get_or_create(workspace_id=ws_id, member_id=member_id, hit_date=today)
...
class WorkspaceGovernanceService:
    ...
    def archive(self, ws: Workspace, *, actor) -> dict:
        ...
        if ws.archived_at:  # 幂等（BR-01）
            return {
                "slug": ws.slug,
                "archived_at": ws.archived_at.isoformat(),
                "affected_projects": ws.projects.filter(deleted_at__isnull=True).count(),
                "idempotent": True,
            }
        # FILE-007（P4 R6）归档强制合规：阻断性检查（BR-01 未结任务/活跃
        # LegalHold）→ 409 RESOURCE_STATE_INVALID 列明清单；force 仅越警告
        from plane.db.services.archive_compliance import check_archive_compliance

        compliance = check_archive_compliance(ws)
        if compliance["blocking"]:
            from plane.base.exception import AppException

            raise AppException(
                "RESOURCE_STATE_INVALID",
                message="归档前置合规检查未通过（存在未结任务或活跃法务保留）",
                details=[
                    {"field": b["kind"], "code": "BLOCKING", "message": str(b)[:200]}
                    for b in compliance["blocking"][:20]
                ],
            )
    ...
    def _require_owner(ws: Workspace, actor) -> None:
        row = WorkspaceMember.objects.filter(
            workspace=ws, member=actor, is_active=True, deleted_at__isnull=True
        ).first()
    ...
    def list_labels(ws: Workspace) -> tuple[list[dict], dict]:
        rows = WorkspaceLabel.objects.filter(workspace=ws, deleted_at__isnull=True).order_by("created_at", "id")
        data = [
            {
                "id": str(r.id),
                "name": r.name,
        ...
        total = rows.count()
        return data, {"count": total, "limit": _LABEL_LIMIT, "remaining": max(0, _LABEL_LIMIT - total)}
    ...
    def create_label(self, ws: Workspace, *, payload: dict, actor) -> dict:
        ...
        if not name or len(name) > 50:
            raise AppException(
                "VALIDATION_ERROR", message="标签名必填且 ≤50 字符", details=[{"field": "name", "code": "INVALID"}]
            )
        if not _LABEL_COLOR_RE.fullmatch(color):
            raise AppException(
                "VALIDATION_ERROR", message="颜色必须为 #RRGGBB", details=[{"field": "color", "code": "INVALID"}]
            )
        if WorkspaceLabel.objects.filter(workspace=ws, name=name, deleted_at__isnull=True).exists():
            raise AppException(
                "RESOURCE_ALREADY_EXISTS", message="同名全局标签已存在", details=[{"field": "name", "code": "UNIQUE"}]
            )
    ...
    def update_label(self, ws: Workspace, label: WorkspaceLabel, *, payload: dict) -> dict:
        ...
        if "name" in payload:
            ...
            if (
                WorkspaceLabel.objects.filter(workspace=ws, name=name, deleted_at__isnull=True)
                .exclude(pk=label.pk)
                .exists()
            ):
        ...
        return {"id": str(label.id), "name": label.name, "color": label.color, "description": label.description}
    ...
    def delete_label(self, ws: Workspace, label: WorkspaceLabel, *, actor) -> dict:
        ...
        override_ids = list(Label.objects.filter(overrides_global_id=label.id).values_list("id", flat=True))
        IssueLabel.objects.filter(
            label_id__in=override_ids,
            deleted_at__isnull=True,
        ).update(name_snapshot=label.name)
        return {"id": str(label.id), "deleted_at": label.deleted_at.isoformat(), "affected_issues": len(affected)}
    ...
    def affected_issue_ids(label: WorkspaceLabel) -> set:
        ...
        direct = IssueLabel.objects.filter(
            label__origin="global",
            deleted_at__isnull=True,
        )  # 直接引用经下发创建的项目级行（见 merged_labels 的创建路径）
        # 直接引用口径：项目 Label 行 origin=global 且名字与全局一致（下发投影）
        projected = Label.objects.filter(
            project__workspace_id=label.workspace_id, origin="global", name=label.name, deleted_at__isnull=True
    ...
    def put_default_states(self, ws: Workspace, *, payload: dict, actor) -> dict:
        ...
        ws.default_states = {"name": str(payload.get("name") or "默认状态集"), "version": version, "groups": groups}
        ws.updated_by = actor
    ...
    def _validate_state_groups(payload: dict) -> list[dict]:
        ...
        for g in groups:
            if not isinstance(g, dict) or g.get("group") not in _STATE_GROUPS:
                raise AppException(
                    "VALIDATION_ERROR", message="group 必须为五组之一", details=[{"field": "groups", "code": "INVALID"}]
                )
            if g["group"] in seen:
            ...
            if not isinstance(states, list) or not states:
                raise AppException(
                    "VALIDATION_ERROR",
                    message="每组至少 1 个状态",
                    details=[{"field": f"groups.{g['group']}.states", "code": "INVALID"}],
                )
            for s in states:
                ...
                if not name or len(name) > 50:
                    raise AppException(
                        "VALIDATION_ERROR",
                        message="状态名非空且 ≤50 字符",
                        details=[{"field": "states.name", "code": "INVALID"}],
                    )
                if s.get("sequence") != seq_expect:
                    raise AppException(
                        "VALIDATION_INVALID_PARAM",
                        message="sequence 必须从 1 连续递增",
                        details=[{"field": "states.sequence", "code": "INVALID", "message": f"期望 {seq_expect}"}],
                    )
                seq_expect += 1
        if seen != set(_STATE_GROUPS):
            raise AppException(
                "VALIDATION_ERROR", message="五组（backlog/unstarted/started/completed/cancelled）各需 ≥1"
            )
        return groups
    ...
    def activity_stats(ws: Workspace, *, days: int) -> dict:
        ...
        total = WorkspaceMember.objects.filter(workspace=ws, is_active=True, deleted_at__isnull=True).count()
        member_ids7 = set(
            IssueActivity.objects.filter(
                issue__project__workspace_id=ws.id,
                created_at__date__gte=since7,
            ).values_list("actor_id", flat=True)
        )
        member_ids7 |= set(
            WorkspaceLoginDailyAggregate.objects.filter(workspace=ws, hit_date__gte=since7).values_list(
                "member_id", flat=True
            )
        )
        member_ids30 = set(
            IssueActivity.objects.filter(
                issue__project__workspace_id=ws.id,
                created_at__date__gte=since30,
            ).values_list("actor_id", flat=True)
        )
        member_ids30 |= set(
            WorkspaceLoginDailyAggregate.objects.filter(workspace=ws, hit_date__gte=since30).values_list(
                "member_id", flat=True
            )
        )
        ...
        hits = dict(
            WorkspaceLoginDailyAggregate.objects.filter(
                workspace=ws,
                hit_date__gte=today - timedelta(days=days - 1),
            )
        ...
        top = (
            {"issue": round(issue_c / denom, 2), "comment": round(comment_c / denom, 2)}
            if denom
            else {"issue": None, "comment": None}
        )
        return {
