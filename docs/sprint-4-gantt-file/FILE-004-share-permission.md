...
> **P4 FILE-006 合规策略第四失效源**：本读时失效判定现仅含 **expired / revoked / 源失效（invalidated）** 三态；P4 合规策略生效时并入第四失效源——`FILE-006` 有效策略 `share_link=deny` 生效时，**存量分享链接自动失效**，读时判定按 `410 RESOURCE_GONE` 处理，与既有失效三态并列、复用统一失效页同码同文案（§2.4 防枚举口径不变；`FILE-006` BR-05 创建侧拦截返回 `RESOURCE_STATE_INVALID`）。（来源：FILE-006 BR-05，P4 批次 2026-09-06 登记）
