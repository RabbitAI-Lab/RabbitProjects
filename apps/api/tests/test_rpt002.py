...
@pytest.fixture()
def env(db):
    owner = User.objects.create_user(email="r2-owner@rabbit.dev", password="Rabbit123!", display_name="张三")
    zhang = User.objects.create_user(email="r2-zhang@rabbit.dev", password="Rabbit123!", display_name="李四")
    viewer = User.objects.create_user(email="r2-viewer@rabbit.dev", password="Rabbit123!", display_name="只读")
    outsider = User.objects.create_user(email="r2-out@rabbit.dev", password="Rabbit123!", display_name="外人")
    ws = Workspace.objects.create(name="W", slug=f"w-r2-{owner.id.hex[:8]}", owner=owner, created_by=owner)
    for u, r in ((owner, WorkspaceRole.OWNER), (zhang, WorkspaceRole.MEMBER), (viewer, WorkspaceRole.MEMBER)):
    ...
    ProjectMember.objects.create(project=proj, member=zhang, role=ProjectRole.CONTRIBUTOR, created_by=owner)
    ...
    if not State.objects.filter(project=proj, group=State.Group.BACKLOG).exists():
        State.objects.create(
            project=proj, name="待规划", group=State.Group.BACKLOG, color="#94A3B8", sort_order=100.0, created_by=owner
        )
    states = {s.group: s for s in State.objects.filter(project=proj)}
    today = timezone.now().date()

    def mk(
        name, *, group="unstarted", assignee=None, target=None, estimate=None, completed_days_ago=None, archived=False
    ):
    ...
    mk("t3", group="completed", assignee=zhang, estimate=60, completed_days_ago=2)
    ...
    WorkLog.objects.create(issue=t1, actor=zhang, minutes=90, worked_on=today, created_by=owner)
    WorkLog.objects.create(
        issue=t1, actor=zhang, minutes=60, worked_on=today - timedelta(days=40), created_by=owner
    )  # 30d 窗口外
    return {
        "owner": owner,
        "zhang": zhang,
...
def test_progress_numbers(env):
    ...
    d = res.json()["data"]
    assert d["state_distribution"] == {"backlog": 0, "unstarted": 1, "started": 2, "completed": 2, "cancelled": 1}
...
def test_members_rows_and_consistency(env):
    ...
    assert d["totals"]["open_count"] == (sum(r["open_count"] for r in d["rows"]) + d["unassigned"]["open_count"])
    assert d["totals"]["logged_minutes_30d"] == sum(r["logged_minutes_30d"] for r in d["rows"])
    # 手工逐条筛选对照（口径单源验证）
    manual_open = Issue.objects.filter(
        project=env["proj"], archived_at__isnull=True, state__group__in=("unstarted", "started")
    ).count()
...
def test_members_ordering(env):
    res = _client(env["viewer"]).get(_url(env, "members/") + "?order_by=-logged_minutes_30d", format="json")
    rows = res.json()["data"]["rows"]
...
def test_members_outsider_404(env):
    assert _client(env["outsider"]).get(_url(env, "members/"), format="json").status_code == 404
