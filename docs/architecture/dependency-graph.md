...
| `TASK-012`（高级自定义字段与字段权限） | `TASK-008` | 高级字段扩展基础字段类型；字段权限先落固定角色码，自定义角色矩阵待 Sprint 8 `AUTH-008` 落地后自动生效（零改表，跨迭代倒挂边撤销） |
...
| 下游功能 | 上游依赖 | 依赖说明 |
| --- | --- | --- |
| `WF-001`（可视化工作流画布与引擎数据模型） | `TASK-001` `TASK-011` | 工作流替换固定状态流转，画布与引擎按任务类型挂载 |
