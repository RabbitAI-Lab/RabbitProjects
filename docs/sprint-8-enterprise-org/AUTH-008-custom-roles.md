...
| 所属模块 | M1-AUTH｜账号与权限 |
| 文档状态 | 待评审（Draft） |
| 最后更新日期 | 2026-09-05（R1 修复 10 项：§1 架构偏离登记块（vs rbac 基线+覆盖）、role.manage 对齐 WS 级、26 码全量对齐注册表、前端改动清单如实化（快照扩展+并集分支）、template_key + ID 引用契约、workspaces 前缀 + 信封重写、GUEST 天花板拒绝挂接（BR-16）、批次列 UUID 容量、apps/api/plane/ 路径、42/60/查询数统一；R2 复评 PASS 9.5×5 一次过） |
