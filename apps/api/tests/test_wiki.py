...
@pytest.fixture()
def env(db):
    owner = User.objects.create_user(email="wk-owner@rabbit.dev", password="Rabbit123!", display_name="管理员")
    ...
    return {"owner": owner, "editor": editor, "viewer": viewer, "ws": ws, "proj": proj, "space": space}
...
class TestPublishChain:
    ...
    def test_publish_and_version_chain(self, env):
        ...
        page.draft_json = DOC
        page.save()
        v1 = WikiService().publish(page_id=page.id, actor=env["owner"], base_version_id=None, change_summary="初版")
        ...
        v2 = WikiService().publish(page_id=page.id, actor=env["owner"], base_version_id=v1.id)
    ...
    def test_publish_conflict_409(self, env):
        ...
        page.base_version = v1
        page.save(update_fields=["draft_json", "base_version", "updated_at"])
        WikiService().publish(page_id=page.id, actor=env["editor"], base_version_id=v1.id)
        # 本人（落后 base）再发 → 409
        page.refresh_from_db()
        page.draft_json = DOC
        page.base_version = v1
    ...
    def test_rollback_creates_new_version(self, env):
        ...
        page.save()
        v1 = WikiService().publish(page_id=page.id, actor=env["owner"], base_version_id=None)
        v2_doc = {"type": "doc", "content": [{"type": "paragraph", "content": [{"type": "text", "text": "v2 内容"}]}]}
        page.refresh_from_db()
        page.draft_json = v2_doc
        page.base_version = v1
        page.save(update_fields=["draft_json", "base_version", "updated_at"])
        WikiService().publish(page_id=page.id, actor=env["owner"], base_version_id=v1.id)
        v3 = WikiService().rollback(page_id=page.id, actor=env["owner"], target_version_id=v1.id)
        ...
        with pytest.raises(RollbackTargetError):
            WikiService().rollback(page_id=page.id, actor=env["owner"], target_version_id=v3.id)
...
class TestPermissionTiers:
    ...
    def test_api_permission_matrix(self, env):
        ...
        resp = _client(env["owner"]).post(
            f"{_base(env)}spaces/", {"project_id": str(env["proj"].id), "name": "新空间"}, format="json"
        )
        assert resp.status_code == 201
        assert (
            _client(env["viewer"])
            .post(f"{_base(env)}spaces/", {"project_id": str(env["proj"].id), "name": "V 建"}, format="json")
            .status_code
            == 403
        )
        ...
        assert (
            _client(env["viewer"])
            .post(f"{_base(env)}pages/{page2.id}/publish/", {"base_version_id": None}, format="json")
            .status_code
            == 403
        )
...
class TestPageTree:
    ...
    def test_same_level_duplicate_title(self, env):
        _mk_page(env, "同名")
        resp = _client(env["owner"]).post(
            f"{_base(env)}pages/", {"space_id": str(env["space"].id), "title": "同名"}, format="json"
        )
        assert resp.status_code == 409
    ...
    def test_move_cycle_rejected(self, env):
        ...
        resp = _client(env["owner"]).post(f"{_base(env)}pages/{p1.id}/move/", {"parent_id": str(p2.id)}, format="json")
        assert resp.status_code == 409
...
class TestTrash:
    def test_soft_delete_tree_and_restore(self, env):
        ...
        # 回收站：editor（删除人）可见
        trash = _client(env["editor"]).get(f"{_base(env)}pages/trash/?space_id={env['space'].id}").json()["data"]
        assert any(t["id"] == str(p1.id) for t in trash)
        # viewer 无 wiki.update 权限 → 403
        assert _client(env["viewer"]).get(f"{_base(env)}pages/trash/?space_id={env['space'].id}").status_code == 403
        # 恢复
...
class TestSearch:
    def _publish_page(self, env, title, text, space=None):
        page = WikiPage.objects.create(
            ...
            title=title,
            created_by=env["owner"],
            draft_json={"type": "doc", "content": [{"type": "paragraph", "content": [{"type": "text", "text": text}]}]},
        )
    ...
    def test_search_permission_prefiltered(self, env):
        ...
        WorkspaceMember.objects.create(
            workspace=env["ws"], member=ws_admin, role=WorkspaceRole.ADMIN, created_by=env["owner"]
        )
        assert len(search_wiki(actor_id=ws_admin.id, q="错误码")) == 1

    def test_search_endpoint(self, env):
        self._publish_page(env, "规范页", "错误码约定正文")
        resp = _client(env["viewer"]).get(f"{_base(env)}search/?q=错误码&space_id={env['space'].id}")
        assert resp.status_code == 200
