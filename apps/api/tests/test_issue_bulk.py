...
class TestBulkUpdateBasics:
    ...
    def test_cross_project_id_item_level(self, env):
        ...
        resp = _Client(env["owner"]).patch(
            _bulk_url(env),
            {
                "issue_ids": [str(a.id), str(foreign.id)],
                "patch": {"priority": "high"},
            },
        )
...
class TestStateGuardAndRollback:
    def test_guard_blocks_whole_batch_with_index(self, env):
        ...
        IssueLink.objects.create(
            issue=issues[6], related_issue=blocker, relation_type="is_blocked_by", created_by=env["owner"]
        )
        ...
        resp = _Client(env["owner"]).patch(
            ...
            {
                "issue_ids": _ids(*issues),
                "patch": {"state_id": str(env["done"].id)},
                "comment": "迭代收尾",
            },
        )
...
class TestSetOperations:
    ...
    def assignee_env(self, env):
        ...
        ProjectMember.objects.create(
            project=env["proj"], member=env["contributor_b"], role=ProjectRole.CONTRIBUTOR, created_by=env["owner"]
        )
        return env
    ...
    def test_assignees_add_union(self, assignee_env):
        ...
        resp = _Client(env["owner"]).patch(
            ...
            {
                "issue_ids": [str(a.id)],
                "assignees": {"mode": "add", "assignee_ids": [str(env["contributor_b"].id), str(env["member"].id)]},
            },
        )
    ...
    def test_assignees_over_limit_item_failure(self, assignee_env):
        ...
        more = [User.objects.create_user(email=f"bulk-x{i}@rabbit.dev", password="Rabbit123!") for i in range(8)]
        for u in more:
            ProjectMember.objects.create(
    ...
    def test_labels_three_modes(self, label_env):
        ...
        assert set(IssueLabel.objects.filter(issue=a).values_list("label_id", flat=True)) == {env["lb"].id}
        resp3 = _Client(env["owner"]).patch(
            base,
            {  # replace 替换
                "issue_ids": [str(a.id)],
        ...
        assert set(IssueLabel.objects.filter(issue=a).values_list("label_id", flat=True)) == {env["la"].id}

    def test_labels_foreign_label_payload_level(self, label_env):
        env = label_env
        a = env["mk"]("A")
        resp = _Client(env["owner"]).patch(
            _bulk_url(env), {"issue_ids": [str(a.id)], "labels": {"mode": "add", "label_ids": [str(env["lother"].id)]}}
        )
...
class TestBulkArchive:
    ...
    def test_archive_member_only_own(self, env):
        ...
        resp = _Client(env["member"]).post(_bulk_url(env, "archive/"), {"issue_ids": _ids(a, b)})
        assert resp.status_code == 400
        ...
        resp2 = _Client(env["member"]).post(_bulk_url(env, "archive/"), {"issue_ids": [str(b.id)]})  # 本人创建可归档
...
class TestBulkDelete:
    def test_confirm_count_mismatch(self, env):
        ...
        resp = _Client(env["owner"]).delete(_bulk_url(env), {"issue_ids": _ids(a, b), "confirm_count": 1})
    ...
    def test_delete_with_cascade(self, env):
        ...
        resp = _Client(env["owner"]).delete(_bulk_url(env), {"issue_ids": [str(root.id)], "confirm_count": 1})
        assert resp.status_code == 200
        data, meta = resp.json()["data"], resp.json()["meta"]
    ...
    def test_contributor_mixed_foreign_items(self, env):
        ...
        resp = _Client(env["member"]).delete(_bulk_url(env), {"issue_ids": _ids(mine, theirs), "confirm_count": 2})
        ...
        resp2 = _Client(env["member"]).delete(_bulk_url(env), {"issue_ids": [str(mine.id)], "confirm_count": 1})
        assert resp2.status_code == 200
    ...
    def test_viewer_rejected_on_all_write_endpoints(self, env):
        ...
        resp4 = _Client(env["viewer"]).post(base + "preview/", {"issue_ids": [str(a.id)], "action": "delete"})
...
class TestPreview:
    def test_stats_and_denied(self, env):
        ...
        WorkLog.objects.create(issue=flat[0], actor=env["owner"], worked_on=datetime.date(2026, 9, 1), minutes=30)
        IssueComment.objects.create(issue=flat[1], actor=env["owner"], comment_html="<p>c</p>")
        before = {i.id: i.updated_at for i in [*roots, *flat]}
        resp = _Client(env["member"]).post(_bulk_url(env, "preview/"), {"issue_ids": _ids(*roots), "action": "delete"})
        assert resp.status_code == 200
        data = resp.json()["data"]
        ...
        assert data["denied"] == [  # member 非 ADMIN 非创建者 → 全部项级暴露
            {"index": i, "issue_key": f"BK-{r.sequence_id}", "reason": "仅创建者可删除"} for i, r in enumerate(roots)
        ]
    ...
    def test_preview_denied_missing_id(self, env):
        ...
        resp = _Client(env["owner"]).post(
            _bulk_url(env, "preview/"), {"issue_ids": [str(a.id), str(uuid.uuid4())], "action": "archive"}
        )
        assert resp.status_code == 200
        assert resp.json()["data"]["denied"] == [{"index": 1, "issue_key": None, "reason": "任务不存在或不可见"}]
        assert resp.json()["data"]["selected"] == 2 and resp.json()["data"]["affected_total"] == 1

    def test_preview_invalid_action(self, env):
        a = env["mk"]("A")
        resp = _Client(env["owner"]).post(_bulk_url(env, "preview/"), {"issue_ids": [str(a.id)], "action": "copy"})
        assert resp.status_code == 400
...
class TestThrottle:
    def test_11th_request_429(self, env, monkeypatch):
        ...
        client = _Client(user)
        for _ in range(10):
            resp = client.post(_bulk_url(env, "preview/"), {"issue_ids": [str(a.id)], "action": "delete"})
            assert resp.status_code == 200
        resp11 = client.post(_bulk_url(env, "preview/"), {"issue_ids": [str(a.id)], "action": "delete"})
        ...
        # 其他用户不受影响（10/min/用户）
        resp_other = _Client(env["owner"]).post(
            _bulk_url(env, "preview/"), {"issue_ids": [str(a.id)], "action": "delete"}
        )
...
class TestIdempotencyKey:
    def test_replay_returns_first_response(self, env):
        ...
        url = _bulk_url(env)
        first = c.delete(url, {"issue_ids": _ids(a, b), "confirm_count": 2}, format="json", headers=headers)
        assert first.status_code == 200
        deleted_at_snapshot = {i.id: i.deleted_at for i in Issue.objects.filter(pk__in=[a.id, b.id])}
        second = c.delete(url, {"issue_ids": _ids(a, b), "confirm_count": 2}, format="json", headers=headers)
    ...
    def test_no_key_pass_through(self, env):
        a = env["mk"]("A")
        resp = _Client(env["owner"]).patch(_bulk_url(env), {"issue_ids": [str(a.id)], "patch": {"priority": "high"}})
        assert resp.status_code == 200
...
def _raw_insert(cur, table: str, **cols) -> uuid.UUID:
    ...
    cur.execute(f'INSERT INTO "{table}" ({cols_sql}) VALUES ({placeholders}) RETURNING id', list(values.values()))
    ...
    def test_second_writer_gets_409(self, db):
        ...
        try:
            with conn.cursor() as cur:
                ...
                owner = _raw_insert(cur, "users", email=f"nw-{suffix}@rabbit.dev", display_name="并发")
                ws = _raw_insert(cur, "workspaces", name="NW", slug=f"w-nw-{suffix}", owner_id=owner)
                _raw_insert(cur, "workspace_members", workspace_id=ws, member_id=owner, role=20, is_active=True)
                proj = _raw_insert(
                    cur, "projects", name="NWP", identifier=suffix[:8].upper(), workspace_id=ws, status="active"
                )
                _raw_insert(
                    cur, "project_members", workspace_id=ws, project_id=proj, member_id=owner, role=30, is_active=True
                )
                issue_ids = [
                    _raw_insert(
                        cur,
            ...
            with conn.cursor() as cur:  # 第二连接 BEGIN + FOR UPDATE NOWAIT 锁住目标行
                cur.execute("BEGIN")
                cur.execute("SELECT id FROM issues WHERE id = ANY(%s) FOR UPDATE NOWAIT", [[str(i) for i in issue_ids]])
                ...
                # 锁持有期间零写入
                assert Issue.objects.filter(pk__in=issue_ids, priority="none").count() == 3
...
class TestUpstreamEpochParams:
    def test_delete_subtree_default_unchanged(self, env, monkeypatch):
        ...
        calls: list[tuple] = []
        monkeypatch.setattr(ih.record_delete, "delay", lambda *a, **k: calls.append(a))
        root = env["mk"]("R")
    ...
    def test_delete_subtree_shared_epoch_suppressed(self, env, monkeypatch):
        ...
        with _DjTestCase.captureOnCommitCallbacks(execute=True):
            out = delete_subtree(root.id, env["owner"].id, epoch=shared, suppress_activity=True)
    ...
    def test_archive_subtree_default_unchanged(self, env, monkeypatch):
        ...
        calls: list[tuple] = []
        monkeypatch.setattr(ih.record_archive, "delay", lambda *a, **k: calls.append(a))
        root = env["mk"]("R")
        env["mk"]("C", parent=root)
        with _DjTestCase.captureOnCommitCallbacks(execute=True):
    ...
    def test_archive_subtree_shared_epoch_suppressed(self, env, monkeypatch):
        ...
        root = env["mk"]("R")
        with _DjTestCase.captureOnCommitCallbacks(execute=True):
            out = archive_subtree(issue_id=root.id, actor_id=env["owner"].id, epoch=42.0, suppress_activity=True)
...
class TestWorkerBatch:
    ...
    def test_record_activity_batch_rows_and_idempotent(self, env):
        ...
        assert IssueActivity.objects.filter(issue_id__in=[a.id, b.id], epoch=epoch).count() == 2  # 行级幂等
    ...
    def test_enqueue_activity_rows_fallback_sync(self, env, monkeypatch):
        ...
        assert IssueActivity.objects.filter(issue_id=a.id, epoch=epoch, comment="batch: 降级").exists()
