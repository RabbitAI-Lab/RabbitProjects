    ...
    def runner(cmd, **kwargs):
        ...
        if cmd[0] == "pg_restore":
            return subprocess.CompletedProcess(
                cmd, 0 if restorable else 1, stderr=b"magic block mismatch" if not restorable else b""
            )
...
def _mock_storage():
    return mock.patch.multiple(
        "plane.storage.minio",
        upload_fileobj=mock.DEFAULT,
        put_object=mock.DEFAULT,
        remove_object=mock.DEFAULT,
...
def test_ut08_corrupt_dump_rejected(monkeypatch):
    ...
    monkeypatch.setattr(bk.subprocess, "run", _fake_pg_dump(size_bytes=128, restorable=False))
...
def test_ut08_success_path_records_all_three_checks(monkeypatch):
    ...
    assert not os.path.exists("/tmp/rp-backup-") or not any(
        f.startswith("rp-backup-") for f in os.listdir("/tmp")
    )  # 临时文件清理
...
def test_ut09_streak_alerts_admins(monkeypatch):
    ...
    admin = User.objects.create_user(email="bk-admin@rabbit.dev", password="Rabbit123!")
    SystemAdmin.objects.create(user=admin)
    with (
        _mock_storage(),
        mock.patch("plane.bgtasks.notifications.send_workspace_notification") as notify,
        pytest.raises(bk.BackupVerificationError),
    ):
        bk.run_backup(kind="daily")
    ...
    mine = [c for c in notify.delay.call_args_list if c.kwargs.get("receiver_id") == str(admin.id)]
    assert len(mine) == 1
...
def test_ut09_success_breaks_streak(monkeypatch):
    ...
    monkeypatch.setattr(bk.dj_settings, "BACKUP_MIN_SIZE_BYTES", 64)
    admin = User.objects.create_user(email="bk-admin2@rabbit.dev", password="Rabbit123!")
    SystemAdmin.objects.create(user=admin)
...
def test_ut10_cleanup_removes_only_expired():
    ...
    assert removed == 2
    assert {c.kwargs["key"] for c in rm.call_args_list} == {"backups/pg/old/db.dump", "backups/uploads/x"}
...
def test_ut12_record_drill_writes_report():
    ok = BackupRun.objects.create(
        ...
        object_key="backups/pg/x/db.dump",
    )
    assert bk.record_drill(str(ok.id), rto_seconds=412, smoke_passed=18, smoke_total=18, notes="演练栈即弃")
    ok.refresh_from_db()
    r = ok.drill_report
    ...
    # 非 success 行不可回写
    failed = BackupRun.objects.create(kind="daily", status="failed", started_at=timezone.now())
    assert bk.record_drill(str(failed.id), rto_seconds=1, smoke_passed=0, smoke_total=18, notes="") is False
    cache.clear()
...
def test_ops_endpoints_permission_and_shape(db):
    ...
    plain = User.objects.create_user(email="ops-plain@rabbit.dev", password="Rabbit123!")
    admin = User.objects.create_user(email="ops-admin@rabbit.dev", password="Rabbit123!")
    SystemAdmin.objects.create(user=admin)
    c = APIClient()
...
def test_ops_backup_trigger_throttled_within_10min(db, monkeypatch):
    ...
    admin = User.objects.create_user(email="ops-trg@rabbit.dev", password="Rabbit123!")
    SystemAdmin.objects.create(user=admin)
    BackupRun.objects.create(kind="manual", status="success", started_at=tz.now() - tz.timedelta(minutes=3))
    c = APIClient()
    c.force_authenticate(admin)
