...
@pytest.fixture()
def env(db):
    owner = User.objects.create_user(email="r3-owner@rabbit.dev", password="Rabbit123!", display_name="负责人")
    member = User.objects.create_user(email="r3-member@rabbit.dev", password="Rabbit123!", display_name="成员")
    ws = Workspace.objects.create(name="W", slug=f"w-r3-{owner.id.hex[:8]}", owner=owner, created_by=owner)
...
class TestAutomationTriggers:
    def test_state_changed_match(self, env):
        ...
        assert match_trigger(rule, {"type": "state_changed", "payload": {"to_state_id": "x", "to_group": "started"}})
        assert not match_trigger(rule, {"type": "state_changed", "payload": {"to_group": "completed"}})

    def test_field_changed_match(self, env):
        rule = {"type": "field_changed", "config": {"fields": ["priority", "assignees"]}}
        assert match_trigger(rule, {"type": "field_changed", "payload": {"field": "priority", "new_value": "high"}})
        assert not match_trigger(rule, {"type": "field_changed", "payload": {"field": "target_date"}})

    def test_issue_types_filter(self, env):
        rule = {"type": "state_changed", "config": {"issue_types": ["req-uuid"]}}
        assert not match_trigger(rule, {"type": "state_changed", "payload": {"issue_type_id": None}})


class TestEventGate:
    def test_origin_blocks_automation_chain(self, env):
        assert event_gate({"type": "state_changed", "origin": "automation:1", "chain_depth": 0}) == "origin"
        assert event_gate({"type": "field_changed", "origin": "automation:1", "chain_depth": 0}) == "origin"
        # due_approaching 不受闸 1（豁免）
        assert event_gate({"type": "due_approaching", "origin": "automation:1", "chain_depth": 0}) is None

    def test_chain_depth_melting(self, env):
        assert event_gate({"type": "state_changed", "chain_depth": 5, "origin": ""}) == "chain_depth"
        assert event_gate({"type": "state_changed", "chain_depth": 4, "origin": ""}) is None
...
class TestValidateRuleDefinition:
    def test_unknown_trigger(self):
        issues = validate_rule_definition(
            {
                "trigger": {"type": "weird"},
                "actions": [{"type": "set_field", "config": {"field": "priority", "value": "high"}}],
            }
        )
        assert any(i["code"] == "NOT_A_CHOICE" and "trigger" in i["field"] for i in issues)

    def test_action_limit(self):
        actions = [{"type": "set_field", "config": {"field": "priority", "value": "high"}}] * 6
        issues = validate_rule_definition({"trigger": {"type": "state_changed"}, "actions": actions})
        assert any(i["code"] == "LIMIT" for i in issues)

    def test_empty_actions(self):
        issues = validate_rule_definition({"trigger": {"type": "state_changed"}, "actions": []})
...
class TestDryRunZeroWrite:
    def test_dry_run_does_not_persist(self, env):
        ...
        issue = Issue.objects.create(
            project=env["proj"], name="i", state=env["states"]["待办"], created_by=env["owner"]
        )
...
class TestRunEventIntegration:
    def test_state_changed_triggers_set_field(self, env):
        ...
        rule = AutomationRule.objects.create(
            ...
            actions=[{"type": "set_field", "config": {"field": "estimate_minutes", "value": 30}}],
            created_by=env["owner"],
        )
    ...
    def test_chain_blocked_with_allow_rule_chain_off(self, env):
        AutomationRule.objects.create(
            ...
            name="链锁",
            trigger={"type": "state_changed", "config": {"to_group": "started"}},
            actions=[{"type": "set_field", "config": {"field": "priority", "value": "low"}}],
            created_by=env["owner"],
        )
        issue = Issue.objects.create(
            project=env["proj"], name="i", state=env["states"]["待办"], created_by=env["owner"]
    ...
    def test_transition_action_runs_engine(self, env):
        ...
        AutomationRule.objects.create(
            ...
            name="auto-submit",
            trigger={"type": "state_changed", "config": {"to_group": "started"}},
            actions=[{"type": "transition", "config": {"to_state_id": str(env["states"]["进行中"].id)}}],
            created_by=env["owner"],
        )
        invalidate_rules_cache(env["proj"].id)
    ...
    def test_consecutive_failures_circuit_breaker(self, env):
        rule = AutomationRule.objects.create(
            ...
            name="boom",
            trigger={"type": "state_changed", "config": {"to_group": "started"}},
            actions=[{"type": "transition", "config": {"to_state_id": "nonexistent-uuid"}}],  # 必失败
            created_by=env["owner"],
        )
        invalidate_rules_cache(env["proj"].id)
        issue = Issue.objects.create(
...
class TestWorkLogApprovalFSM:
    ...
    def test_submit_then_approve_freezes_and_locks(self, env):
        ...
        assert (
            WorkLogSummary.objects.get(project=env["proj"], actor=env["member"], week_start=ws).is_frozen is True
        )  # BR-13
    ...
    def test_approve_then_revoke_unlocks(self, env):
        ...
        assert WorkLogSummary.objects.get(project=env["proj"], actor=env["member"], week_start=ws).is_frozen is False
    ...
    def test_frozen_summary_rejects_daily_refresh(self, env):
        ...
        batch = submit(actor=env["member"], project=env["proj"], week_start=ws)
        review(batch_id=batch.id, reviewer=env["owner"], action="approve")
        s_frozen = WorkLogSummary.objects.get(project=env["proj"], actor=env["member"], week_start=ws)
        ...
        s_after = WorkLogSummary.objects.get(project=env["proj"], actor=env["member"], week_start=ws)
...
class TestMutationReverts:
    def test_soft_limit_mutation_red_then_green(self, env):
        ...
        with mpatch.object(_wla, "_week_range", return_value=(_date(2020, 1, 6), _date(2020, 1, 12))):
