...
def _make_tree(owner: User, depth: int = 5, proj: Project | None = None) -> list[Issue]:
    ...
    if proj is None:
        ws = Workspace.objects.create(name="W", slug=f"w-hier-{owner.id.hex[:8]}", owner=owner, created_by=owner)
        proj = Project.objects.create(name="P", identifier="HIER", workspace=ws, created_by=owner)
    chain: list[Issue] = []
    prev = None
...
def test_delete_subtree_cascades_and_purges_join_tables(owner):
    ...
    assert not Issue.objects.filter(id__in=[c.id for c in chain], deleted_at__isnull=True).exists()
