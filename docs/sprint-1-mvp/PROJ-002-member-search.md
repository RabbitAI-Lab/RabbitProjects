...
| 场景 | 处置 |
| --- | --- |
| 列表卡片 | 「已归档」badge（`⊘ 已归档` 灰徽标，§3.1） |
| 进入项目后 | 顶部琥珀色横幅「项目已归档，仅可查看」（`bg-amber-50 text-amber-700`，常驻，关闭不可） |
| 写入口 | 全部禁用：新建任务 / 编辑任务 / 看板拖拽 / 评论 / 附件上传（后端通用守卫 403 `PERM_PROJECT_ARCHIVED`，§2.5 归档只读口径） |
