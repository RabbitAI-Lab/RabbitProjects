...
def _sync_audit(monkeypatch):
    ...
    monkeypatch.setattr(mod.audit_record, "delay", lambda payload: mod.audit_record.run(payload))


@pytest.fixture()
def env(db):
    owner = User.objects.create_user(email="au-owner@rabbit.dev", password="Rabbit123!", display_name="主")
    member = User.objects.create_user(email="au-member@rabbit.dev", password="Rabbit123!", display_name="员")
    ws = Workspace.objects.create(name="A", slug=f"w-au-{owner.id.hex[:8]}", owner=owner, created_by=owner)
...
def _payload(env, key="k1", **over):
    p = {
        ...
        "actor": {"id": str(env["owner"].id), "name": "主", "email": "au-owner@rabbit.dev"},
...
class TestPipeline:
    ...
    def test_br11_blacklist(self, env):
        cleaned = blacklist_filter({"password": "x", "api_token": "y", "client_secret": "z", "role": "ok"})
    ...
    def test_compat_bridge_routes_events(self, env, django_capture_on_commit_callbacks):
        ...
        with django_capture_on_commit_callbacks(execute=True):
            record_audit("department.created", actor_id=str(env["owner"].id), object_id="d1")
        row = AuditLog.objects.filter(
            category="department", actor_id=str(env["owner"].id), action="created"
        ).first()  # 作用域过滤（坑 18：共享库有 worker 落行）
    ...
    def test_chain_verify_detects_tamper(self, env):
        ...
        mine = {str(r.id) for r in AuditLog.objects.filter(workspace_id=env["ws"].id, event_key__in=("k1", "k2"))}
        assert not [b for b in vr["broken"] if b in mine]  # 本作用域两行不破链
        # 裸 SQL 篡改 detail（绕 ORM 只增约束模拟攻击）
        with connection.cursor() as cur:
            cur.execute("UPDATE audit_log SET detail = '{\"evil\": true}'::jsonb WHERE event_key = 'k2'")
...
class TestSearchEndpoints:
    def _seed(self, env, n=5):
        for i in range(n):
            write_chained_row(_payload(env, f"k{i}", detail={"i": i}))
    ...
    def test_filters_and_meta(self, env):
        self._seed(env, 6)
        url = f"/api/v1/workspaces/{env['ws'].slug}/audit-logs/"
        r = _client(env["owner"]).get(url, {"category": "member", "per_page": 3})
        body = r.json()
    ...
    def test_unknown_filter_400(self, env):
        r = _client(env["owner"]).get(f"/api/v1/workspaces/{env['ws'].slug}/audit-logs/", {"hacker_field": "1"})
        assert r.status_code == 400

    def test_workspace_isolation(self, env, db):
        other_owner = User.objects.create_user(email="au2@rabbit.dev", password="x", display_name="乙")
        ws2 = Workspace.objects.create(
            name="B", slug=f"w-au2-{other_owner.id.hex[:6]}", owner=other_owner, created_by=other_owner
        )
        write_chained_row(_payload(env, "mine"))
        p2 = _payload(env, "theirs")
        p2["workspace_id"] = str(ws2.id)
        write_chained_row(p2)
        r = _client(env["owner"]).get(f"/api/v1/workspaces/{env['ws'].slug}/audit-logs/")
        keys = {row["event_key"] for row in r.json()["data"]}
        assert keys == {"mine"}  # BR-03 强制空间过滤

    def test_catalog(self, env):
        r = _client(env["owner"]).get(f"/api/v1/workspaces/{env['ws'].slug}/audit-logs/catalog/")
    ...
    def test_export_csv_and_self_audit(self, env, django_capture_on_commit_callbacks):
        ...
        with django_capture_on_commit_callbacks(execute=True):
            r = _client(env["owner"]).post(
                f"/api/v1/workspaces/{env['ws'].slug}/audit-logs/exports/", {"password": "Rabbit123!"}, format="json"
            )
        assert r.status_code == 200
        assert r["Content-Type"].startswith("text/csv")
        assert b"role_changed" in r.content
        assert AuditLog.objects.filter(category="audit", action="exported").exists()  # BR-08

    def test_export_wrong_password_401(self, env):
        r = _client(env["owner"]).post(
            f"/api/v1/workspaces/{env['ws'].slug}/audit-logs/exports/", {"password": "bad"}, format="json"
        )
...
class TestInstanceEndpoint:
    def test_system_admin_self_audited(self, env, django_capture_on_commit_callbacks):
        SystemAdmin.objects.create(user=env["owner"], is_active=True, created_by=env["owner"])
        with django_capture_on_commit_callbacks(execute=True):
            r = _client(env["owner"]).get("/api/v1/instances/audit-logs/")
        assert r.status_code == 200
        assert AuditLog.objects.filter(action="instance_query", workspace_id__isnull=True).exists()

    def test_non_system_admin_403(self, env):
        assert _client(env["owner"]).get("/api/v1/instances/audit-logs/").status_code == 403
...
class TestRetention:
    def test_drop_old_partitions(self, env):
        ...
        with connection.cursor() as cur:
            cur.execute("SELECT EXISTS (SELECT 1 FROM pg_tables WHERE tablename='audit_log_p2020_01')")
            assert not cur.fetchone()[0]

    def test_registry_nonempty(self):
        assert EVENT_REGISTRY and all(actions for actions in EVENT_REGISTRY.values())
