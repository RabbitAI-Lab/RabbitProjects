...
@pytest.fixture()
def env(db):
    owner = User.objects.create_user(email="pf-owner@rabbit.dev", password="Rabbit123!",
                                     display_name="空间管理员")
    ...
    for email, role in (("pf-manager@rabbit.dev", WorkspaceRole.MEMBER),
...
def _mk_pf(env, name, parent=None, manager=None) -> Portfolio:
    return Portfolio.objects.create(
        workspace=env["ws"], name=name, parent_id=parent.id if parent else None,
        depth=1 if parent is None else parent.depth + 1,
        manager_id=manager.id if manager else None, created_by=env["owner"])
...
class TestTreeManagement:
    def test_ut01_depth_limit(self, env):
        ...
        resp = _client(env["owner"]).post(
            _base(env), {"name": "第四层", "parent_id": str(l3.id)}, format="json")
    ...
    def test_ut02_move_cycle_rejected(self, env):
        ...
        resp = _client(env["owner"]).patch(
            f"{_base(env)}{l1.id}/", {"parent_id": str(l2.id)}, format="json")
        assert resp.status_code == 409
...
class TestAggregation:
    def _setup_3projects(self, env):
        ...
        proj3 = Project.objects.create(name="数据迁移", identifier="DM",
                                       workspace=env["ws"], created_by=env["owner"])
        seed_project_states(proj3)
        ProjectMember.objects.create(project=proj3, member=env["owner"],
    ...
    def test_ut10_resource_matrix_matches_worklog(self, env):
        leaf, _ = self._setup_3projects(env)
        WorkLogSummary.objects.create(project=env["proj"], actor=env["manager"],
                                      week_start="2026-09-07", total_minutes=1800)
        WorkLogSummary.objects.create(project=env["proj2"], actor=env["manager"],
                                      week_start="2026-09-07", total_minutes=720)
        rows = list(psvc.PortfolioService().resource_matrix(
            env["owner"], leaf.id, "2026-09-01", "2026-09-30"))
        assert len(rows) == 2
...
class TestCrossProjectLinks:
    ...
    def test_ut07_three_paths(self, env, db):
        ...
        # ① 同工作空间跨项目：放行
        fwd, mirror = create_relation(issue_id=a.id, related_issue_id=b.id,
                                      relation_type="blocks", actor_id=env["owner"].id)
        ...
        with pytest.raises(RelationValidationError):
            create_relation(issue_id=a.id, related_issue_id=x.id,
                            relation_type="blocks", actor_id=env["owner"].id)
        ...
        draft = Project.objects.create(name="D", identifier="DR", workspace=env["ws"],
                                       status=Project.Status.DRAFT, created_by=env["manager"])
        ...
        with pytest.raises(TargetInvisibleError):
            create_relation(issue_id=a.id, related_issue_id=d.id,
                            relation_type="blocks", actor_id=member.id)
    ...
    def test_ut08_guard_project_filter(self, env):
        ...
        create_relation(issue_id=b.id, related_issue_id=a.id,
                        relation_type="blocks", actor_id=env["owner"].id)  # B blocks A（跨项目）
        ...
        create_relation(issue_id=c.id, related_issue_id=a2.id,
                        relation_type="blocks", actor_id=env["owner"].id)
    ...
    def test_ut09_cross_project_cycle(self, env):
        ...
        create_relation(issue_id=a.id, related_issue_id=b.id,
                        relation_type="blocks", actor_id=env["owner"].id)   # A(p1)→B(p2)
        create_relation(issue_id=b.id, related_issue_id=c.id,
                        relation_type="blocks", actor_id=env["owner"].id)   # B(p2)→C2(p1)
    ...
    def test_it02_dependency_graph_payload(self, env):
        ...
        c = _mk_issue(env["proj"], "C", actor=env["owner"])
        create_relation(issue_id=c.id, related_issue_id=a.id,
                        relation_type="blocks", actor_id=env["owner"].id)   # 同项目
        create_relation(issue_id=a.id, related_issue_id=b.id,
                        relation_type="blocks", actor_id=env["owner"].id)   # 跨项目
        leaf = _mk_pf(env, "组合")
...
class TestAlertsAndPermissions:
    def _milestone(self, env, days_left=5, manager=None):
        ...
        ms = PortfolioMilestone.objects.create(
            portfolio=leaf, name="临期里程碑",
            target_date=today + timezone.timedelta(days=days_left), created_by=env["owner"])
    ...
    def test_ut12_item_scope_validation(self, env):
        ...
        resp = _client(env["owner"]).post(
            f"{_base(env)}{leaf.id}/milestones/{ms.id}/items/",
            {"issue_id": str(outsider.id)}, format="json")
        assert resp.status_code == 400
    ...
    def test_ut15_manager_ancestor_chain(self, env):
        ...
        resp = _client(env["manager"]).post(
            f"{_base(env)}{leaf.id}/milestones/",
            {"name": "M", "target_date": "2026-10-31"}, format="json")
        assert resp.status_code == 201
        # ② 非 manager（另一 MEMBER）：403
        other = User.objects.create_user(email="pf-m2@rabbit.dev", password="Rabbit123!")
        WorkspaceMember.objects.create(workspace=env["ws"], member=other,
                                       role=WorkspaceRole.MEMBER, created_by=env["owner"])
        resp = _client(other).post(
            f"{_base(env)}{leaf.id}/milestones/",
            {"name": "M2", "target_date": "2026-10-31"}, format="json")
    ...
    def test_it06_permission_matrix(self, env):
        ...
        assert _client(owner).post(
            f"{_base(env)}{leaf.id}/milestones/",
            {"name": "A", "target_date": "2026-10-01"}, format="json").status_code == 201
        # 写端点：manager（祖先=自身）✓
        assert _client(manager).post(
            f"{_base(env)}{leaf.id}/milestones/",
            {"name": "B", "target_date": "2026-10-02"}, format="json").status_code == 201
        # 写端点：MEMBER → 403
        assert _client(member).post(
            f"{_base(env)}{leaf.id}/milestones/",
            {"name": "C", "target_date": "2026-10-03"}, format="json").status_code == 403
    ...
    def test_it05_mount_requires_both_sides(self, env):
        ...
        assert _client(member).post(
            f"{_base(env)}{leaf.id}/projects/",
            {"project_id": str(env["proj"].id)}, format="json").status_code == 403
        # owner 两端皆 ADMIN：✓
        assert _client(env["owner"]).post(
            f"{_base(env)}{leaf.id}/projects/",
            {"project_id": str(env["proj"].id)}, format="json").status_code == 201
