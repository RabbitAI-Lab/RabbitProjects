...
| 最后更新日期 | 2026-09-11（v2 范围裁定：12 项需求池未承接项全量纳入，新增 11 份规格编号 + INTG-004 增补章；R0 基线收口完成，开发于 `p4-future-enhancements` 分支进行。**R1（AUTH-012）全栈交付中**：hifi 原型 FROZEN、七模型+门控迁移、六规则引擎+审计扇出、17 端点、配额五强制点+治理中间件、五 UI 面+冻结横幅、30 用例 + known-debt A#1/#2 收口；ADR-0032 登记实现偏差） |
...
> **v2 范围裁定（2026-09-10 用户拍板）**：需求池对账发现的 12 项未承接需求（企业微信/钉钉通道、第三方应用市场本体、跨组织经营分析、超时自动流转、跨项目资源统一调度、归档强制合规、跨项目甘特汇总、资源负载甘特、关键路径锁定、跨项目全局看板、企业级统一视图模板、统一消息推送策略）**全部纳入 P4 开发范围**，不再视为范围排除。新增 11 份规格编号（INTG-005 / FILE-007 / RPT-006 / GANTT-004~006 / WF-007 / PROJ-005 / BOARD-006~007 / COLLAB-005，README §4 索引 #83~#93）+ INTG-004 增补「应用市场」章；每份在对应轮次第 0 阶段产出（七章结构 + 五维评审 ≥9.5 + hifi 冻结）。下表仅剩各文档**技术边界类**排除仍然有效。
...
| GraphQL 对外面 / 重应用框架 | REST + OpenAPI 3.1 保留（否决 GraphQL 双轨维护）；应用侧凭证与授权层为本文范围（~~iframe 嵌入与 UI 扩展点不做~~ **v2 已纳入**：应用市场本体含 UI 扩展点归 INTG-004 增补章，2026-09-10 裁定） | `INTG-004` §6（应用市场增补章随 R6 轮产出） |
| 安全面开放 | 认证/SSO/权限/审计/工作流管理/系统管理端点永不进入 Open API 白名单（BR-06）；implicit / password grant 不支持 | `INTG-004` BR-06 / §2.5 |
| Slack 订阅查询语言 / 内嵌会议 UI | Slack 订阅只给事件类型勾选（汲取 Jira 噪音教训，BR-04）；Zoom 只做「关联 + 回挂」不做内嵌会议界面；~~钉钉/飞书集成另案~~（**v2 已纳入**：INTG-005 企业微信/钉钉通道，2026-09-10 裁定） | `INTG-003` §6 / §1.2 |
| 目录「全量覆盖式」禁用 | 不采纳网络分区下会误禁全员的覆盖式语义，改为「连续两次同步缺席才禁用」双确认 + 干跑先行 | `AUTH-011` §6 |
| P4 全量排期 | ~~标准版 / 企业版 V1.0 发布前一律不排期，全部能力签约驱动、需求入池评估~~ 企业版 V1.0 已发布（Sprint-9）：2026-09-10 起进入开发（R0 基线已收口，分支 `p4-future-enhancements`）；商业化启动条件开发期按「模拟达线 + 签约核验检查单」口径随各规格验收输出 | 本文档 §4 排期纪律 |
...
| 11 | `INTG-005-wecom-dingtalk.md`（待编·R8 轮） | 企业微信 / 钉钉通道 |
| 12 | `FILE-007-archive-compliance.md`（待编·R10 轮） | 项目归档强制合规 |
| 13 | `RPT-006-cross-org-analytics.md`（待编·R12 轮） | 跨组织经营分析 |
| 14 | `GANTT-004-portfolio-gantt.md`（待编·R15 轮） | 跨项目甘特汇总 |
| 15 | `GANTT-005-resource-load.md`（待编·R15 轮） | 资源负载甘特 |
| 16 | `GANTT-006-cp-lock.md`（待编·R15 轮） | 关键路径锁定 |
| 17 | `BOARD-006-global-board.md`（待编·R16 轮） | 跨项目全局看板 |
| 18 | `BOARD-007-view-templates.md`（待编·R16 轮） | 企业级统一视图模板 |
| 19 | `WF-007-timeout-transition.md`（待编·R17 轮） | 超时自动流转 |
| 20 | `PROJ-005-resource-scheduling.md`（待编·R17 轮） | 跨项目资源统一调度 |
| 21 | `COLLAB-005-push-strategy.md`（待编·R18 轮） | 统一消息推送策略 |

> 序号 11~21 为 v2 范围裁定（2026-09-10）新增的 11 份规格，随对应轮次第 0 阶段产出后转入正式清单；INTG-004 的「应用市场」增补章随 R6 轮并入其文档（不另立编号）。
