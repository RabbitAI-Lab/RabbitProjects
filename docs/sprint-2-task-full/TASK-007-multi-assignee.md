...
3. **认领**：无执行人的任务，任何项目成员可一键「认领」为自己——把「指派真空」变成「举手补位」。
...
| # | 能力 | 说明 |
| --- | --- | --- |
| 1 | 多执行人 | 上限 10 人；去重保序；全部必须是本项目 active 成员 |
...
flowchart TD
    ...
    E -->|否| E1["403"]
    E -->|是| F{"集合校验：≤10 人？<br/>全部为本项目 active 成员？"}
    F -->|否| F1["400 VALIDATION_ERROR"]
    F -->|是| G["事务：diff 新旧集合"]
    G --> H["added 逐人 INSERT（assigned_by=操作者）<br/>removed 逐人软删"]
...
> 三条路径的正确性都锚定在「**同一把 Issue 行锁 + 软删/归档过滤在最外层**」这一件事上——不存在为认领单独设计的锁或检查，并发语义因此可被一条规则说清（评审与排障成本最低）。
...
| BR-02 | 每个执行人必须是本项目 active `ProjectMember`（VIEWER 不可被指派——无写权限者不该背任务） | Serializer | `400 DOES_NOT_EXIST` |
| BR-03 | 集合去重**保序**：按请求顺序落库，展示顺序 = 加入顺序（头像堆叠稳定） | Service | — |
...
| BR-11 | 任务软删 → IssueAssignee 级联软删（`TASK-004` delete_subtree 已含）；恢复则恢复 | 级联服务 | — |
| BR-12 | 移除成员出项目（`TEAM-002`/`PROJ-002`）→ 其在该项目的全部 IssueAssignee 同事务清理，任务变为「未指派」 | 级联钩子 | — |
...
| 含 VIEWER 成员 | 400 | `VALIDATION_ERROR` | `DOES_NOT_EXIST` | 候选过滤 VIEWER，提示「查看者不能被指派」 |
...
| 移出项目成员的级联 | 同事务清理其全部指派 | 任务入「未指派」待分派视图 |
...
| 元素 | 规格 |
| --- | --- |
| 成员列表 | active 项目成员，CONTRIBUTOR+ 可勾选；VIEWER 灰显标注；自己带「（我）」 |
| 已选 chips | 顺序即落库顺序（BR-03）；`×` 移除 |
| 计数 | `n/10`，达 10 后未选项禁用 |
...
| 2 | `POST` | `…/issues/{issue_id}/assignees/claim/` | 认领（空集合时把自己加入） | `PROJ_CONTRIBUTOR`(15)+ | `200` |
...
| 5 | `GET` | `…/issues/?assignee_id=<uuid>,…` | 多值筛选（`TASK-003` 已有，逗号 OR） | `PROJ_VIEWER`(5)+ | `200` |
...
```json
{
  ...
  "data": {
    ...
    "changes": {
      ...
      "removed": [{ "id": "9a1b2c3d-4e5f-6071-8293-a4b5c6d7e8f9", "display_name": "赵六" }]
    }
  },
...
> `changes` 回传让前端无需本地 diff 即可做通知预览与 Activity 展示。
...
#### 4.2.5 `?assignee_id=null` — 「未指派」筛选语义

`TASK-003` 筛选白名单中 `assignee_id` 追加 `null` 语法糖（等价 `assignee_ids__isnull=true`，即 `TASK-001` §4.2.2 既有约定）：

```http
GET …/issues/?assignee_id=null&state_group=unstarted,started&order_by=target_date HTTP/1.1
```

| 语义细节 | 说明 |
| --- | --- |
| `null` 的判定 | 任务不存在任何 `deleted_at IS NULL` 的 IssueAssignee 行（软删行不算——被移出即未指派） |
| 与 BR-06 呼应 | 清空集合的任务由此筛选兜住，构成「待分派池」 |
| 与 `me` 组合禁止 | `assignee_id=me,null` 无意义，FilterSet 忽略 `null` 之外的值并回退等值语义（`meta.warning` 提示） |
...
```python
# apps/api/plane/db/services/issue_assignee.py
import uuid
...
def sync_assignees(*, issue_id: uuid.UUID, new_ids: list[uuid.UUID],
    ...
    reactivated = [a for a in removed if a.assignee_id in new_ids]   # 见下方「复活」说明

    for a in removed:                                          # 软删（保留历史行供审计）
        a.deleted_at = timezone.now()
        a.save(update_fields=["deleted_at"])
    # 唯一约束不带 deleted 条件 → 重新加入采用「复活旧行」策略，不插入新行
    for a in reactivated:
        a.deleted_at = None
        a.assigned_by_id = actor_id
        a.save(update_fields=["deleted_at", "assigned_by", "updated_at"])
    fresh_ids = [i for i in added_ids if i not in {a.assignee_id for a in reactivated}]
    IssueAssignee.objects.bulk_create([                        # 新增逐行（保序 = created_at 序）
        IssueAssignee(issue_id=issue_id, assignee_id=uid, assigned_by_id=actor_id)  # BR-04
    ...
    changes = {
        ...
        "removed": [{"id": a.assignee_id, "display_name": a.assignee.display_name}
                    for a in removed if a.assignee_id not in {r.assignee_id for r in reactivated}],
    }
...
def claim(*, issue_id: uuid.UUID, actor_id: uuid.UUID) -> dict:
    ...
    with transaction.atomic():
        ...
        has_any = IssueAssignee.objects.filter(
            issue_id=issue_id, deleted_at__isnull=True).exists()
        if has_any:
...
> **为什么「移除」是软删而非物理 DELETE**：`assigned_by` + `created_at` 构成「谁在何时把任务分给了谁」的审计链；物理删除会抹掉「曾被指派」的历史（转交溯源、P3 工时分摊复核都需要）。`uniq_issue_assignee` 不带 `deleted_at` 条件——同一个人被移除后**重新加入**会撞唯一约束，因此用「复活旧行」策略（UT-09 锚定）。
...
def dispatch_assignment_events(self, issue_id: str, actor_id: str,
    ...
    - added 中每人一条「xx 把任务指派给你」（操作者本人跳过 BR-09）
    - removed 中每人一条「xx 将你移出任务」（含 comment 引用）
...
```python
# 并入 PROJ-002/TEAM-002 的成员移除服务（BR-12）：
...
#   → 受影响任务进入「未指派」；on_commit 批量通知项目负责人（P3 自动化规则接管）
```

#### 4.3.4 成员资格校验与自退服务（`sync_assignees` 的支撑件）

```python
# apps/api/plane/db/services/issue_assignee.py（续）
...
def _assert_assignable(project_id: uuid.UUID, user_ids: list[uuid.UUID]) -> None:
    ...
    if invalid:
        ...
        raise ValidationError(
            {"assignee_ids": f"{names} 不是本项目成员或为查看者，不能被指派"})
...
def remove_self(*, issue_id: uuid.UUID, user_id: uuid.UUID) -> None:
    ...
    remaining = list(IssueAssignee.objects.filter(
        issue_id=issue_id, deleted_at__isnull=True
    ).exclude(assignee_id=user_id).values_list("assignee_id", flat=True).order_by("created_at"))
    sync_assignees(issue_id=issue_id, new_ids=remaining,
                   actor_id=user_id)                    # 操作者=退出者；无 comment
```

> `remove_self` 复用 `sync_assignees` 的收益：软删/复活语义、`changes` 通知、Activity 逐人留痕、归档拦截（BR-13）四件事零重复实现——「唯一写入口」约定（§1.2）的价值在第三个调用方出现时开始兑现。
...
export const AssigneePicker = observer(function AssigneePicker({
  ...
  issueId: string;
  initialIds: string[];                 // 顺序即当前落库顺序（BR-03 基线）
  ...
  /** 候选：过滤 VIEWER（服务端 BR-02 的前端预判）；搜索按显示名/邮箱前缀 */
...
**实现要点**：候选过滤 VIEWER 只是预判——服务端 `_assert_assignable`（§4.3.4）是权威；`selected` 数组即唯一事实（勾选与 chips 双向操作同一数组，顺序天然一致）；提交结果用服务端 `assignee_ids` 整体替换（并发下不漂移）。
...
| UT-06 | VIEWER 指派 | 项目 VIEWER | 400 | 安全 |
| UT-07 | 认领空任务 | 集合空 | 200，assigned_by=自己 | 正常 |
| UT-08 | 认领非空 | 集合 1 人 | 409 STATE | 异常 |
| UT-09 | 重加同人 | 删后重加 | 复活旧行（不撞唯一约束），assigned_by 刷新 | 边界 |
...
| UT-15 | 级联清理 | 成员移出项目 | 其该项目全部指派软删 | 正常 |
| UT-16 | 自退收敛 | 3 人中 1 人自退 | 剩余 2 人集合不变（remove_self 经 sync）；最后一人自退 = 清空 | 正常 |
| UT-17 | 候选校验单查询 | 10 候选含 1 个非法 | `assertNumQueries(1)`（IN 批量，§4.3.4） | 性能 |
...
| IT-06 | 任务软删级联 | 有 3 执行人 | 删除任务 | 指派软删；恢复重现 |
...
| IT-09 | 未指派筛选 | 清空 2 个任务集合 | `?assignee_id=null` | 恰返回 2 条；软删行不算（§4.2.5） |
...
- 本系统 P2 的留痕链（`assigned_by` + 逐人 Activity + 软删保留历史）已等价覆盖审计诉求；负载视图归 `RPT-002`（成员任务量）。
...
3. **软删保留指派历史**：`uniq` 约束不带 deleted 条件 + 「复活旧行」策略，兼得审计链完整与约束简洁——比「带条件的部分唯一索引 + 双行」方案少一半心智负担。
4. **认领是 409 不是静默合并**：「补位」与「加入」是两个动作，混用会让多人协作边界（谁在负责）重新模糊；要加入走编辑（PUT）。
5. **级联清理前置到成员移除**：BR-12 把「移出成员 → 清指派」做在同一个事务里，杜绝「离职成员仍挂在我任务上」的经典脏数据。
...
| 后端 | `sync_assignees`（唯一写入口）/ `claim` / `remove_self` / `_assert_assignable` 服务；`PUT …/assignees/`、`POST …/assignees/claim/`、`DELETE …/assignees/{user_id}/` 三端点；PATCH `assignee_ids` 多人开放（§4.2.4）；`?assignee_id=null` 筛选（§4.2.5）；成员移除级联钩子 |
...
| 测试 | UT-01~17、IT-01~09、E2E-01~05 |
...
4. 提交 11 人集合被 409 拦截且前端预拦截（第 11 人不可勾选）；VIEWER 不出现在候选且直连被 400。
5. 被移出项目的成员其全部任务指派同事务清空，任务转为未指派；两人视角的「我的待办」各自正确（无串任务）。
6. 删除含 3 执行人的任务后恢复：指派关系完整重现（软删不丢审计链）。
