...
@pytest.fixture()
def env(db):
    owner = User.objects.create_user(email="w2-owner@rabbit.dev", password="Rabbit123!",
                                     display_name="张三")
    member = User.objects.create_user(email="w2-member@rabbit.dev", password="Rabbit123!",
                                      display_name="李成员")
    ws = Workspace.objects.create(name="W", slug=f"w-w2-{owner.id.hex[:8]}",
...
def _mk_endpoint(env, *, events=None, url="https://hooks.example.com/rp",
    ...
    return WebhookEndpoint.objects.create(
        project=env["proj"], workspace=env["ws"], url=url,
        events=events or ["issue.updated", "comment.created"],
        secret_encrypted=encrypt_secret(new_endpoint_secret()),
        is_active=status, consecutive_failures=failures, created_by=env["owner"])


def _base(env):
    return (f"/api/v1/workspaces/{env['ws'].slug}/"
            f"projects/{env['proj'].id}/webhooks/")
...
def test_endpoint_crud(env):
    url = _base(env)
    assert _client(env["member"]).post(url, format="json", data={
        "url": "https://x.example.com", "events": ["issue.updated"]}
    ).status_code == 403   # integration.config = PROJ_ADMIN
    r = _client(env["owner"]).post(url, format="json", data={
        "url": "https://hooks.example.com/rp", "events": ["issue.updated"]})
    assert r.status_code == 201
    d = r.json()["data"]
    assert d["secret_shown_once"] and d["is_active"] == "active"
    eid = d["id"]
    # 非法事件 / 空 events / 重复 URL / ping 免勾选
    assert _client(env["owner"]).post(url, format="json", data={
        "url": "https://a.example.com", "events": ["bogus.event"]}).status_code == 400
    assert _client(env["owner"]).post(url, format="json", data={
        "url": "https://a.example.com", "events": []}).status_code == 400
    assert _client(env["owner"]).post(url, format="json", data={
        "url": "https://hooks.example.com/rp",
        "events": ["issue.updated"]}).status_code == 409
    assert _client(env["owner"]).post(url, format="json", data={
        "url": "https://b.example.com", "events": ["webhook.ping"]}).status_code == 400
    # 上限 20
    for i in range(19):
        WebhookEndpoint.objects.create(
            project=env["proj"], workspace=env["ws"], url=f"https://x{i}.example.com",
            events=["issue.updated"], secret_encrypted="x", created_by=env["owner"])
    assert _client(env["owner"]).post(url, format="json", data={
        "url": "https://cap.example.com", "events": ["issue.updated"]}
    ).status_code == 409
    # 停用 / 启用（连败清零）
...
def test_dispatch_fanout_and_idempotent_anchor(env, monkeypatch):
    ...
    _mk_endpoint(env, events=["comment.created"],
                 url="https://hooks.example.com/c")              # 不匹配
    wo.dispatch_events("issue.updated",
                       {"event_id": "00000000-0000-0000-0000-000000000001",
                        "data": {"x": 1}}, project_id=env["proj"].id)
    assert len(sent) == 1
    ...
    wo.dispatch_events("webhook.ping", {"data": {"ping": True}},
...
def test_deliver_success_and_retry_to_dead(env, monkeypatch):
    ...
    row = WebhookDelivery.objects.create(
        endpoint=e1, event="issue.updated",
    ...
    row2 = WebhookDelivery.objects.create(
        ...
        payload={"event": "issue.updated"})
    rescheduled: list = []
    monkeypatch.setattr(wo.deliver_webhook, "apply_async",
                        lambda a, countdown=None: rescheduled.append(countdown))
...
def test_deliver_auto_disable_and_notify(env, monkeypatch, django_capture_on_commit_callbacks):
    ...
    monkeypatch.setattr(
        "plane.bgtasks.notifications.send_workspace_notification.delay",
...
def test_project_events_hook(env, monkeypatch, django_capture_on_commit_callbacks):
    ...
    with django_capture_on_commit_callbacks(execute=True):
        ProjectLifecycleService().transition(
            env["proj"], to_status="archived", actor=env["owner"])
