...
@pytest.fixture()
def env(db):
    admin = User.objects.create_user(email="rg-admin@rabbit.dev",
                                     password="Rabbit123!", display_name="发布管")
    SystemAdmin.objects.create(user=admin)
    outsider = User.objects.create_user(email="rg-out@rabbit.dev",
                                        password="Rabbit123!")
...
def _mk_gate(env, version="1.0.0", sha="a" * 40):
    return svc.create_release_gate(version=version, commit_sha=sha,
                                   actor=env["admin"])
...
def test_create_idempotent(env):
    r1 = _c(env["admin"]).post(BASE + "/create/",
                               {"version": "1.0.0", "commit_sha": "b" * 40},
                               format="json")
    assert r1.status_code == 201 and r1.headers["Location"].endswith(
        r1.json()["data"]["id"] + "/")
    r2 = _c(env["admin"]).post(BASE + "/create/",
                               {"version": "1.0.0", "commit_sha": "b" * 40},
                               format="json")
    assert r2.json()["data"]["id"] == r1.json()["data"]["id"]   # 幂等
    assert _c(env["admin"]).post(BASE + "/create/",
                               {"version": "", "commit_sha": ""},
...
def test_gate_update_validation_and_snapshot(env):
    ...
    assert _c(env["admin"]).post(url, {"gate": "nope", "status": "passed"},
                                 format="json").status_code == 400
    assert _c(env["admin"]).post(url, {"gate": "gate_perf", "status": "green"},
                                 format="json").status_code == 400
    r = _c(env["admin"]).post(url, {"gate": "gate_perf", "status": "passed",
                                    "artifacts": {"rounds": 2}}, format="json")
    assert r.status_code == 200
    gate.refresh_from_db()
    assert gate.gate_perf == "passed" and gate.artifacts["gate_perf"]["rounds"] == 2
    assert ReleaseGateEvent.objects.filter(
        gate=gate, event_type="gate_update").count() == 1
...
def test_sign_revoke_resign_flow(env):
    ...
    r = _c(env["admin"]).post(url, {"signed": True, "note": "两轮已归档"},
                              format="json")
    assert r.status_code == 200
    # 反签（signed=false → revoked 事件；原记录保留）
    r = _c(env["admin"]).post(url, {"signed": False, "note": "报告数据存疑"},
                              format="json")
    assert r.status_code == 200
    types = list(ReleaseGateEvent.objects.filter(gate=gate)
    ...
    # 非法 key 400
    assert _c(env["admin"]).post(
        f"{BASE}/{gate.id}/checklist/no_such/sign/", {"signed": True},
        format="json").status_code == 400
...
def test_verdict_guard_blocked_by_gate(env):
    ...
    gate.save()
    r = _c(env["admin"]).post(f"{BASE}/{gate.id}/verdict/",
    ...
    # 打回无条件允许
    assert _c(env["admin"]).post(f"{BASE}/{gate.id}/verdict/",
                                 {"verdict": "rejected"},
...
def test_verdict_approved_when_all_green(env):
    ...
    assert {e["event_type"] for e in detail["events"]} >= {
        "created", "gate_update", "checklist_sign", "verdict"}
