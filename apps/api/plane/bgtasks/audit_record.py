...
def write_chained_row(payload: dict):
    ...
    with transaction.atomic():
        ...
        prev = (
            AuditLog.objects.filter(workspace_id=payload["workspace_id"])
            .order_by("-created_at", "-id")
            .values_list("hash", flat=True)
            .first()
        ) or "0" * 64
        row_hash = hashlib.sha256(f"{prev}|{_canonical(payload)}".encode()).hexdigest()
...
@shared_task(bind=True, max_retries=5, acks_late=True, acks_on_failure_or_timeout=False)
def audit_record(self, payload: dict) -> None:
    ...
    try:
        ...
        logger.error("audit.unregistered_event %s.%s", payload["category"], payload["action"])
        raise
    ...
    try:
        ...
        _fanout_risk(payload)  # ⑥ risk 订阅者扇出（AUTH-012 P4 R1；异常不影响审计链）
...
def _fanout_risk(payload: dict) -> None:
    """AUTH-012（P4 R1）§4.3：审计管道事件扇出 risk 订阅者（不引入第二条
    事件流）。门控在 risk_ingest 任务体内（BR-11）；此处仅做投递异常隔离。"""
    try:
        from plane.governance.tasks import risk_ingest

        risk_ingest.delay(payload)
    except Exception as exc:  # noqa: BLE001 —— broker 不可达不阻塞审计链
        logger.warning("audit.risk_fanout_failed err=%s", exc)
...
def create_next_month_partition() -> str:
    ...
    now = timezone.now()
    nxt = (now.replace(day=1, hour=0, minute=0, second=0, microsecond=0) + _dt.timedelta(days=32)).replace(day=1)
    ...
    with connection.cursor() as cur:
        ...
        if cur.fetchone()[0]:
            return name
        cur.execute(
            f"CREATE TABLE {name} PARTITION OF audit_log FOR VALUES FROM ('{nxt:%Y-%m-%d}') TO ('{end:%Y-%m-%d}')"
        )
    return name
...
def drop_partitions_older_than(days: int = 180) -> list[str]:
    ...
    with connection.cursor() as cur:
        cur.execute("SELECT tablename FROM pg_tables WHERE tablename ~ '^audit_log_p[0-9]{4}_[0-9]{2}$'")
        dropped = []
        for (name,) in cur.fetchall():
            y, m = name.rsplit("_p", 1)[1].split("_")
            part_end = _dt.datetime(int(y), int(m), 1, tzinfo=_dt.UTC) + _dt.timedelta(days=32)
...
def verify_hash_chain(*, sample_rows: int = 200) -> dict:
    ...
    broken: list[str] = []
    checked = 0
    for row in AuditLog.objects.order_by("-created_at", "-id").iterator(chunk_size=200):
        checked += 1
        ...
        expected = hashlib.sha256(f"{prev}|{_canonical(rebuilt)}".encode()).hexdigest()
