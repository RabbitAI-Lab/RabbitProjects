...
| 修订 3（2026-09-02 晚，测试脚本规范）| 写 e2e / 接口 / 静态检查的 7 条强制规则：① API 真相源唯一——后端契约事实源是 `tests/jmeter/sprint-0-flow.py`，`tests/e2e/no-console-errors.ts` 的 `API_TRUTH` 镜像同一份契约（status + 字段名 + 错误码常量），所有断言 import 拒绝各自硬编码；② Playwright spec 必装 console guard（`attachConsoleGuard` + `expect.soft(errors).toEqual([])`）；③ 跨 test 状态边界（每个 `test.describe` 自带 beforeEach/afterEach 显式 `signOut/clearCookies/重置 store`，禁止依赖 Worker 复用 page）；④ Playwright ≥ 1.62 + reporter=line（1.54/1.56 的 list reporter 在 `expect.soft` 失败时 NPE `base.js:320 undefined.startsWith`）；⑤ 性能压测 vs 端到端分清（`.py` = CI gate 单线程，`.jmx` = 性能压测多线程）；⑥ 加/移 `SerializerMethodField` 必加 `Meta.fields` 元组；⑦ dropdown 全局点击监听禁 `document.click`，改 `mousedown` 阶段 + `target.closest('[data-sb-scope="..."]')`。详细规则与示例：`docs/sprint-0-poc/lessons-learned.md` §"测试脚本规范" + `CLAUDE.md` 工作流约定。 |
