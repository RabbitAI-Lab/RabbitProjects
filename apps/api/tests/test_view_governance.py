...
@pytest.fixture()
def env(db):
    owner = User.objects.create_user(email="vg-owner@rabbit.dev", password="Rabbit123!", display_name="主")
    member = User.objects.create_user(email="vg-member@rabbit.dev", password="Rabbit123!", display_name="员")
    ws = Workspace.objects.create(name="V", slug=f"w-vg-{owner.id.hex[:8]}", owner=owner, created_by=owner)
    ...
    proj = Project.objects.create(name="P", identifier="VGP", workspace=ws, created_by=owner)
    seed_project_states(proj)
    ProjectMember.objects.create(project=proj, member=owner, role=ProjectRole.ADMIN, created_by=owner)
    ProjectMember.objects.create(project=proj, member=member, role=ProjectRole.VIEWER, created_by=owner)
...
def _mk_view(env, name="我的视图", owner=None, **kw) -> IssueView:
    ...
    return IssueView.objects.create(
        workspace=env["ws"],
        project=env["proj"],
        owner=owner or env["member"],
        name=name,
        created_by=owner or env["member"],
        **kw,
    )
...
class TestGovernance:
    def test_share_then_lock_then_default(self, env):
        ...
        # 锁定 + 设默认（admin）
        vg.lock_view(actor=env["owner"], view=v, is_locked=True, is_project_default=True)
        v.refresh_from_db()
    ...
    def test_br02_lock_interception_precedes_permissions(self, env):
        ...
        vg.lock_view(actor=env["owner"], view=v, is_locked=True, is_project_default=None)
        # owner 本人（也是 board.lock 持有者）改锁定视图 → 仍 409 RESOURCE_LOCKED
        r = _client(env["member"]).patch(f"{_base(env)}{v.id}/", {"name": "改名"}, format="json")
    ...
    def test_br03_locked_delete_rejected(self, env):
        ...
        assert r.status_code == 409
        # 解锁后可删
        vg.lock_view(actor=env["owner"], view=v, is_locked=False, is_project_default=False)
        assert _client(env["owner"]).delete(f"{_base(env)}{v.id}/").status_code == 204
    ...
    def test_br04_default_atomic_replacement(self, env):
        ...
        assert IssueView.objects.filter(project=env["proj"], is_project_default=True).count() == 1

    def test_br05_default_requires_locked(self, env):
        v = _mk_view(env)
        with pytest.raises(AppException) as ei:
            vg.lock_view(actor=env["owner"], view=v, is_locked=False, is_project_default=True)
    ...
    def test_br06_duplicate_personal_fork(self, env):
        v = _mk_view(
            env,
            "复杂视图",
            filters={"op": "AND", "conditions": [{"field": "priority", "operator": "eq", "value": "urgent"}]},
        )
        vg.share_view(actor=env["member"], view=v, access="shared")
    ...
    def test_br15_unlock_two_step(self, env):
        ...
        vg.lock_view(actor=env["owner"], view=v, is_locked=True, is_project_default=True)
        with pytest.raises(AppException) as ei:
            vg.lock_view(actor=env["owner"], view=v, is_locked=False, is_project_default=None)
        assert ei.value.error_code == "RESOURCE_STATE_INVALID"
        # 两步：先取消默认 → 再解锁
        vg.lock_view(actor=env["owner"], view=v, is_locked=True, is_project_default=False)
        vg.lock_view(actor=env["owner"], view=v, is_locked=False, is_project_default=False)
    ...
    def test_new_member_auto_subscribe(self, env):
        ...
        vg.lock_view(actor=env["owner"], view=v, is_locked=True, is_project_default=True)
        # 新成员加入 → 自动订阅
        newbie = User.objects.create_user(email="vg-new@rabbit.dev", password="x", display_name="新")
        WorkspaceMember.objects.create(
            workspace=env["ws"], member=newbie, role=WorkspaceRole.MEMBER, created_by=env["owner"]
        )
        ...
        with TestCase.captureOnCommitCallbacks(execute=True):
            ProjectMemberService().add_members(
                project=env["proj"], actor=env["owner"], member_ids=[str(newbie.id)], role=ProjectRole.VIEWER
            )
        assert UserViewPreference.objects.filter(user=newbie, view=v, pinned=True, deleted_at__isnull=True).exists()

    def test_remove_member_cascades_preferences(self, env):
        v = _mk_view(env)
        vg.share_view(actor=env["member"], view=v, access="shared")
        vg.pin_view(actor=env["member"], view=v)
        pm = ProjectMember.objects.get(project=env["proj"], member=env["member"])
        from plane.db.services.project_member import ProjectMemberService

        ProjectMemberService().remove_member(project=env["proj"], member=pm, actor=env["owner"])
        assert not UserViewPreference.objects.filter(user=env["member"], deleted_at__isnull=True).exists()
...
class TestEndpoints:
    def test_lock_endpoint_permissions(self, env):
        ...
        r = _client(env["member"]).post(
            f"{_base(env)}{v.id}/lock/", {"is_locked": True, "is_project_default": False}, format="json"
        )
    ...
    def test_duplicate_and_pin_endpoints(self, env):
        ...
        prefs = _client(env["owner"]).get(
            f"/api/v1/workspaces/{env['ws'].slug}/projects/{env['proj'].id}/views/preferences/"
        )
    ...
    def test_lock_endpoint_br15_response(self, env):
        ...
        _client(env["owner"]).post(
            f"{_base(env)}{v.id}/lock/", {"is_locked": True, "is_project_default": True}, format="json"
        )
        r = _client(env["owner"]).post(f"{_base(env)}{v.id}/lock/", {"is_locked": False}, format="json")
        assert r.status_code == 409
...
class TestApplicationSurface:
    ...
    def test_shared_view_visible_to_all_members(self, env):
        ...
        v = _mk_view(
            env,
            "共享高优",
            filters={"op": "AND", "conditions": [{"field": "name", "operator": "contains", "value": "任务"}]},
        )
        # 未共享：他人用 view_id → 404（P2 行为保持）
        r = _client(env["owner"]).get(
            f"/api/v1/workspaces/{env['ws'].slug}/projects/{env['proj'].id}/issues/", {"view_id": str(v.id)}
        )
    ...
    def test_zero_diff_personal_views_unchanged(self, env):
        ...
        r2 = _client(env["member"]).patch(f"{_base(env)}{v.id}/", {"name": "改名"}, format="json")
        assert r2.status_code == 200
    ...
    def test_matrix_response_and_counts(self, env):
        ...
        r = _client(env["owner"]).get(
            f"/api/v1/workspaces/{env['ws'].slug}/projects/{env['proj'].id}/issues/",
