...
### 1.1 设计基线（2026-09-02 冻结）

依产品负责人裁决「**有界面交互的迭代，开发前必须先有高保真可交互设计稿并评审冻结**」（已回写需求文档 §8.3 第 8 条修订），本迭代开发启动前已产出并冻结高保真原型：

- **冻结产物**：[`docs/design/sprint-0-hifi-prototype.html`](../design/sprint-0-hifi-prototype.html)（单文件、浏览器直接打开；六界面一条动线：登录/注册 → 团队工作台 → 项目列表 → 任务列表/抽屉 → 三列看板拖拽；设计 token 与 `INFRA-001` §4.9 一致；含路由守卫、会话过期、账号禁用、409 冲突、拖拽失败回滚等演示模拟）
- **评审记录**：独立 subagent 对照 10 份文档 §3 与本文档 §2/§6 评审，覆盖率约 90%；3 个缺口（抽屉 `?peekIssue` URL 同步、列表编号复制 + 卡片键盘可达、会话过期 `next` 回跳）与 2 处偏差（404 空态保留导航、`sort_order` 65535 口径）已全部修复并浏览器实测通过
- **验收口径**：前端实现的视觉与交互以冻结稿为基准；「不追求 UI 精美度」边界不变
- **排期影响**：设计前置消耗 §8 排期 10 个工作日中约 1 天缓冲（原 2 天），剩余缓冲 1 天
...
4. **UI parity**（ADR-0010 后追加）：全部 UI 表面（页面/弹窗）已入 test-cases.md 附录 C 清单，`tests/e2e/parity.spec.ts` 字段级断言全绿。
