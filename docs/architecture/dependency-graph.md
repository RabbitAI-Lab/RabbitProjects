...
> **回改记录（2026-09-06 · 架构文档专项回改批次）**：按各 sprint 评审轮登记的「（dependency-graph 待回改）」项统一回改，编号-标题口径全面对齐 [`docs/README.md`](../README.md) §4 官方索引——① **AUTH-009/010**：`AUTH-009`＝SSO 单点登录（SAML 2.0 / OIDC）、`AUTH-010`＝全站操作审计日志（原 dg 字面互换）；② **WF-002~006**：`WF-002`＝审批流（会签 / 或签 / 逐级 / 驳回 / 撤回）、`WF-003`＝自动化规则引擎、`WF-004`＝流转守卫与字段锁定、`WF-005`＝工作流模板库与全局下发、`WF-006`＝审批留痕与合规溯源（原 dg 编号-标题整体错位一档，且无 WF-006 行；`WF-001` 标题同步为 README 实文「可视化工作流画布与引擎数据模型」）；③ **INTG-003/004**：`INTG-003`＝Slack / Zoom 全量集成、`INTG-004`＝完整 Open API 与应用接入（原 dg 字面互换）。§1.3 台账、§4 依赖表行标题与 §4.15 mermaid 节点同步修正；依赖边随之逐条改挂正确编号（审计/SSO 消费边、WF 守卫→审批→留痕链、Slack/Zoom 与 OpenAPI 行上游按修复后 §1 实文重建），并落实已登记缺边/倒挂边：`GANTT-003` 上游补 `TASK-005`、`TASK-012 ← AUTH-008` 倒挂边撤销、`TASK-015 ← WF-001` 边撤销、`RPT-005`/`AI-001` 行按其修复后 §1 实文核对补齐。登记来源：sprint-7 / sprint-8 / sprint-9 / sprint-future-p4 四轮评审登记（各 sprint 文档内的消解注为历史登记，保留不动）。

> **回改记录（2026-09-06 · 架构文档专项回改批次 · 第二期）**：处理 TASK 族遗留编号-标题错位（登记来源：sprint-2 评审 [`sprint-2-task-full/sprint-overview.md`](../sprint-2-task-full/sprint-overview.md) §3 编号口径注记，文档质量评审状态 sprint-2 overview ISSUE ④ + dg 批次复核发现）——④ **TASK-009/010/011**：`TASK-009`＝任务复制 / 归档 / 恢复、`TASK-010`＝全操作留痕审计日志（均 Sprint 2）、`TASK-011`＝全字段 AND/OR 组合筛选器与视图保存（Sprint 3）；原 dg 三行标题整体错位一档（009 挂筛选器、010 挂复制归档、011 挂「任务类型字段模板与需求池」——后者无对应文档，需求池实为 `TASK-011` 内置种子视图）。§1.3 台账、§4.5 三行、§4.15 mermaid TASK 簇、§6/§7 引用同步修正；受牵连边按各文档 §1 实文逐条改挂：`WF-001` 上游 `TASK-001`/`TASK-011` → `TASK-005`/`TASK-010`（按 WF-001 §1，任务类型挂载表述撤销）、`TASK-012` 上游补 `TASK-011`（FilterCompiler）、`TASK-009 → TASK-011` 边撤销（TASK-011 §1 无此前置）、`TASK-009` 补 `TASK-004` 子树深拷贝边、`TASK-011` 补 `BOARD-003`/`BOARD-002` 边；`BOARD-004` 行编号不变、理由按其 §1 改为归档服务复用 / epoch 约定；`RPT-002` 行 `TASK-009` 边保留、理由改挂归档统计口径；「操作日志作为统一事件源」行经复核确认归属 `TASK-010`（审计日志）无需移挂，§7.4 需求池行确认归属 `TASK-011` 内置种子视图亦不动。
...
| `AUTH-009` | SSO 单点登录（SAML 2.0 / OIDC） | P3 | Sprint 8 |
| `AUTH-010` | 全站操作审计日志 | P3 | Sprint 8 |
...
| `TASK-009` | 任务复制 / 归档 / 恢复 | P2 | Sprint 2 |
| `TASK-010` | 全操作留痕审计日志 | P2 | Sprint 2 |
| `TASK-011` | 全字段 AND/OR 组合筛选器与视图保存 | P2 | Sprint 3 |
...
| 文档 ID | 文档名称 | 优先级 | 迭代 |
| --- | --- | :-: | :-: |
| `WF-001` | 可视化工作流画布与引擎数据模型 | P3 | Sprint 7 |
| `WF-002` | 审批流（会签 / 或签 / 逐级 / 驳回 / 撤回） | P3 | Sprint 7 |
| `WF-003` | 自动化规则引擎 | P3 | Sprint 7 |
| `WF-004` | 流转守卫与字段锁定 | P3 | Sprint 7 |
| `WF-005` | 工作流模板库与全局下发 | P3 | Sprint 7 |
| `WF-006` | 审批留痕与合规溯源 | P3 | Sprint 7 |
...
| `INTG-003` | Slack / Zoom 全量集成 | P4 | 远期 |
| `INTG-004` | 完整 Open API 与应用接入 | P4 | 远期 |
...
| `INFRA-006`（高可用集群与私有化部署） | `INFRA-005` `AUTH-012` | 高可用集群是生产部署基线的集群化扩展，多租户拓扑与租户档位约束 SaaS 集群与私有化形态 |
...
| `AUTH-008`（自定义角色组与细粒度资源权限） | `AUTH-007` `WF-004` | 自定义角色需完整权限 Key 集合与组织结构（字段级权限面由 WF-004 守卫与字段锁定协议提供） |
| `AUTH-009`（SSO 单点登录（SAML 2.0 / OIDC）） | `AUTH-007` `AUTH-008` | JIT 部门映射落点为组织架构，SSO 用户落入既有组织与角色体系 |
| `AUTH-010`（全站操作审计日志） | `AUTH-008` | 审计覆盖细粒度权限、组织与角色变更事件 |
| `AUTH-011`（LDAP / SCIM 账号同步） | `AUTH-007` `AUTH-009` | 目录同步目标为组织架构，并与企业认证体系（SSO）衔接 |
| `AUTH-012`（多租户隔离与风控告警溯源） | `AUTH-006` `AUTH-010` | 多租户隔离复用行级过滤，风控告警溯源消费全站审计日志 |
...
| `TASK-009`（任务复制 / 归档 / 恢复） | `TASK-001` `TASK-002` `TASK-003` `TASK-004` `TASK-005` `TASK-006` `TASK-008` | 复制复用 Issue 创建服务与子树深拷贝，副本 `duplicates` 关联经 `IssueLink` 成对写入，`archived` 参数落列表筛选白名单，动态字段与估算随行拷贝 |
| `TASK-010`（全操作留痕审计日志） | `INFRA-003` `INFRA-002` `INFRA-004` `TASK-001` `TASK-002`~`TASK-009` | 各写操作事件源统一接入留痕管道（Celery 异步投递 + 死信兜底），`issue_activities` 表与 broker / 统一信封为基础设施前提 |
| `TASK-011`（全字段 AND/OR 组合筛选器与视图保存） | `TASK-003` `TASK-008` `BOARD-003` `BOARD-002` `INFRA-004` | 筛选 DSL 扩展参数筛选与 Schema 等值子集为全量编译器，读取内置与动态字段元数据并持久化视图 filters（`IssueView` 表以 `BOARD-003` 为唯一事实源），分组契约沿 `BOARD-002` |
| `TASK-012`（高级自定义字段与字段权限） | `TASK-008` `TASK-011` | 高级字段扩展基础字段类型，并复用 `TASK-011` FilterCompiler（进 Schema 即自动可筛选）；字段权限先落固定角色码，自定义角色矩阵待 Sprint 8 `AUTH-008` 落地后自动生效（零改表，跨迭代倒挂边撤销） |
| `TASK-013`（团队工时统计与工时管控） | `TASK-006` `RPT-002` | 复用工时数据与报表聚合口径 |
| `TASK-014`（公式 / 级联 / 跨项目关联字段） | `TASK-012` `PROJ-004` | 公式与级联扩展高级字段，跨项目关联依赖项目集能力 |
| `TASK-015`（任务基线与版本对比） | `TASK-010` | 基线快照消费 Activity 变更日志；不消费工作流版本/状态机定义（快照仅取 `state_group` 状态语义组，`WF-001` 边撤销） |

### 4.6 M11-WF｜企业工作流与审批（6）

| 下游功能 | 上游依赖 | 依赖说明 |
| --- | --- | --- |
| `WF-001`（可视化工作流画布与引擎数据模型） | `TASK-005` `TASK-010` | 工作流替换固定状态流转（Sprint 6 状态模型冻结基线上扩表），完成守卫复用 `TASK-005` 依赖图 BLOCKER 钩子，流转留痕经 `TASK-010` Activity 管道 |
| `WF-002`（审批流（会签 / 或签 / 逐级 / 驳回 / 撤回）） | `WF-001` `WF-004` `COLLAB-001` | 审批节点复用流转守卫规则，待办与提醒走通知通道 |
| `WF-003`（自动化规则引擎） | `WF-001` `TASK-010` `TASK-007` `COLLAB-001` | 触发器订阅操作日志，动作复用指派与通知能力 |
| `WF-004`（流转守卫与字段锁定） | `WF-001` `TASK-005` `TASK-006` `AUTH-005` | 守卫校验消费依赖、工时与权限体系 |
| `WF-005`（工作流模板库与全局下发） | `WF-001` `TEAM-003` | 模板下发依赖工作流引擎与团队全局模板配置 |
| `WF-006`（审批留痕与合规溯源） | `WF-001` `WF-002` `TASK-010` | 留痕读取审批执行模型与状态机执行模型，留痕事件经 Activity 管道汇入 |
...
| `BOARD-004`（视图配置保存与任务批量操作） | `BOARD-003` `TASK-009` `TASK-010` | 批量作用域取 `BOARD-003` 视图结果集，批量归档复用 `TASK-009` 归档服务，批量变更共享 `TASK-010` Activity epoch 约定 |
| `BOARD-005`（视图共享 / 锁定 / 多维度分组） | `BOARD-004` `AUTH-008` | 视图锁定与共享依赖细粒度权限 |

### 4.8 M6-GANTT｜甘特图进度（3）

> 依赖集与规格现行版同步（sprint-4 六份规格评审 PASS 定稿；Sprint-4 概览 §3 注登记的回改项）：`GANTT-001` 行按其元信息 + §1.6 补全筛选 / 视图 / live 边（原摘要行漏 `TASK-003` `TASK-011` `BOARD-003` `INFRA-002`、多 `TASK-001`——`TASK-001` 写通道边属 `GANTT-002`）；`GANTT-002` 行按其 §1.5 补全（原漏 `TASK-001` `TASK-005` `TASK-010` `COLLAB-004`、多 `TASK-004`）。

| 下游功能 | 上游依赖 | 依赖说明 |
| --- | --- | --- |
| `GANTT-001`（甘特图渲染与日 / 周 / 月粒度） | `TASK-003` `TASK-004` `TASK-005` `TASK-006` `TASK-011` `BOARD-003` `INFRA-002` | 任务条消费起止时间、依赖连线与工时数据；筛选语义复用列表筛选与筛选 DSL；视图框架（layout=gantt）；live 服务承载条实时刷新 |
| `GANTT-002`（任务条拖拽 / 依赖连线 / 导出） | `GANTT-001` `TASK-001` `TASK-005` `TASK-010` `COLLAB-004` | 拖拽写通道收敛 Issue PATCH；依赖关系判冲突；改期 Activity 与实时广播 |
| `GANTT-003`（关键路径计算与延期预警） | `GANTT-002` `TASK-005` `PROJ-004` `WF-003` | 关键路径以 `TASK-005` 的 blocks 依赖图（无环前提）为计算前提，跨项目汇总依赖项目集，预警自动化转发走自动化规则引擎（可选增强通道） |

### 4.9 M7-FILE｜文件资源管理（6）

> 依赖集与规格现行版同步（sprint-4 六份规格评审 PASS 定稿；Sprint-4 概览 §3 注登记的回改项）：`FILE-002` 行 `PROJ-001`→`PROJ-002`（成员与角色）并补 `AUTH-005` `TASK-010`；`FILE-003` 行补全直传 / MinIO+Celery / 动态流 / WS 边；`FILE-004` 行 `AUTH-006`→`AUTH-001`（Argon2id 基线 Sprint 0 已交付，`AUTH-006` 属 Sprint 5）并按规格改定 `FILE-002` `FILE-001` `FILE-003` `AUTH-001` `AUTH-005`。

| 下游功能 | 上游依赖 | 依赖说明 |
| --- | --- | --- |
| `FILE-001`（任务级附件上传下载） | `TASK-001` `INFRA-004` | 附件挂在 Issue 上，MVP 阶段使用可替换存储抽象 |
| `FILE-002`（项目文件库与多层级目录） | `FILE-001` `INFRA-002` `PROJ-002` `AUTH-005` `TASK-010` | 复用预签名直传与 `FileAsset` 演进、MinIO 对象存储与 pg_trgm；成员角色定三态可见性；操作入 Activity 管道 |
| `FILE-003`（大文件分片续传与在线预览） | `FILE-002` `FILE-001` `INFRA-002` `COLLAB-003` `COLLAB-004` | 文件库骨架与版本指针；直传白名单与 302 换发先例；multipart 与 Celery 转码编排；版本事件入动态流并经 WS 通知 |
| `FILE-004`（文件多版本 / 分享链接 / 权限） | `FILE-002` `FILE-001` `FILE-003` `AUTH-001` `AUTH-005` | 可见性单入口与预签名复用；匿名下载 302 范式；分享跟随当前版本；Argon2id 密码与权限门控（(IP, slug) 防爆破限流为本文自带，Sprint 6 `INFRA-005` 收编） |
| `FILE-005`（项目 Wiki 与全局知识检索） | `FILE-004` `AUTH-008` `COLLAB-004` | Wiki 权限委托项目权限，协同编辑依赖实时服务 |
| `FILE-006`（文件水印 / 脱敏 / 合规留存） | `FILE-004` `AUTH-010` | 合规留存与敏感文件访问纳入全站审计 |
...
| `INTG-003`（Slack / Zoom 全量集成） | `INTG-001` `INTG-002` `COLLAB-001` `COLLAB-002` | 复用 GitHub 集成框架与 Webhook 出站管道，纪要回挂与 Slack→任务评论以评论体系为落库载体，线程同步落楼中楼 |
| `INTG-004`（完整 Open API 与应用接入） | `INTG-002` `AUTH-012` | 以 API v1 全量稳定为前提，Webhook 出站为应用事件通道，租户级 API 配额治理走多租户域（与 INTG-003 方向互补、无依赖边） |
...
| `RPT-002`（项目进度与成员任务量统计） | `TASK-009` `AUTH-006` `PROJ-003` | 归档任务退出统计口径与 `?archived` 参数语义对齐 `TASK-009`，聚合查询叠加行级过滤 |
| `RPT-003`（燃尽图 / 迭代速率 / 累积流图） | `RPT-002` `TASK-010` | 敏捷图表依赖状态变更历史 |
| `RPT-004`（团队负载 / 项目健康度 / 报表导出） | `RPT-002` `TASK-013` `GANTT-002` | 团队负载消费工时、进度与甘特数据 |
| `RPT-005`（企业数据大屏与自定义报表） | `RPT-003` `RPT-004` `TASK-013` `PROJ-004` `AUTH-012` | 数据集挂载既有快照产出（燃尽/速率快照、健康度、工时台账、项目集聚合），大屏/报表配额自建并与租户配额分域 |

### 4.13 M12-AI / M13-QA｜AI 与质量保障（2）

| 下游功能 | 上游依赖 | 依赖说明 |
| --- | --- | --- |
| `AI-001`（AI 辅助能力（摘要 / 预警 / 生成）） | `RPT-003` `RPT-004` `AUTH-012` `FILE-006` | 风险预警消费报表聚合特征（全量数据面 Sprint 1-9 为基线前提），多租户隔离与存储配额走治理域（AI 调用配额自建），数据出域复用脱敏基础设施 |
...
graph LR
    ...
    TASK008 --> TASK009
    TASK004 --> TASK009
    TASK003 --> TASK011["TASK-011"]
    TASK008 --> TASK011
    BOARD003 --> TASK011
    TASK011 --> TASK012["TASK-012"]
    TASK008 --> TASK012
    ...
    AUTH010 --> AUTH012["AUTH-012"]

    TASK005 --> WF001["WF-001"]
    TASK010 --> WF001
    WF001 --> WF002["WF-002"]
    WF001 --> WF003["WF-003"]
    WF001 --> WF004["WF-004"]
...
| JSONB 动态字段 | `TASK-008` `TASK-012` `TASK-014`、`TASK-011`（组合筛选器） | P0 须预留 JSONB 列（`custom_fields`），P2 建 GIN 索引；新增字段不执行 DDL |
...
| 操作日志作为统一事件源 | `TASK-010`、`PROJ-003`、`COLLAB-003`、`WF-003`、`RPT-003`、`AUTH-010` | 动态流、自动化触发器、燃尽图、审计日志全部消费同一份变更事件表，事件结构一次定型多处复用 |
...
| 视图筛选 DSL 单一实现 | `TASK-003` `TASK-011` `BOARD-004` `GANTT-001` `RPT-002` | 列表 / 看板 / 甘特 / 表格四大视图共用同一套筛选与排序构造器，禁止各视图自建查询逻辑 |
...
| Sprint 2 | `TASK-005` `TASK-006` `TASK-007` `TASK-008` | `TASK-004` `TASK-010` | `TASK-009` 需等 `TASK-004`（子树深拷贝）与 `TASK-003`/`TASK-008`；`TASK-010` 收口需等 `TASK-002`~`TASK-009` 各写操作事件源接入完毕 |
| Sprint 3 | `COLLAB-002` `COLLAB-004` | `BOARD-003` `TASK-011` | `COLLAB-003` 需等 `TASK-010`；`BOARD-004` 需等 `BOARD-003`+`TASK-009`；`TASK-011` 需等 `BOARD-003`（同迭代 `IssueView` 表）与跨迭代 `TASK-003`/`TASK-008` |
...
| Sprint 7 | `WF-001` → `WF-004` / `WF-003`（并行） | `TASK-012` `TASK-013` | `WF-002` 等 `WF-004`；`WF-005` 等 `TEAM-003`；`WF-006` 等 `WF-002` |
| Sprint 8 | `AUTH-007` `AUTH-010` | `AUTH-009` `BOARD-005` | `AUTH-008` 需等 `AUTH-007` + `WF-004`，是细粒度权限收口项 |
...
**测试提前介入的关键点**：`TASK-011` 组合筛选器、`AUTH-006` 行级过滤、`TASK-004` 递归子任务、`TASK-005` 依赖循环检测四处是缺陷高发区，要求在实现前先写测试用例清单并评审。
