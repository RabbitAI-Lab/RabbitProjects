...
| 9 | **高保真原型流程债（2026-09-09 用户裁定）**：S5~8 四迭代未走「先原型后开发」纪律 | Sprint-8 补做件 `docs/design/sprint-8-hifi-prototype.html`（DRAFT 评审稿，冻结后按稿整改六页面 + 更新 parity）；**S5~7 存量表面无原型清单**：admin 台四页 + 审批中心/画布/工时/工作流四页/字段权限矩阵/自动化规则页（S6~7 批次）——随 S9 各自触发整改时先补原型再动代码；CLAUDE.md 已加硬门槛「无冻结原型不得写 UI 代码」 |
