...
1. **批量分拣范式**：一次取数 + 内存分拣 + 逐条独立落库 + 逐条结果。该范式被 `PROJ-002`（项目成员批量添加）直接沿袭，适用于**成员治理类**批量端点；P2 `BOARD-004` 批量任务操作**不同源**——任务批量走 `api-conventions.md` §10.5 单事务全成败范式（整批回滚 + 失败项索引定位），批量语义以其为唯一事实源（Sprint-3 回改 2026-09-05，原「同源/标准形态」表述废止）。
