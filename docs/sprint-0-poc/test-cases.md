...
> 依据 ADR-0010 纪律 ①：Sprint-2（TASK-004~010）新增/变更 UI 表面先行登记，每行标规格出处；跨文档裁决 R1~R5 与原型开放点 O1~O4 见 `docs/design/sprint-2-hifi-prototype.html` 头部（评审冻结后回写本表基线列）。视觉/交互验收基准 = 该冻结原型。
...
| 键盘 | 【默认态】Tab 顺序 类型 → 搜索 → 结果方向键 → 提交；<768 全屏 | 断言 | TASK-005 §3.2/§3.7 |
...
| 12 类型宫格 | 【默认态】12 类型图标+名称（text/textarea/select/multi_select/number/currency/date/checkbox/member/url/email/phone）；选定后表单下段按类型变形（选项区/日期格式/币种）；auto_increment 选项区隐藏、必填强制关 | 断言 | TASK-008 §3.2/§4.4.2 |
