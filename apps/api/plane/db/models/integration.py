...
class IntegrationInstallation(BaseModel):
    ...
    provider = models.CharField(max_length=16, choices=Provider.choices,
                                default=Provider.GITHUB, verbose_name="集成提供方")
    ...
    project = models.ForeignKey("db.Project", on_delete=models.CASCADE,
                                related_name="integrations", verbose_name="项目")
    repository_full_name = models.CharField(
        max_length=200, verbose_name="仓库全名",
        help_text="owner/repo，小写")
    repository_node_id = models.CharField(max_length=64, blank=True, default="",
                                          verbose_name="仓库 node_id")
    webhook_secret = models.TextField(
        verbose_name="Webhook 验签密钥（Fernet 密文；HMAC 用 Service 层解密原文）")
    default_issue_type = models.ForeignKey(
        "db.IssueType", null=True, blank=True, on_delete=models.SET_NULL,
        verbose_name="入站建任务默认类型")
    ...
    token_cache = models.JSONField(default=dict, blank=True,
                                   verbose_name="installation token 缓存")
    ...
    class Meta(BaseModel.Meta):
        ...
        constraints = [
            ...
            models.CheckConstraint(
                condition=~models.Q(repository_full_name=""),
                name="chk_repo_name"),
        ]
        indexes = [
...
class SyncConflictLog(BaseModel):
    ...
    binding = models.ForeignKey("db.IntegrationInstallation",
                                ...
                                related_name="conflict_logs", verbose_name="绑定")
    issue = models.ForeignKey("db.Issue", on_delete=models.CASCADE,
                              related_name="github_conflict_logs", verbose_name="任务")
    scope = models.CharField(max_length=16, choices=Scope.choices,
                             db_index=True, verbose_name="冲突面")
    direction = models.CharField(max_length=16, choices=Direction.choices,
                                 db_index=True, verbose_name="方向")
    ...
    delivery_id = models.CharField(max_length=64, blank=True, default="",
                                   verbose_name="X-GitHub-Delivery 溯源")
...
def encrypt_secret(plaintext: str) -> str:
    ...
    if not key:
        key = base64.urlsafe_b64encode(
            hashlib.sha256(dj_settings.SECRET_KEY.encode()).digest()).decode()
    return Fernet(key if isinstance(key, str) else key).encrypt(
