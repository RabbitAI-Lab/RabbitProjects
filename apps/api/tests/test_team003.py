...
@pytest.fixture()
def env(db):
    owner = User.objects.create_user(email="t3-owner@rabbit.dev", password="Rabbit123!", display_name="张三")
    admin = User.objects.create_user(email="t3-admin@rabbit.dev", password="Rabbit123!", display_name="李管理")
    member = User.objects.create_user(email="t3-member@rabbit.dev", password="Rabbit123!", display_name="王成员")
    ws = Workspace.objects.create(name="W", slug=f"w-t3-{owner.id.hex[:8]}", owner=owner, created_by=owner)
    for u, r in ((owner, WorkspaceRole.OWNER), (admin, WorkspaceRole.ADMIN), (member, WorkspaceRole.MEMBER)):
    ...
    issue = Issue.objects.create(
        name="任", project=proj, state=todo, priority="none", sequence_id=1, sort_order=100, created_by=owner
    )
    return {"owner": owner, "admin": admin, "member": member, "ws": ws, "proj": proj, "issue": issue}
...
def test_archive_owner_only_and_idempotent(env):
    ...
    Issue.objects.filter(project__workspace=env["ws"]).update(completed_at=_tz.now())
    assert _client(env["admin"]).post(f"{_base(env)}/archive/", format="json").status_code == 403
...
def test_archive_middleware_write_guard(env):
    ...
    assert (
        _client(env["admin"]).get(f"{_base(env)}/projects/{env['proj'].id}/issues/", format="json").status_code == 200
    )
    # 写短路 403 PERM_WORKSPACE_ARCHIVED（信封）
    r = _client(env["owner"]).post(f"{_base(env)}/projects/", format="json", data={"name": "X", "identifier": "XA1"})
    ...
    # 恢复后写恢复
    r3 = _client(env["owner"]).post(f"{_base(env)}/projects/", format="json", data={"name": "X", "identifier": "XA1"})
...
def test_label_crud_and_soft_delete_snapshot(env):
    ...
    r = _client(env["admin"]).post(base, format="json", data={"name": "bug", "color": "#E5484D", "description": "缺陷"})
    assert r.status_code == 201
    lid = r.json()["data"]["id"]
    assert _client(env["admin"]).post(base, format="json", data={"name": "bug", "color": "#111111"}).status_code == 409
    assert _client(env["admin"]).post(base, format="json", data={"name": "bad", "color": "red"}).status_code == 400
    # 项目侧覆盖行（overrides_global_id）挂任务 → 删除全局时快照 + affected=1
    cover = Label.objects.create(
        project=env["proj"],
        name="bug-自定义",
        color="#222222",
        origin="local",
        overrides_global_id=lid,
...
def test_project_label_list_merges_globals(env):
    ...
    g = WorkspaceLabel.objects.create(workspace=env["ws"], name="urgent", color="#F59E0B", created_by=env["owner"])
    g2 = WorkspaceLabel.objects.create(workspace=env["ws"], name="feature", color="#8E4EC6", created_by=env["owner"])
    Label.objects.create(project=env["proj"], name="本地", color="#333333", created_by=env["owner"])
    covered = Label.objects.create(
        project=env["proj"], name="urgent-覆盖", color="#444444", overrides_global_id=g.id, created_by=env["owner"]
    )
    r = _client(env["admin"]).get(f"{_base(env)}/projects/{env['proj'].id}/labels/", format="json")
...
def test_default_states_get_put(env):
    ...
    d = r.json()["data"]
    assert d["version"] == 0 and {g["group"] for g in d["groups"]} >= {"unstarted", "started", "completed", "cancelled"}
    payload = {
        "groups": [
    ...
    assert _client(env["member"]).put(f"{_base(env)}/default-states/", format="json", data=payload).status_code == 403
    r2 = _client(env["admin"]).put(f"{_base(env)}/default-states/", format="json", data=payload)
    ...
    assert _client(env["admin"]).put(f"{_base(env)}/default-states/", format="json", data=bad).status_code == 400
    # 缺组 → 400
    bad2 = {"groups": payload["groups"][:4]}
    assert _client(env["admin"]).put(f"{_base(env)}/default-states/", format="json", data=bad2).status_code == 400
...
def test_activity_stats_schema_and_buckets(env):
    ...
    assert WorkspaceLoginDailyAggregate.objects.filter(member=env["member"], workspace=env["ws"]).count() == 1


def test_activity_stats_permission_and_days(env):
    assert _client(env["member"]).get(f"{_base(env)}/activity-stats/", format="json").status_code == 403
    assert _client(env["admin"]).get(f"{_base(env)}/activity-stats/?days=15", format="json").status_code == 400
