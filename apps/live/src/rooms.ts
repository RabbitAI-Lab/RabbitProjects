 ...
 *
 *  三类房间：project:{pid} / issue:{iid} / user:{uid}；单连接房间 ≤10（票据声明
 *  已限）、单房间连接 ≤500（§2.6 分片上限，超容仅 WARN 跳过——单副本部署不达
 *  此量级）；每用户全局并发 ≤5（BR-12：第 6 条踢最旧 4000 DUP_SESSION）；同
 *  ws 标识（{sub}:{client_tab_id}）重连幂等替换旧连接（不占新额度）。
 *
