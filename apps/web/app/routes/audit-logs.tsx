 ...
 *
 *  筛选条（category/action 级联 + actor + 关键词）+ 事件表（游标分页）+
 *  行详情抽屉（kv 网格 + 哈希链行 + 敏感键过滤）+ 导出对话框（密码双确认）。
 *  视觉基准：docs/design/sprint-8-hifi-prototype.html（FROZEN 2026-09-09）。 */
...
const ACTION_LABEL: Record<string, string> = {
  ...
  config_updated: "SSO 配置更新", connection_check: "SSO 测试连接",
  enforce_on: "开启强制 SSO", enforce_off: "关闭强制 SSO",
  exported: "导出审计 CSV", instance_query: "实例级检索",
  maintenance: "系统维护", general: "通用事件",
};
          ...
          <div className="mx-auto max-w-[1240px] px-2 py-4">
            <div className="mb-[18px] flex items-center gap-3">
              ...
              <div className="ml-auto flex gap-2">
                <button className="h-7 rounded-md border border-neutral-300 px-2.5 text-xs text-neutral-600 hover:bg-neutral-50" data-sb-scope="audit-chain-check">🔗 链校验</button>
                <button onClick={() => setExportOpen(true)} className="h-7 rounded-md bg-blue-600 px-2.5 text-xs font-medium text-white hover:bg-blue-700" data-sb-scope="audit-export-open">导出 CSV</button>
              </div>
            </div>
            ...
            <div className="overflow-hidden rounded-lg border bg-white shadow-sm">
              <div className="flex flex-wrap items-center gap-2 border-b border-neutral-200 px-3.5 py-3">
                ...
                <select value={fAction} onChange={(e) => setFAction(e.target.value)} disabled={!fCat}
                        className="h-[30px] w-[130px] rounded-md border border-neutral-300 px-1.5 text-[12.5px] disabled:opacity-50" data-sb-scope="audit-filter-action">
                  <option value="">全部动作</option>
                  {actionOpts.map((a) => <option key={a.action} value={a.action}>{a.label}</option>)}
                </select>
                <input value={fActor} onChange={(e) => setFActor(e.target.value)} placeholder="操作者 ID"
                       className="h-[30px] w-[170px] rounded-md border border-neutral-300 px-2 text-[12.5px]" data-sb-scope="audit-filter-actor" />
                <input value={fSearch} onChange={(e) => setFSearch(e.target.value)} placeholder="对象名关键词"
                       className="h-[30px] w-[160px] rounded-md border border-neutral-300 px-2 text-[12.5px]" data-sb-scope="audit-filter-search" />
                <button onClick={() => load("")} className="h-[30px] rounded-md border border-neutral-300 px-3 text-[12.5px] text-neutral-600 hover:bg-neutral-50" data-sb-scope="audit-filter-apply">筛选</button>
                {meta && (
              ...
              <table className="w-full text-[13px]">
                <thead className="text-left text-xs font-medium text-neutral-400">
                  <tr>
                    <th className="px-3 py-2" style={{ width: 140 }}>时间</th>
                    <th>事件</th><th>操作者</th><th>对象</th>
                    <th style={{ width: 110 }}>IP</th>
                  </tr>
                </thead>
                <tbody data-sb-scope="audit-rows">
                  {rows.map((r) => (
                    <tr key={r.id} className="cursor-pointer border-t border-neutral-200 hover:bg-neutral-50" onClick={() => setDetail(r)}>
                      <td className="px-3 py-2 font-mono text-xs text-neutral-400">
                        {new Date(r.created_at).toLocaleString("zh-CN", { hour12: false })}
                      </td>
                      <td>
                        <span className={`cat-dot ${r.category}`} />
                        <span className="font-mono text-[11px] text-neutral-400">{r.category}</span>
                        <span className="ml-1 text-[12.5px]">{ACTION_LABEL[r.action] ?? r.action}</span>
                      </td>
              ...
              <div className="mt-3 flex items-center justify-between border-t border-neutral-200 px-3.5 py-3">
                <button onClick={() => load(String(meta?.prev_cursor ?? ""))} disabled={!meta?.prev_cursor}
                        className="rounded-md border border-neutral-300 px-3 py-1 text-sm text-neutral-600 hover:bg-neutral-50 disabled:opacity-40">
                  上一页
                </button>
                <button onClick={() => load(String(meta?.next_cursor ?? ""))} disabled={!meta?.next_page_results}
                        className="rounded-md border border-neutral-300 px-3 py-1 text-sm text-neutral-600 hover:bg-neutral-50 disabled:opacity-40"
                        data-sb-scope="audit-next">
                  下一页
                </button>
              </div>
            </div>
        ...
        <aside className="fixed right-0 top-0 z-40 h-full w-[380px] overflow-auto border-l border-neutral-200 bg-white p-5"
          ...
          <div className="mb-3.5 flex items-center justify-between">
            <b className="text-[14px]">
              <span className={`cat-dot ${detail.category}`} />
              {ACTION_LABEL[detail.action] ?? detail.action}
            </b>
            <button onClick={() => setDetail(null)} className="flex h-7 w-7 items-center justify-center rounded-md text-neutral-400 hover:bg-neutral-100">✕</button>
          </div>
          <dl className="grid gap-1 text-[12.5px]" style={{ gridTemplateColumns: "88px 1fr" }}>
            ...
            <dd>
              <pre className="overflow-auto rounded-lg bg-neutral-100 p-2.5 font-mono text-[11.5px]">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-[2px]">
          <div className="w-[400px] max-w-full rounded-xl bg-white p-6 shadow-2xl" role="dialog" aria-label="导出审计 CSV" data-sb-scope="audit-export-dialog">
            <div className="mb-3 flex items-center justify-between">
              ...
              <button onClick={() => setExportOpen(false)} className="flex h-7 w-7 items-center justify-center rounded-md text-neutral-400 hover:bg-neutral-100">✕</button>
            </div>
            ...
            <div className="mt-5 flex justify-end gap-2.5">
              <button onClick={() => setExportOpen(false)} className="h-9 rounded-md border border-neutral-300 px-3.5 text-[13px] text-neutral-600 hover:bg-neutral-50">取消</button>
              <button onClick={doExport} disabled={exporting || !password}
                      className="h-9 rounded-md bg-blue-600 px-3.5 text-[13px] font-medium text-white hover:bg-blue-700 disabled:opacity-50"
