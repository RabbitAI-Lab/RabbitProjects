...
export function Topbar() {
  ...
  // 系统管理入口可见性（WS_ADMIN+；快照未到 fail-open，页面后端守卫兜底）
  const wsRole4System = permission.snapshot === null
    ? WorkspaceRole.OWNER
    : permission.workspaceRole(undefined, session.currentWsSlug ?? "");
  const canSeeSystem = wsRole4System >= WorkspaceRole.ADMIN;
        ...
        <div className="relative">
          ...
          {menu === "avatar" && (
            <div data-sb-scope="topbar-menu" className="absolute top-[36px] right-0 w-[190px] bg-white border border-neutral-200 rounded-lg shadow-lg py-1" role="menu">
              ...
              {/* 系统管理入口（2026-09-12 用户走查定稿）：治理/身份/安全族
                  从空间侧栏迁入本下拉——WS_ADMIN+ 可见（成员级隐藏） */}
              {canSeeSystem && (
                <button role="menuitem" data-sb-scope="topbar-system-entry" onClick={(e) => {
                  e.stopPropagation(); setMenu(null);
                  location.href = `/${session.currentWsSlug ?? ""}/system`;
                }}
