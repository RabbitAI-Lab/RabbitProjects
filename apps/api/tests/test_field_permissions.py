...
@pytest.fixture()
def env(db):
    owner = User.objects.create_user(email="f-owner@rabbit.dev", password="Rabbit123!",
                                     display_name="管理员")
    member = User.objects.create_user(email="f-member@rabbit.dev", password="Rabbit123!",
                                      display_name="成员")
    ws = Workspace.objects.create(name="W", slug=f"w-f-{owner.id.hex[:8]}",
...
class TestFieldValueValidation:
    def test_cascade_length_must_match_levels(self, env):
        d = CustomFieldDefinition(workspace=env["ws"], name="区", field_key="cf_region",
                                   field_type=CustomFieldDefinition.FieldType.CASCADE,
                                   cascade_config={
            "levels": [
                {"name": "省", "options": [{"label": "浙", "value": "zj"}]},
                {"name": "市", "options": [
                    {"label": "杭", "value": "hz", "parent_value": "zj"}]},
            ]
        },
    ...
    def test_relation_20_limit_and_cross_project(self, env):
        d = CustomFieldDefinition(workspace=env["ws"], name="关联", field_key="cf_rel",
                                   field_type=CustomFieldDefinition.FieldType.RELATION,
                                   created_by=env["owner"])
        from plane.db.models import Issue as _I

        i1 = _I.objects.create(project=env["proj"], name="x", state=State.objects.filter(
            project=env["proj"], name="待办").first(), created_by=env["owner"])
    ...
    def test_date_range_start_le_end(self, env):
        d = CustomFieldDefinition(workspace=env["ws"], name="窗口", field_key="cf_win",
    ...
    def test_attachment_not_uploaded_rejected(self, env):
        ...
        d = CustomFieldDefinition(workspace=env["ws"], name="合同", field_key="cf_atta",
                                   field_type=CustomFieldDefinition.FieldType.ATTACHMENT,
                                   created_by=env["owner"])
        not_uploaded = FileAsset.objects.create(
            workspace=env["ws"], project=env["proj"], entity_type="issue",
            entity_id=str(_uuid.uuid4()), size=0,
            storage_path="cf/na.txt", created_by=env["owner"])
...
class TestConfigValidation:
    def test_cascade_3_levels_max(self, env):
        issues = validate_cascade_config({"levels": [{"name": str(i), "options": []}
                                                     for i in range(4)]})
    ...
    def test_cascade_300_total_limit(self, env):
        ...
        issues2 = validate_cascade_config({
            "levels": [
                ...
                {"name": "L2", "options": [{"label": "b"+str(i), "value": "b"+str(i),
                                            "parent_value": "a" + str(i % 80)}
                                            for i in range(250)]},
...
class TestFieldPermission:
    def _field(self, env, **cfg):
        return CustomFieldDefinition.objects.create(
            ...
            name="xname", field_key="cf_xname", field_type="text",
            permission_config=cfg, created_by=env["owner"])
    ...
    def test_readonly_no_write(self, env):
        d = self._field(env, read=["role:proj_contributor"],
                          write=["role:proj_admin"])
        m = FieldPermissionService().resolve(env["member"], env["proj"], [d])
        assert m["cf_xname"] == "readonly"

    def test_required_for(self, env):
        d = self._field(env, read=["role:proj_contributor"],
                          write=["role:proj_contributor"],
    ...
    def test_drop_non_writable_br16(self, env):
        d = self._field(env, read=["role:proj_contributor"],
                          write=["role:proj_admin"])
        m = FieldPermissionService().resolve(env["member"], env["proj"], [d])
        dropped = FieldPermissionService.drop_non_writable(
            {"cf_xname": "v", "cf_other_field": "k"}, m)
...
class TestFilterHiddenRejection:
    def test_cf_hidden_in_filter_403(self, env):
        ...
        d = CustomFieldDefinition.objects.create(
            workspace=env["ws"], project=env["proj"], name="hfield", field_key="cf_hfield",
            field_type="text", permission_config={"read": ["role:proj_admin"],
                                                    "write": ["role:proj_admin"]},
            created_by=env["owner"])
        m = FieldPermissionService().resolve(env["member"], env["proj"], [d])
        ctx_hidden = CompileContext.build(project=env["proj"], user=env["member"],
                                          access_map=m)
    ...
    def test_cf_non_hidden_filter_works(self, env):
        ...
        CustomFieldDefinition.objects.create(
            workspace=env["ws"], project=env["proj"], name="vfield", field_key="cf_vfield",
            field_type="text", created_by=env["owner"])
