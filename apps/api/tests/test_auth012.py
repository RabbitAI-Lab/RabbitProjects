...
def env(db):
    ...
    ops1 = User.objects.create_user(email="g-ops1@rabbit.dev", password="Rabbit123!", display_name="张运营")
    ops2 = User.objects.create_user(email="g-ops2@rabbit.dev", password="Rabbit123!", display_name="李运营")
    admin = User.objects.create_user(email="g-admin@rabbit.dev", password="Rabbit123!", display_name="客户管理员")
    SystemAdmin.objects.create(user=ops1, is_active=True, is_tenant_ops=True, created_by=ops1)
    SystemAdmin.objects.create(user=ops2, is_active=True, is_tenant_ops=True, created_by=ops2)
    tenant = Tenant.objects.create(name="Beta Test", tier="free", created_by=ops1)
    ws = Workspace.objects.create(name="W", slug=f"w-gov-{tenant.id.hex[:8]}", owner=admin, created_by=admin)
    Workspace.objects.filter(pk=ws.pk).update(tenant=tenant)
    ws.refresh_from_db()
    WorkspaceMember.objects.create(workspace=ws, member=admin, role=WorkspaceRole.OWNER, created_by=admin)
    return {"ops1": ops1, "ops2": ops2, "admin": admin, "tenant": tenant, "ws": ws}
...
def test_ut01_quota_null_follows_tier(env):
    ...
    TenantQuota.objects.create(tenant=env["tenant"], storage_bytes=100, created_by=env["ops1"])
...
def test_ut03_member_quota(env):
    ...
    ws2 = Workspace.objects.create(
        name="W2", slug=f"w2-gov-{env['tenant'].id.hex[:6]}", owner=env["admin"], created_by=env["admin"]
    )
    ...
    for i in range(8):  # free 档 10 人
        ...
        WorkspaceMember.objects.create(workspace=env["ws"], member=u, role=WorkspaceRole.MEMBER, created_by=u)
    with GOV:
        enforcement.check_member_quota(env["ws"])  # 9 人 +1 = 10 未超
        u10 = User.objects.create_user(email="m10@rabbit.dev", password="x")
        WorkspaceMember.objects.create(workspace=ws2, member=u10, role=WorkspaceRole.MEMBER, created_by=u10)
...
def test_ut04_downgrade_grace(env):
    ...
    with GOV:
        ...
        TenantQuota.objects.create(
            tenant=env["tenant"], downgrade_grace_until=dt.date.today() + dt.timedelta(days=5), created_by=env["ops1"]
        )
...
def test_ut25_webhook_quota(env):
    ...
    with GOV:
        project = Project.objects.create(workspace=env["ws"], name="P", identifier="GV", created_by=env["admin"])
...
def test_ut26_project_quota(env):
    ...
    with GOV:
        for i in range(3):  # free 档项目 3
            Project.objects.create(workspace=env["ws"], name=f"P{i}", identifier=f"G{i}", created_by=env["admin"])
        with pytest.raises(AppException) as exc:
...
def _fire_rule_seeded():
    ...
    for code, threshold, action in [
        ...
        ("R-05", {"count": 50, "window": "1d", "night_start_hour": 0, "night_end_hour": 6}, "alert"),
        ("R-06", {"quota_ratio": 0.5, "getlist_ratio": 0.95, "window": "1h"}, "throttle"),
    ]:
        RiskRule.objects.get_or_create(
            code=code, tenant__isnull=True, defaults={"threshold": threshold, "action": action}
        )
...
def test_ut05_ut13_r01_login_storm(env, monkeypatch):
    ...
    with GOV:
        ...
        ev = {"action": "login_failed", "actor_id": "u1", "ip": "203.0.113.5", "workspace_id": str(env["ws"].id)}
        for _ in range(200):
        ...
        assert RiskEvent.objects.filter(tenant_id=str(other.id), rule_code="R-01").count() == 1
        assert RiskEvent.objects.filter(tenant_id=tid, rule_code="R-01").count() == 1
...
def test_ut14_r02_geo_distance(env):
    ...
    with GOV:
        ...
        ev = {
            ...
            "detail": {"geo": {"lat": 31.23, "lng": 121.47}},
        }  # 上海
...
def test_ut15_r04_operator_subject(env):
    ...
    with GOV:
        eng = RiskRuleEngine()
        tid = str(env["tenant"].id)
        ev = {"action": "role_grant", "actor_id": "op1", "workspace_id": str(env["ws"].id)}
        for _ in range(5):
...
def test_ut16_r05_night_predicate(env, monkeypatch):
    ...
    with GOV:
        tid = str(env["tenant"].id)
        ev = {"action": "issue.delete", "actor_id": "u1", "workspace_id": str(env["ws"].id)}
...
def test_ut17_r06_dual_counter(env):
    ...
    with GOV:
        TenantQuota.objects.create(
            tenant=env["tenant"], api_rate_per_minute=10, created_by=env["ops1"]
        )  # 10rpm → 600/h → 50%=300
        eng = RiskRuleEngine()
        tid = str(env["tenant"].id)
        base = {"action": "api_call", "workspace_id": str(env["ws"].id), "detail": {"token_id": "tk1"}}
        for i in range(300):  # 93.3% GET 且未越量线
            eng.on_api_call(tid, {**base, "detail": {"token_id": "tk1", "is_get_list": i < 280}})
        assert RiskEvent.objects.filter(tenant_id=tid, rule_code="R-06").count() == 0
        for _ in range(240):  # 追加全 GET：t≥400 后占比过 95%
            eng.on_api_call(tid, {**base, "detail": {"token_id": "tk1", "is_get_list": True}})
        ...
        for i in range(500):
            eng.on_api_call(tid, {**base, "detail": {"token_id": "tk2", "is_get_list": i % 5 != 0}})
...
def test_ut11_ungoverned_skip(env):
    ...
    _fire_rule_seeded()
    with GOV:
        plain = Workspace.objects.create(
            name="P", slug=f"w-plain-{env['ops1'].id.hex[:6]}", owner=env["admin"], created_by=env["admin"]
        )
        before = RiskEvent.objects.count()
        RiskRuleEngine().ingest({"action": "login_failed", "ip": "1.2.3.4", "workspace_id": str(plain.id)})
...
def test_ut12_private_mode_off(env):
    ...
    before = RiskEvent.objects.count()
    RiskRuleEngine().ingest({"action": "login_failed", "ip": "1.2.3.4", "workspace_id": str(env["ws"].id)})
    assert RiskEvent.objects.count() == before
...
def test_ut07_rules_only_tighter(env):
    ...
    with GOV:
        ...
        assert RiskRule.objects.get(code="R-01", tenant=env["tenant"]).threshold["count"] == 100
...
def test_ut08_ut19_freeze_two_sign_and_release(env):
    ...
    with GOV:
        ...
        # 第一签
        r = c1.post(
            f"/api/v1/instances/risk-events/{ev.id}/actions/", {"action": "freeze", "note": "复核"}, format="json"
        )
        ...
        # 同人二签 → 403
        r = c1.post(f"/api/v1/instances/risk-events/{ev.id}/freeze-approval/", {}, format="json")
        assert r.status_code == 403
        assert r.json()["error"]["code"] == "PERM_DENIED"
        assert not env["tenant"].is_frozen
        # 异人二签 → 生效 + Redis 标记
        r = c2.post(f"/api/v1/instances/risk-events/{ev.id}/freeze-approval/", {}, format="json")
        assert r.status_code == 200
        ...
        r = c2.post(f"/api/v1/instances/risk-events/{ev.id}/releases/", {"note": "复核完成"}, format="json")
...
def test_ut19_throttle_release_immediate(env):
    ...
    with GOV:
        ...
        c = _ops_client(env["ops1"])
        r = c.post(f"/api/v1/instances/risk-events/{ev.id}/releases/", {"note": "误操作"}, format="json")
...
def test_ut18_l2_gate_and_written_confirm(env):
    ...
    with GOV:
        ...
        t = GovernanceTicket.objects.create(
            tenant=env["tenant"],
            risk_event=ev,
            requested_by=env["ops1"],
            scope={"fields": ["issue.title"], "ids": ["u_1"]},
            approve_channel="written",
            created_by=env["ops1"],
        )
        c = _ops_client(env["ops1"])
        r = c.post(f"/api/v1/instances/l2-tickets/{t.id}/written-confirm/", {"note": "无单号"}, format="json")
        assert r.status_code == 400
        body = r.json()["error"]
        assert body["details"][0]["field"] == "written_ref"
        assert body["details"][0]["code"] == "REQUIRED"
        r2 = c.post(
            f"/api/v1/instances/l2-tickets/{t.id}/written-confirm/", {"written_ref": "GT-2026-0099"}, format="json"
        )
        assert r2.status_code == 200
        t.refresh_from_db()
        ...
        # 过期后剥离
        GovernanceTicket.objects.filter(pk=t.id).update(expires_at=dt.datetime.now(dt.UTC) - dt.timedelta(hours=1))
...
def test_it07_l2_online_approval_chain(env):
    ...
    with GOV:
        ...
        c = _ops_client(env["admin"])
        r = c.post(
            f"/api/v1/workspaces/{env['ws'].slug}/security/l2-tickets/{t.id}/approval/",
            {"decision": "approve"},
            format="json",
        )
        assert r.status_code == 200
...
def test_ut20_it09_appeal_chain(env):
    ...
    with GOV:
        ...
        r = c_admin.post(
            f"/api/v1/workspaces/{env['ws'].slug}/security/events/{ev.id}/appeals/",
            {"reason": "批量导出系报表例行"},
            format="json",
        )
        assert r.status_code == 200
        ...
        r2 = c_ops.post(
            f"/api/v1/instances/risk-appeals/{appeal_id}/review/",
        ...
        cache.set(f"risk:throttle:{env['tenant'].id}", 0.1, timeout=3600)
        r3 = c_admin.post(
            f"/api/v1/workspaces/{env['ws'].slug}/security/events/{ev2.id}/appeals/", {"reason": "x"}, format="json"
        )
        r4 = c_ops.post(
            f"/api/v1/instances/risk-appeals/{r3.json()['data']['appeal_id']}/review/",
            {"decision": "rejected"},
            format="json",
        )
...
def test_ut21_risk_rule_unique_constraints(env):
    ...
    with pytest.raises(IntegrityError):
        with transaction.atomic():  # 保存点：防事务中止污染
            RiskRule.objects.create(code="R-01", threshold={"count": 1}, action="alert")
    RiskRule.objects.create(
        code="R-01", tenant=env["tenant"], threshold={"count": 1}, action="alert", created_by=env["ops1"]
    )
    with pytest.raises(IntegrityError):
        with transaction.atomic():
            RiskRule.objects.create(code="R-01", tenant=env["tenant"], threshold={"count": 2}, action="alert")
...
def test_ut22_it10_r03_deny_tier(env):
    ...
    with GOV:
        ...
        deny = RiskEvent.objects.filter(tenant_id=tid, rule_code="R-03").order_by("-created_at").first()
...
def test_r03_under_warn_passes(env):
    ...
    with GOV:
        ...
        cache.set(_export_day_key(str(env["tenant"].id)), int(TIER_DEFAULTS["free"]["export_rows_per_day"] * 0.95))
...
def test_gov_endpoints_require_tenant_ops(env):
    ...
    plain = User.objects.create_user(email="g-plain@rabbit.dev", password="Rabbit123!")
    SystemAdmin.objects.create(user=plain, is_active=True, is_tenant_ops=False, created_by=plain)
    with GOV:
...
def test_it05_quota_patch_audited(env, monkeypatch, django_capture_on_commit_callbacks):
    ...
    monkeypatch.setattr(_mod.audit_record, "delay", lambda payload: _mod.audit_record.run(payload))
    from plane.governance import tasks as _gt

    monkeypatch.setattr(_gt.risk_ingest, "delay", lambda payload: None)  # 测试内不扇出风控
    with GOV, django_capture_on_commit_callbacks(execute=True):
        c = _ops_client(env["ops1"])
        r = c.patch(f"/api/v1/instances/tenants/{env['tenant'].id}/quota/", {"tier": "standard"}, format="json")
    assert r.status_code == 200
    from plane.db.models import AuditLog

    row = AuditLog.objects.filter(category="governance", action="quota_changed").order_by("-created_at").first()
...
def test_tenant_list_water(env):
    ...
    with GOV:
        ...
        r = c.get("/api/v1/instances/tenants/")
        assert r.status_code == 200
        row = next(t for t in r.json()["data"] if t["id"] == str(env["tenant"].id))
...
def test_security_events_view(env):
    ...
    WorkspaceMember.objects.create(workspace=env["ws"], member=member, role=WorkspaceRole.MEMBER, created_by=member)
    with GOV:
        _mk_event(env)
        r = _ops_client(env["admin"]).get(f"/api/v1/workspaces/{env['ws'].slug}/security/events/")
        assert r.status_code == 200 and len(r.json()["data"]) == 1
        r2 = _ops_client(member).get(f"/api/v1/workspaces/{env['ws'].slug}/security/events/")
...
def test_it08_boundary_report_chain(env, monkeypatch):
    ...
    with GOV:
        c = _ops_client(env["ops1"])
        r = c.post(f"/api/v1/instances/tenants/{env['tenant'].id}/boundary-reports/", {}, format="json")
        ...
        r2 = c.get(f"/api/v1/instances/tenants/{env['tenant'].id}/boundary-reports/{data['report_id']}/")
