...
> **范围声明**：交付**全字段（内置 + 自定义）AND/OR 嵌套组合筛选器**（递归 DSL，深度 ≤ 3、条件节点 ≤ 20——锚定 [`api-conventions.md`](../architecture/api-conventions.md) §5.3；`dynamic-fields-design.md` §5.2/§6.4 的「≤ 5 层 / ≤ 50 条」为架构文档待回改口径）、**视图 filters 能力点亮**（消费 `BOARD-003` 交付的 `IssueView` 表与 `views/` CRUD 端点族，把 filters 从其扁平子集放开为嵌套布尔树；P2 仅个人视图，共享归 P3 `BOARD-005`（s8 已交付）；四布局共用 filters 与 display_props）、**架构 §5.5 筛选层级在 P2 的生效组合**（权限过滤恒最外层 × 项目域 × 视图 filters × 临时 filters；①全局跨项目层级 P3 激活）。全局跨项目筛选（P4 `BOARD-005` 视图治理——原标 P3，跨项目全局视图归 P4，需求文档 §8.2 看板 P4 列「跨项目全局看板」；来源：BOARD-005 s8 文首登记，P4 批次 2026-09-06 登记）、共享视图（P3）、管理员视图锁定（P3）、筛选器自然语言输入（P4 AI）不在范围。
...
2. **视图 filters 能力点亮（Saved View）**——`IssueView` 表、`views/` CRUD 端点族与内置五视图种子由同迭代 `BOARD-003` 交付（唯一事实源）；本文档把 `filters` 从其扁平子集放开为嵌套布尔树并存入该既有字段（P2 仅个人视图，共享归 P3 `BOARD-005`（s8 已交付）），四大视图（列表/看板/甘特/表格）共用——**切换布局筛选不丢**；
...
| 层           | 作用域                     | 可用字段                              | 保存方式                             | 本迭代            |
| ------------ | -------------------------- | ------------------------------------- | ------------------------------------ | ----------------- |
| ① 全局跨项目 | Workspace 内可见项目集合   | 内置 + 全局字段（项目私有字段不可用） | Workspace 级视图                     | ❌ P4 `BOARD-005` |
...
| 共享视图（`access=shared`）                                     | ❌ 列已建不启用（`BOARD-003` BR-01：P2 传 shared 即 400）      | P3 `BOARD-005`（s8 已交付）             |
| 跨项目全局视图（`IssueView.project=NULL`）                      | ❌ 列已建不启用                                                | P4 `BOARD-005`（视图治理） |
| 管理员视图锁定（`is_locked`）                                   | ❌ 列已建不启用                                                | P3 `BOARD-005`（s8 已交付）             |
...
| 传 `access=shared`    | P2 创建/编辑携带                     | 400（共享归 P3 `BOARD-005`——s8 已交付）                                          | 共享入口不渲染                                       | BR-10（`BOARD-003` BR-01） |
...
├──────────────────────────────────────────────────────────────────────────┤
│  （当前视图主体：列表/看板/甘特/表格——布局切换器在左侧）                     │
└──────────────────────────────────────────────────────────────────────────┘
  ●全部 = 前端固定入口（不入库）  🔒 = 内置视图（不可改）
  Tab 右键 / ⋯ 菜单：重命名·编辑条件·复制·删除（共享设置 P3 出现）
```

> 勘误 2026-09-06（ADR-0020）：① 布局切换器位置原标「右侧」为笔误，按 `BOARD-003` §3.1（「工具条最左四段」）更正为**左侧**（R1 裁决，下方元素表同改）；② mockup 内视图图标（📋 / 🚀 等）为示意笔误——不在 `BOARD-003` §3.4 八枚 emoji 预设（✨ 📦 🐛 👤 📅 🧪 🔥 🚒）内的图标均以预设清单为准（R3 裁决）。

| 元素        | 规格                                                                                                                                                     |
| ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 视图 Tab 条 | 「全部」固定首项（前端入口不入库，`BOARD-003` §3.6）+ 内置 + 本人个人视图（可见性 = 内置 + 本人，`BOARD-003` BR-11）；当前高亮；URL `?view_id=` 同源     |
| 布局切换    | 左侧 `列表/看板/甘特/表格` 四段器（勘误 2026-09-06（ADR-0020）：原「右侧」笔误，同 `BOARD-003` §3.1 工具条最左位）——切换只改 `view.layout` 独立字段（PATCH 视图，`BOARD-003` §4.1.1；`layout` 不入 display_props），filters 保持（BR-13） |
| 新建视图    | 从当前筛选态「保存为视图」弹层：名称 + 图标 + 布局（`BOARD-003` §3.4 同款；P2 仅个人视图，共享入口 P3 `BOARD-005`（s8 已交付））                                      |
...
- 企业筛选器体系完整：嵌套布尔、全字段、视图权限/锁定/全局下发（治理三件套）。本系统 P2 交付前两者；治理三件套的**列已建齐**（`is_locked`、`project=NULL` 全局位），共享/锁定已由 `BOARD-005`（s8）激活，跨项目全局视图归 P4 `BOARD-005` 激活零 DDL——「能力分期、结构一次」原则的又一实例。
