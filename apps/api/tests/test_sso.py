...
def _make_id_token(claims: dict) -> str:
    return pyjwt.encode(claims, _RSA_KEY, algorithm="RS256",
...
def idp_env(db, monkeypatch, settings):
    ...
    monkeypatch.setattr(oidc, "fetch_jwks",
                        lambda url, force=False: {"keys": [{**_jwk(), "kid": "test-kid"}]})
    owner = User.objects.create_user(email="sso-owner@rabbit.dev",
                                     password="Rabbit123!", display_name="主")
    ws = Workspace.objects.create(name="S", slug=f"w-sso-{owner.id.hex[:8]}",
    ...
    idp = IdentityProvider.objects.create(
        workspace=ws, protocol=IdentityProvider.PROTOCOL_OIDC,
        is_enabled=True, issuer="https://idp.test",
        client_id="rp-client", jwks_url="https://idp.test/jwks",
        created_by=owner,
    )
...
def _claims(env, sub="user-1", email="sso-user@rabbit.dev", name="SSO 用户",
            extra=None):
    c = {"iss": env["idp"].issuer, "aud": env["idp"].client_id,
         "sub": sub, "email": email, "name": name, "exp": 9999999999,
         "iat": 1700000000}
    if extra:
        c.update(extra)
    return c
...
def _begin(client: APIClient, env) -> tuple[str, str]:
    ...
    r = client.get(f"/api/v1/auth/sso/{env['ws'].slug}/sign-in/",
                   {"next": "/x/"})
...
def _complete(client: APIClient, env, claims: dict, txn: tuple | None):
    ...
    try:
        if txn:
            ...
            state = "whatever"
        return client.get("/api/v1/auth/sso/callback/",
                          {"code": "c", "state": state})
    finally:
...
class TestModelAndGate:
    def test_br01_unique_anchor(self, idp_env):
        ...
        SSOAccount.objects.create(idp=idp_env["idp"], user=u, subject="s1",
                                  email_at_binding="dup@rabbit.dev")
        u2 = User.objects.create_user(email="dup2@rabbit.dev", password="x")
        from django.db import IntegrityError
        with pytest.raises(IntegrityError):
            SSOAccount.objects.create(idp=idp_env["idp"], user=u2, subject="s1",
                                      email_at_binding="dup2@rabbit.dev")
    ...
    def test_gate_order_wrong_password_401(self, idp_env, settings):
        ...
        c = APIClient()
        r = c.post("/api/v1/auth/sign-in/",
                   {"email": "sso-owner@rabbit.dev", "password": "wrong"},
                   format="json")
    ...
    def test_gate_correct_password_enforced_403(self, idp_env):
        ...
        c = APIClient()
        r = c.post("/api/v1/auth/sign-in/",
                   {"email": "sso-owner@rabbit.dev", "password": "Rabbit123!"},
                   format="json")
    ...
    def test_route_endpoint(self, idp_env):
        c = APIClient()
        r1 = c.post("/api/v1/auth/sso/route/",
                    {"email": "sso-owner@rabbit.dev"}, format="json")
        assert r1.status_code == 200 and r1.json()["data"]["mode"] == "password"
        idp_env["idp"].enforce_sso = True
        idp_env["idp"].save(update_fields=["enforce_sso"])
        r2 = c.post("/api/v1/auth/sso/route/",
                    {"email": "sso-owner@rabbit.dev"}, format="json")
...
class TestOIDCFlow:
    def test_jit_provision_and_login(self, idp_env):
        ...
        assert SSOAccount.objects.filter(idp=idp_env["idp"],
                                         subject="user-1").exists()
        assert WorkspaceMember.objects.filter(
            workspace=idp_env["ws"], member=user,
            role=WorkspaceRole.MEMBER).exists()  # BR-08 默认角色
    ...
    def test_claim_flow(self, idp_env):
        # 邮箱已存在且设过密码 → 认领页
        User.objects.create_user(email="local@rabbit.dev", password="Local123!",
                                 display_name="本地")
        c = APIClient()
        txn = _begin(c, idp_env)
        r = _complete(c, idp_env, _claims(idp_env, sub="s-claim",
                                          email="local@rabbit.dev"), txn)
        ...
        # 密码对 → 绑定 + 会话
        r3 = c.post("/api/v1/auth/sso/claim/", {"password": "Local123!"},
                    format="json")
        assert r3.status_code == 200
        assert SSOAccount.objects.filter(
            subject="s-claim", user__email="local@rabbit.dev").exists()
        # 重放：txn 已消费 → SSO_TXN_INVALID
        r4 = c.post("/api/v1/auth/sso/claim/", {"password": "Local123!"},
                    format="json")
        assert r4.status_code == 401
        assert r4.json()["error"]["code"] == "SSO_TXN_INVALID"

    def test_sync_profile_department(self, idp_env):
        dept = Department.objects.create(workspace=idp_env["ws"],
                                         name="平台组", path=f"/{uuid.uuid4()}/",
                                         created_by=idp_env["owner"])
        # 首次登录（JIT）带部门
        c = APIClient()
        txn = _begin(c, idp_env)
        r = _complete(c, idp_env, _claims(idp_env, extra={"department": "平台组"}), txn)
        assert r.status_code == 302
        wm = WorkspaceMember.objects.get(workspace=idp_env["ws"],
                                         member__email="sso-user@rabbit.dev")
    ...
    def test_sync_profile_unmapped_kept(self, idp_env):
        ...
        txn = _begin(c, idp_env)
        _complete(c, idp_env,
                  _claims(idp_env, extra={"department": "不存在的部门"}), txn)
        wm = WorkspaceMember.objects.get(workspace=idp_env["ws"],
                                         member__email="sso-user@rabbit.dev")
    ...
    def test_signature_rejected(self, idp_env):
        ...
        token = pyjwt.encode(claims, bad_key, algorithm="RS256",
                             headers={"kid": "test-kid"})
        c = APIClient()
        txn = _begin(c, idp_env)
        import plane.sso.oidc as oidc_mod
        ...
        try:
            ...
            r = c.get("/api/v1/auth/sso/callback/",
                      {"code": "c", "state": txn[1]})
        finally:
            oidc_mod.exchange_token = orig
        assert r.status_code == 401
...
class TestAdminEndpoints:
    ...
    def test_config_secret_not_echoed(self, idp_env, settings):
        ...
        r = c.patch(f"/api/v1/workspaces/{idp_env['ws'].slug}/sso/",
                    {"client_secret": "topsecret"}, format="json")
    ...
    def test_br05_enforce_requires_owner_binding(self, idp_env):
        ...
        assert r.status_code == 409
        assert any(d.get("code") == "OWNER_BINDING_REQUIRED"
                   for d in r.json()["error"]["details"])
        # OWNER 完成 SSO 绑定后可开
        SSOAccount.objects.create(idp=idp_env["idp"], user=idp_env["owner"],
                                  subject="owner-sub",
                                  email_at_binding="sso-owner@rabbit.dev")
    ...
    def test_permission_owner_only(self, idp_env):
        member = User.objects.create_user(email="sso-mem@rabbit.dev",
                                          password="x")
        WorkspaceMember.objects.create(workspace=idp_env["ws"], member=member,
...
class TestMyBindings:
    def test_list_and_unbind(self, idp_env):
        ...
        b = SSOAccount.objects.create(idp=idp_env["idp"], user=u,
                                      subject="b1",
                                      email_at_binding="bind@rabbit.dev")
        c = APIClient()
    ...
    def test_br11_unbind_requires_password(self, idp_env):
        ...
        b = SSOAccount.objects.create(idp=idp_env["idp"], user=u, subject="b2",
                                      email_at_binding="nopw@rabbit.dev")
        c = APIClient()
        c.force_authenticate(user=u)
        r = c.delete(f"/api/v1/users/me/sso/bindings/{b.id}/")
        assert r.status_code == 400
        assert any(d.get("code") == "REQUIRED"
                   for d in r.json()["error"]["details"])
...
class TestSAML:
    ...
    def test_acs_invalid_assertion_401(self, saml_env):
        import base64
        c = APIClient()
        r = c.post("/api/v1/auth/sso/saml/acs/",
                   {"SAMLResponse": base64.b64encode(b"<not-saml/>").decode()})
        assert r.status_code == 401
