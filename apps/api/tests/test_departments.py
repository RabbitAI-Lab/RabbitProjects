...
@pytest.fixture()
def env(db):
    owner = User.objects.create_user(email="dept-owner@rabbit.dev", password="Rabbit123!", display_name="部门管理员")
    member = User.objects.create_user(email="dept-member@rabbit.dev", password="Rabbit123!", display_name="成员甲")
    member2 = User.objects.create_user(email="dept-member2@rabbit.dev", password="Rabbit123!", display_name="成员乙")
    inactive = User.objects.create_user(email="dept-inactive@rabbit.dev", password="Rabbit123!", display_name="停用者")
    ws = Workspace.objects.create(name="D", slug=f"w-dept-{owner.id.hex[:8]}", owner=owner, created_by=owner)
    for u, r in (
        (owner, WorkspaceRole.OWNER),
        (member, WorkspaceRole.MEMBER),
        (member2, WorkspaceRole.MEMBER),
        (inactive, WorkspaceRole.MEMBER),
    ):
...
class TestTreeManagement:
    ...
    def test_ut05_move_to_self_or_descendant_rejected(self, env):
        ...
        with pytest.raises(AppException) as ei:
            svc.move_department(actor=env["owner"], department=l1, new_parent_id=str(l4.id))
        assert getattr(ei.value, "error_code", "") == "RESOURCE_CIRCULAR_DEPENDENCY"
        # 移到自身同样拒绝
        with pytest.raises(AppException):
            svc.move_department(actor=env["owner"], department=l2, new_parent_id=str(l2.id))
    ...
    def test_ut06_move_subtree_depth_overflow_rejected(self, env):
        ...
        with pytest.raises(AppException):
            svc.move_department(actor=env["owner"], department=l1, new_parent_id=str(o4.id))
    ...
    def test_ut07_delete_nonempty_rejected(self, env):
        ...
        WorkspaceMember.objects.filter(workspace=env["ws"], member=env["member"]).update(department=leaf)
        with pytest.raises(AppException):
    ...
    def test_ut13_rebalance_on_spacing_collapse(self, env):
        ...
        svc.reorder_department(actor=env["owner"], department=b, sort_after_id=str(a.id))
        siblings = list(
            Department.objects.filter(workspace=env["ws"], parent=None, deleted_at__isnull=True).order_by("sort_order")
        )
        steps = [siblings[i + 1].sort_order - siblings[i].sort_order for i in range(len(siblings) - 1)]
...
class TestMemberAssignment:
    def test_ut08_assign_and_unassign(self, env):
        ...
        # 置 null = 移入未分配
        resp = c.patch(f"/api/v1/workspaces/{env['ws'].slug}/members/{wm.id}/", {"department_id": None}, format="json")
    ...
    def test_ut14_company_role_too_long_400(self, env):
        ...
        resp = _client(env["owner"]).patch(
            f"/api/v1/workspaces/{env['ws'].slug}/members/{wm.id}/", {"company_role": "超" * 65}, format="json"
        )
    ...
    def test_bulk_move_members(self, env):
        ...
        c = _client(env["owner"])
        resp = c.post(
            f"{_base(env)}{l1.id}/members/bulk-move/",
            {"member_ids": [str(wm1.id), str(wm2.id)], "department_id": str(l2.id)},
            format="json",
        )
        assert resp.status_code == 200, resp.json()
...
class TestGrantExpansion:
    ...
    def test_ut09_with_without_descendants(self, env):
        ...
        r = svc.expand_grant(
            actor=env["owner"], department=l3, project=proj, role=ProjectRole.CONTRIBUTOR, with_descendants=False
        )
        ...
        r2 = svc.expand_grant(
            actor=env["owner"], department=l1, project=proj, role=ProjectRole.CONTRIBUTOR, with_descendants=True
        )
        # 第一段已把 wm1 加进项目（unchanged 桶）——目标集口径 = added ∪ unchanged
        target2 = {e["member_id"] for e in r2.member_snapshot if e["action"] in ("added", "unchanged")}
    ...
    def test_ut10_idempotent_resync(self, env):
        ...
        proj = env["proj"]
        svc.expand_grant(actor=env["owner"], department=l1, project=proj, role=ProjectRole.CONTRIBUTOR)
        count_before = ProjectMember.objects.filter(project=proj).count()
        batch2 = svc.expand_grant(actor=env["owner"], department=l1, project=proj, role=ProjectRole.CONTRIBUTOR)
    ...
    def test_ut11_role_adjust_only_when_different(self, env):
        ...
        proj = env["proj"]
        svc.expand_grant(actor=env["owner"], department=l1, project=proj, role=ProjectRole.CONTRIBUTOR)
        b2 = svc.expand_grant(actor=env["owner"], department=l1, project=proj, role=ProjectRole.ADMIN)
    ...
    def test_ut12_inactive_member_skipped(self, env):
        ...
        b = svc.expand_grant(actor=env["owner"], department=l1, project=env["proj"], role=ProjectRole.CONTRIBUTOR)
        skipped = [e for e in b.member_snapshot if e["action"] == "skipped:member_inactive"]
        assert [e["member_id"] for e in skipped] == [str(wmi.member_id)]

    def test_ut15_guest_role_cap(self, env):
        l1, *_ = _mk_tree4(env)
        # member2 降为 GUEST
        WorkspaceMember.objects.filter(workspace=env["ws"], member=env["member2"]).update(role=WorkspaceRole.GUEST)
        wm1, wm2, _ = self._assign_members(env, l1, l1)
        b = svc.expand_grant(
            actor=env["owner"], department=l1, project=env["proj"], role=ProjectRole.CONTRIBUTOR
        )  # 15 > COMMENTER
    ...
    def test_ut16_soft_deleted_member_excluded(self, env):
        ...
        WorkspaceMember.objects.filter(pk=wm2.id).update(
            deleted_at=__import__("django.utils.timezone", fromlist=["timezone"]).now()
        )
        b = svc.expand_grant(actor=env["owner"], department=l1, project=env["proj"], role=ProjectRole.CONTRIBUTOR)
    ...
    def test_ut18_batch_snapshot_consistency(self, env):
        ...
        assert (
            (b.added_count + b.role_changed_count + b.skipped_count + b.unchanged_count) == len(b.member_snapshot) == 3
        )
    ...
    def test_ut20_delete_granted_department_batch_survives(self, env):
        ...
        # 清空成员与子部门后删除部门
        WorkspaceMember.objects.filter(workspace=env["ws"], department=l1).update(department=None)
        # 深度逆序删子树（先叶子后父，BR-04 空部门约束）
        for d in sorted(
            Department.objects.filter(workspace=env["ws"], deleted_at__isnull=True, path__startswith=l1.path).exclude(
                pk=l1.pk
            ),
        ...
        # 批次详情端点仍可溯源（404 不发生）
        resp = _client(env["owner"]).get(f"{_base(env)}{l1_id}/grants/{b.id}/")
        assert resp.status_code == 200
    ...
    def test_grant_location_and_response_shape(self, env):
        ...
        resp = _client(env["owner"]).post(
            ...
            {"project_id": str(env["proj"].id), "role": ProjectRole.CONTRIBUTOR, "with_descendants": True},
            format="json",
        )
    ...
    def test_grant_role20_requires_admin(self, env):
        ...
        with pytest.raises(AppException) as ei:
            svc.expand_grant(actor=env["member"], department=l1, project=env["proj"], role=ProjectRole.ADMIN)
    ...
    def test_archived_project_rejected(self, env):
        ...
        proj2 = Project.objects.create(
            name="归档P", identifier="DPA", workspace=env["ws"], created_by=env["owner"], status=Project.Status.ARCHIVED
        )
        with pytest.raises(AppException) as ei:
            svc.expand_grant(actor=env["owner"], department=l1, project=proj2, role=ProjectRole.CONTRIBUTOR)
...
class TestStats:
    def test_stats_direct_vs_with_descendants(self, env):
        l1, l2, l3, l4 = _mk_tree4(env)
        WorkspaceMember.objects.filter(workspace=env["ws"], member=env["member"]).update(department=l3)
        WorkspaceMember.objects.filter(workspace=env["ws"], member=env["member2"]).update(department=l1)
    ...
    def test_it06_unassigned_identity(self, env):
        ...
        l1, l2, l3, l4 = _mk_tree4(env)
        WorkspaceMember.objects.filter(workspace=env["ws"], member=env["member"]).update(department=l3)
        total = WorkspaceMember.objects.filter(workspace=env["ws"], is_active=True, deleted_at__isnull=True).count()
        assigned = WorkspaceMember.objects.filter(
            workspace=env["ws"], is_active=True, deleted_at__isnull=True, department__isnull=False
        ).count()
        unassigned = WorkspaceMember.objects.filter(
            workspace=env["ws"], is_active=True, deleted_at__isnull=True, department__isnull=True
        ).count()
        assert total == assigned + unassigned  # 恒等式（IT-06 对账口径）

    def test_issue_count_by_assignee_department(self, env):
        l1, *_ = _mk_tree4(env)
        WorkspaceMember.objects.filter(workspace=env["ws"], member=env["member"]).update(department=l1)
        state = env["proj"].states.first()
        issue = Issue.objects.create(project=env["proj"], name="部门任务", state=state, created_by=env["owner"])
...
class TestPermissionMatrix:
    ...
    def subjects(self, env, db):
        ...
        admin = User.objects.create_user(
            email="dept-admin@rabbit.dev", password="Rabbit123!", display_name="空间管理员"
        )
        guest = User.objects.create_user(email="dept-guest@rabbit.dev", password="Rabbit123!", display_name="访客")
        WorkspaceMember.objects.create(
            workspace=env["ws"], member=admin, role=WorkspaceRole.ADMIN, created_by=env["owner"]
        )
        WorkspaceMember.objects.create(
            workspace=env["ws"], member=guest, role=WorkspaceRole.GUEST, created_by=env["owner"]
        )
        return {"owner": env["owner"], "admin": admin, "member": env["member"], "guest": guest}
    ...
    def test_pm02_write_endpoints(self, env, subjects):
        ...
        for who in ("owner", "admin"):
            r = _client(subjects[who]).post(_base(env), {"name": f"新部门-{who}"}, format="json")
            assert r.status_code == 201, f"{who} 建部门应 201"

    def test_pm03_admin_cannot_assign_owner_row(self, env, subjects):
        l1, *_ = _mk_tree4(env)
        owner_wm = WorkspaceMember.objects.get(workspace=env["ws"], member=env["owner"])
        r = _client(subjects["admin"]).patch(
            f"/api/v1/workspaces/{env['ws'].slug}/members/{owner_wm.id}/", {"department_id": str(l1.id)}, format="json"
        )
        assert r.status_code == 403
        assert r.json()["error"]["code"] == "PERM_ROLE_INSUFFICIENT"
    ...
    def test_non_member_404(self, env):
        outsider = User.objects.create_user(
            email="dept-outsider@rabbit.dev", password="Rabbit123!", display_name="外人"
        )
