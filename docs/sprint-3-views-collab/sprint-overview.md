...
| 实时同步 | live WebSocket：拖卡 / 状态 / 评论 / 指派 / 新建删除推送；在线成员感知 | 富文本协同编辑（`COLLAB-004` 的 Yjs 通道随 Sprint 4 文档协作点亮）；消息已读回执（P3） |
...
| `COLLAB-004` | WebSocket 房间推送 / 多人同步 | live 房间协议 + 事件扇出 | Sprint 4 文档协同、`WF-003` |
...
| 分组契约 | 分组列从字段 `options` / 枚举表生成，**禁止 `SELECT DISTINCT` 扫数据表**（架构文档 §6.4）；空组恒渲染 |
| 批量契约 | 单事务全成败；上限 100；`VALIDATION_BULK_LIMIT_EXCEEDED` 超限即拒；同批 Activity 共享 epoch + batch 聚合（`TASK-010` BR-12） |
...
| 性能 | 分组看板首屏 SQL ≤ 10 条；动态流首屏 P95 < 200ms；WS 推送端到端 < 500ms |
...
- [ ] 批量：100 条上限与超限 409；单事务回滚验证（中途失败零残留）；动态流聚合为「批量更新了 N 个任务」。
